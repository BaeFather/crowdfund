...
if(substr($mgtKey, 0, 1)=='C') {
	...
	if(substr($mgtKey, 0, 1)=='P') {
		$doc_type = 'Cashbill';
	}
	else {
		$sql = "
			SELECT
				B.member_type, B.is_creditor, B.is_owner_operator, B.mb_co_reg_num
			FROM
				cf_product_give A
			INNER JOIN
		...
		if($DATA['member_type']=='2') {
			...
			if($DATA['is_creditor']=='Y' && $DATA['is_owner_operator']=='1' && $DATA['mb_co_reg_num']) {
				$doc_type = 'Taxinvoice';
			}
			else {
				$doc_type = 'Cashbill';
			...
			$msg_close('발행받은 회원의 정보가 존재하지 않습니다.');
		}
	}
}
...
//$log_sql = "INSERT INTO TaxinvoiceLog (mgtKey, req_type, action, code, msg, proc_mb_id, req_date) VALUES ('{$mgtKey}', '{$record_req_type}', '팝업URL조회', '{$code}', '{$message}', '{$_SESSION['ss_mb_id']}', NOW())";
//sql_query($log_sql);
