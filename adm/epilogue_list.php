...
<div class="local_ov01 local_ov">
	...
	<span>전체 투자후기 <?=($total_count) ? $total_count : 0; ?>건</span>
</div>

<div class="local_desc01 local_desc">
	<ol>
		<li>홈 > 이용안내 > 투자후기를 관리할 수 있습니다.</li>
		<li><strong>새로추가</strong>를 눌러 투자후기 글을 생성합니다.</li>
		<li>마우스로 드래그하여 순서를 변경할 수 있습니다.</li>
		<li>새로추가 후 선택수정을 하면 순서가 정렬됩니다.</li>
	</ol>
	<a href="/bbs/epilogue.php">투자후기글로 이동</a><br/><br/>
</div>

<div class="btn_add01 btn_add" style="padding-bottom:7px;overflow:hidden;text-align:left;">
	<form id="frmSearch" name="frmSearch" method="get" class="form-horizontal">
		<ul class="col col-md-* list-inline" style="width:100%;padding-left:0;margin-bottom:5px">
			<li>통합검색 :</li>
			<li><?=fn_general_select($S1,"",fn_epilogue_section(),"::투자후기 구분::","S1","class='form-control input-sm'","");?>
			</li>
			<li>
				<input type="submit" class="btn btn-sm" value="검색하기" />
			</li>
		</ul>
	</form>
</div>
...
<div class="btn_add01 btn_add" style="padding-bottom:7px;overflow:hidden;">
	<div style="float:left;width:50%;text-align:left;">
		&nbsp;
		<?=fn_general_select("","radio",fn_epilogue_section(),"::투자후기 구분::","section","","");?>
		<?=fn_general_select("","",fn_epilogue_snskind(),"::SNS 구분::","snskind","class='input-sm' style='width:150px;'","");?>
		<button type="button" onClick="check_epilogue_section();" class="btn btn-sm">구분변경</button>
	</div>
	<div style="float:left;width:50%;">
		<button type="button" onClick="del_this();" class="btn btn-sm">선택삭제</button>
		<button type="button" onClick="sort_this();" class="btn btn-sm">선택수정</button>
		<button type="button" onClick="location.href='epilogue_reg.php';" class="btn btn-sm">새로추가</button>
	</div>
</div>
...
for($i=1; $row = sql_fetch_array($result); $i++) {
	...
	$row['thumb_url'] = (!empty($row['thumbnail']) && file_exists(G5_IMG_PATH."/review/".$row['thumbnail'])) ? $img_url.$row['thumbnail'] : G5_IMAGES_URL.'/review/sumnail_img01.jpg';

	$dis_txt = "";
	if ($row['display_yn']=="Y") $dis_txt="노출";
	else if ($row['display_yn']=="N") $dis_txt="비노출";
	else if ($row['display_yn']=="R") $dis_txt="대기";
?>
			<tr data-sort-order="<?=$nLoop?>">
				...
				<td class="text-center">
					<?=fn_general_select($row['section'],"txt",fn_epilogue_section(),"","section","","");?>
					<? if($row['section'] == "2") { echo "<br>\n".fn_general_select($row["snskind"],"txt",fn_epilogue_snskind(),"","","",""); } ?>
				</td>
				<td class="text-center"><?//=($row['display_yn']=='Y')?'노출':'비노출';?><?=$dis_txt?></td>
				<td class="text-center"><?=$row['id']?></td>
				<td><img src="<?=$row['thumb_url']?>" alt="<?=$row['subject']?>" width="150" height="100" /></td>
				<td>
					<span><?=$row["mem_name"].' ('.$row["mem_id"].')';?></span><br/>
				...
				<td>
					<?
					if (substr($row["regdate"],0,10)==date("Y-m-d")) $regdate = substr($row["regdate"],-8);
					else $regdate = substr($row["regdate"],0,10);

					echo $regdate;
					?>
				</td>
				<td class="text-center"><?=($row['sort'] == 9999) ? '미지정' : $row['sort'];?></td>
				<td class="text-center">
					<button type="button" class="btn btn-sm" onClick="go_link(<?=$row['id'];?>, 'edit');">수정</button>
					<button type="button" class="btn btn-sm" onClick="del_this(<?=$row['id'];?>);">삭제</button>
				</td>
			</tr>
<?
}

if($i <= 1) echo '<tr><td colspan="20" class="empty_table"><span>데이터가 없습니다.</span></td></tr>';
...
function go_link(id, mode){
	document.location.href = '/adm/epilogue_reg.php?id='+id+'&page=<? echo $page;?>&mode='+mode;
}
...
function check_epilogue_section(id){
	...
	var snskind = $("select[name='snskind']").val();
				$.ajax({
						url: g5_admin_url + "/epilogue_update.php?mode=update",
						type: "POST",
