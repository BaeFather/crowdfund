...
$sql = "
	SELECT
		A.member_idx, A.amount, A.insert_datetime, A.syndi_id, IF(B.member_type='2','법인회원','개인회원') AS type_title,
		...
		C.title,
		IF((SELECT COUNT(idx) FROM cf_product_invest_detail WHERE product_idx=A.product_idx AND member_idx=A.member_idx AND invest_state='Y' AND is_auto_invest='1')>0,'자동투자','일반투자') AS invest_type
	FROM
		cf_product_invest A
	LEFT JOIN
...
$page_title = "대상기간내 단일상품 " . price_cutting($target_amount) . "원 이상 투자자 현황 (".preg_replace("/-/", ".", $sdate)."~".preg_replace("/-/", ".", $edate).")";
...
echo "
	<table border=1 style='font-size:9pt'>
		<tr>
			...
			<td align=center bgcolor='#D9E1F2'>투자구분</td>
			<td align=center bgcolor='#D9E1F2'>투자금액</td>
			<td align=center bgcolor='#D9E1F2'>투자일시</td>
...
for($i=0,$j=$rows; $i<$rows; $i++,$j--) {
	...
	echo "
		<tr>
			...
			<td align=center>".$row['invest_type']."</td>
			<td align=right>".number_format($row['amount'])."</td>
			<td align=center>".$row['insert_datetime']."</td>
