...
$where_sub = "AND ( SELECT IFNULL(MAX(AA.idx),0) FROM cf_product_invest AA LEFT JOIN cf_product BB  ON AA.product_idx=BB.idx WHERE 1 AND AA.member_idx=A.member_idx AND AA.idx<A.idx AND AA.invest_state='Y'";
if($category2) {
	if( in_array($category2, array('1','2','3')) ) {
		$where_sub.= " AND BB.category='".$category2."'";
	}
	else {
		if($category2=='2A') $where_sub.= " AND BB.category='2' AND BB.mortgage_guarantees=''";
		if($category2=='2B') $where_sub.= " AND BB.category='2' AND BB.mortgage_guarantees='1'";
		if($category2=='3A') $where_sub.= " AND BB.category='3' AND BB.title LIKE '%면세점%'";
		if($category2=='3B') $where_sub.= " AND BB.category='3' AND BB.title LIKE '%소상공인%'";
	}
}
$where_sub.= ") = 0";
...
$sql0 = "
	SELECT
		...
		A.idx AS invest_idx,
		A.amount, A.insert_datetime,
		B.mb_id, B.member_type, B.member_investor_type,
		(IF(B.member_type='2', B.mb_co_name, B.mb_name)) AS mb_title,
		B.mb_hp, B.mb_co_reg_num,
		B.va_bank_code2, B.virtual_account2, B.va_private_name2, B.pid,
		A.syndi_id,
		(
			SELECT COUNT(idx) AS prev_invest_count
			FROM cf_product_invest
			WHERE idx < A.idx AND member_idx=A.member_idx AND invest_state='Y'
		) AS prev_invest_count
	FROM
		cf_product_invest A
	LEFT JOIN
...
for($i=0; $i<$rows0; $i++) {
	...
	$R = sql_fetch_array($res0);

	if($R['now_mb_level']=='200') {
		$R['mb_hp'] = '';
		$R['is_drop_member'] = 1;
	}
	else {
...
//print_rr($LIST,'font-size:11px'); exit;
...
?>
			...
			<select id='category2' name='category2'>
				<option value=''>이전 전체 투자내역 1도 없는 회원</option>
...
?>
			...
			<button type='button' class="btn-success" onClick="location.href='<?=$_SERVER['REQUEST_URI']?>&download=1';">엑셀 다운로드</button></p>
			</form>
...
echo "
		<table border=1 style='font-size:9pt;border-collapse:collapse;'>
			<tr>
				...
				<td align=center bgcolor='#D9E1F2'>".$sdate." 이전<br/>투자건수(타상품군)</td>
...
if($list_count) {
	...
	for($i=0,$j=$list_count; $i<$list_count; $i++,$j--) {
		...
		$print_prev_invest_count = ($LIST[$i]['prev_invest_count']>0) ? $LIST[$i]['prev_invest_count'] : "<span style='color:#AAA'>".$LIST[$i]['prev_invest_count']."</span>";
		...
		echo "
			<tr style='color:$fcolor;background:$bgcolor'>
				...
				<td align=right>".$print_prev_invest_count."</td>
