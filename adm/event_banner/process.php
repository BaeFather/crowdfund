...
<?php
	...
	{
		$strPost = ARRAY(
							ARRAY("SE","",""),ARRAY("page","","Y"),ARRAY("STXT","",""),ARRAY("S1","",""),ARRAY("S2","",""),ARRAY("S3","",""),ARRAY("S4","",""),
							ARRAY("cfcode","","Y"),ARRAY("sdate","","Y"),ARRAY("edate","","Y"),ARRAY("targeturl","","Y"),ARRAY("targetlink","",""),
							ARRAY("recyn","","Y"),ARRAY("reg_date","","Y"),ARRAY("sort_id","",""),ARRAY("title","","")
					);
	} ELSEIF($kind == "update") {
		$strPost = ARRAY(
							ARRAY("SE","","Y"),ARRAY("page","","Y"),ARRAY("STXT","",""),ARRAY("S1","",""),ARRAY("S2","",""),ARRAY("S3","",""),ARRAY("S4","",""),
							ARRAY("cfcode","","Y"),ARRAY("sdate","","Y"),ARRAY("edate","","Y"),ARRAY("targeturl","","Y"),ARRAY("targetlink","",""),
							ARRAY("recyn","","Y"),ARRAY("reg_date","","Y"),ARRAY("sort_id","",""),ARRAY("title","","")
					);
	} ELSEIF($kind == "del") {
...
//	echo "<BR>";
	...
	$strIFileName = fn_file_upload("i_file","/data/event",ARRAY("","","",""),"single");
	$strSFileName = fn_file_upload("s_file","/data/event",ARRAY("","","",""),"single");
	...
	IF($kind == "save" || $kind == "update")
	{
		...
		$strColumn	= ARRAY(
							"cfcode","sdate","edate","repimg","mrepimg",
							"targeturl","targetlink","recyn","reg_date","sort_id",
							"icount","mcount","title"
						);

		$strValues = ARRAY(
							$cfcode, $sdate, $edate, $strIFileName[0],$strSFileName[0],
							$targeturl, $targetlink, $recyn, $reg_date,$sort_id,
							$icount, $mcount, urldecode($title)
						);
		...
		$objval = ARRAY("retcode"=>"OK","retalert"=>STR_REPLACE("+"," ",urlencode("글이 정상 ".$strRet[0]." 되었습니다")),"retval"=>"/adm/event_banner/?".$strRet[1]);
		ECHO json_encode($objval);
		EXIT;
	}
?>