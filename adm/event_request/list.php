...
?>
		<tr>
			...
			<td align="center"><?=fn_general_txt($recyn,$gstrHelloEventRequest->FnRecyn())?></td>
			<td align="center"><?=$mb_id?></td>
			<td align="center"><?=$mb_name?></td>
			<td align="center"><span id="hp<?=$i?>" onMouseOver="swapText('hp<?=$i?>','<?=$mb_phone?>');" onMouseOut="swapText('hp<?=$i?>','<?=$mb_phoneval?>');" onClick="copy_trackback('<?php ECHO $mb_phone;?>');" style="cursor:pointer" <?=$mb_phone?>><?=$mb_phoneval?></span></td>
			<td align="center">
