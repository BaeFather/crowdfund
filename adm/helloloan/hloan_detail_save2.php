...
if ($hcseq) {
	...
	//$reset_sql = "DELETE FROM hloan_content_loan WHERE hcseq='$hcseq'";
	//sql_query($reset_sql); // 여기는 초기 입력페이지라 초기화 로직은 필요없음.

	for ($i=0 ; $i<count($P_creditor) ; $i++) {
		echo "구분 : ".$P_reg_gubun[$i]. " ";
		echo "채권자 : ".$P_creditor[$i]. " ";
		echo "채권최고액 : ".$P_limit_amount[$i]. " ";
		echo "기대출금액 : ".$P_loan_amount[$i]. " ";
		echo "설정율 : ".$P_loan_percent[$i]. " ";
		echo "채무자 : ".$P_debtor[$i]. " ";
		echo "등기목적 : ".$P_reg_obj[$i]. " ";
		echo "<br/>";
	...
	for ($i=0 ; $i<count($R_creditor) ; $i++) {
		echo "구분 : ".$R_reg_gubun[$i]. " ";
		echo "채권자 : ".$R_creditor[$i]. " ";
		echo "채권최고액 : ".$R_limit_amount[$i]. " ";
		echo "기대출금액 : ".$R_loan_amount[$i]. " ";
		echo "설정율 : ".$R_loan_percent[$i]. " ";
		echo "채무자 : ".$R_debtor[$i]. " ";
		echo "등기목적 : ".$R_reg_obj[$i]. " ";
		echo "<br/>";
...
?>
<?
FUNCTION fn_file_upload2222($fname,$strFileFolder,$strImgArr,$kind)
{
	global $_FILES;
	global $_POST;
	global $gstrNdate;
...
function file_upload222($strFileKind,$strWidth,$strHeight,$strImgThumKind,$strNumber,$strKind,$strFileName,$strFileNameTemp,$dtmDate,$strOrFileName,$strKindDel)
{
