// 아이디 체크
var strHellidUrl = "/adm/hellomember_new/id_check.php";
var loanProcessUrl = "/adm/hellomember_new/process.php";

function hellloan_id_check(idval,event)
{
		if(!event)
	  {
		   event =window.event;
		...
		if(!idval)
		{
			  $("#idtext").html("아이디가 없습니다. 아이디를 입력하여 주십시오.");
		}

		var idor = $("input[name='idor']").val();
		var str = "id="+idval+"&idor="+idor;
		...
		$.ajax({
			...
			success : function(data){
				if(data.retcode == "OK"){
					...
					$("#idtext").html("<span class='fred'>"+stralert+"</span>");
...
function check_w_form(fmname, event)
{
	  ...
	  var checkform = check_form(fmname);
		...
		var intLength = $("input[name='i_file[]']").length;

		for(var j=0;j<intLength;j++)
		{
			formData.append('i_file[]', $("input[name='i_file[]']")[j].files[0]);
		}
