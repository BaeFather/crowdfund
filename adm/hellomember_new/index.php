...
IF($ST==2) { $html_title = $menu['menu910'][5][1]; }
	...
	SWITCH($RD)
	{
		...
		CASE "2"	:	// read
			...
			$strColumn	=	ARRAY(
									...
									"mname","co_number1","co_number2","rname","rphone","ifile"
							);
			...
			IF($idx)
			{
				...
				$strWhere	=	" WHERE hmseq='".add_str($idx)."' AND section='".$ST."'";
				$strOrder	=	$intSeqName;
				$intLimit1	=	0;
				$intLimit2	=	1;
				$intStrlen	=	100;
			...
			$strListUrl = "?S2=".$S2."&STXT=".$STXT."&page=".$page."&ST=".$ST;
		...
		DEFAULT		:
			$num_per_page = 20;
			$intSeqName = "hmseq";
			$strColumn	= ARRAY($intSeqName,"phmseq","hid","cname","hname","hphone","reg_date","recyn","level","cname2","mname");
			$strTable	= "
			(
			SELECT st1.hmseq,st1.phmseq, hid, st1.cname, hname, hphone, reg_date, recyn, level, section,mname,IFNULL(st2.cname,'') as cname2 FROM
			(SELECT hmseq,phmseq, hid, cname, hname, hphone, reg_date, recyn, level ,mname, section FROM hloan_member_renew)  st1 LEFT JOIN (SELECT hmseq, cname FROM hloan_member_renew WHERE level='2') st2 ON st1.phmseq=st2.hmseq
			) t1
			";
			$frQuery	= "";

			$strWhere = " WHERE section='".$ST."'";
			IF($STXT) {
			...
			$strHelloKind = fn_hellloan_kind_renew();
