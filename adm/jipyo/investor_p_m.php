...
<div class="tbl_head02 tbl_wrap">
	...
	<form method="post" name="f_srch">
		<select name="d_type" style="margin-right:20px;height:25px;width:250px;" onchange="go_inv();">
			...
			<option value="3" selected>개인회원 월별 평균투자금액 현황</option>
...
for ($i = $to_m ; $i >= $start_m ; $i--) {
	...
	?>
		<tr>
			...
			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][1]["avg_amt"]?number_format($LIST[$i][1]["avg_amt"]):""?></td>

			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][2]["sub_cnt"]?number_format($LIST[$i][2]["sub_cnt"]):""?></td>
			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][2]["avg_amt"]?number_format($LIST[$i][2]["avg_amt"]):""?></td>

			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][3]["sub_cnt"]?number_format($LIST[$i][3]["sub_cnt"]):""?></td>
			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][3]["avg_amt"]?number_format($LIST[$i][3]["avg_amt"]):""?></td>

			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][4]["sub_cnt"]?number_format($LIST[$i][4]["sub_cnt"]):""?></td>
			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][4]["avg_amt"]?number_format($LIST[$i][4]["avg_amt"]):""?></td>

			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][5]["sub_cnt"]?number_format($LIST[$i][5]["sub_cnt"]):""?></td>
			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][5]["avg_amt"]?number_format($LIST[$i][5]["avg_amt"]):""?></td>

			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][6]["sub_cnt"]?number_format($LIST[$i][6]["sub_cnt"]):""?></td>
			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][6]["avg_amt"]?number_format($LIST[$i][6]["avg_amt"]):""?></td>

			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][7]["sub_cnt"]?number_format($LIST[$i][7]["sub_cnt"]):""?></td>
			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][7]["avg_amt"]?number_format($LIST[$i][7]["avg_amt"]):""?></td>

			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][8]["sub_cnt"]?number_format($LIST[$i][8]["sub_cnt"]):""?></td>
			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][8]["avg_amt"]?number_format($LIST[$i][8]["avg_amt"]):""?></td>

			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][9]["sub_cnt"]?number_format($LIST[$i][9]["sub_cnt"]):""?></td>
			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i][9]["avg_amt"]?number_format($LIST[$i][9]["avg_amt"]):""?></td>

			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i]["total"]["cnt"]?number_format($LIST[$i]["total"]["cnt"]):""?></td>
			<td style="text-align:right; margin-right:12px; border:1px solid green;"><?=$LIST[$i]["total"]["avg_amt"]?number_format($LIST[$i]["total"]["avg_amt"]):""?></td>
...
function get_sum_case($ym) {
	...
	for ($i=0 ; $i<$cnt ; $i++) {
		...
		if ($row["cc"]==1) {
			$retval[1] = $row;
			if ($row["sub_cnt"]) $retval[1]["avg_amt"] = floor($row["sub_sum"]/$row["sub_cnt"]);
			$retval["total"]["cnt"] += $row["sub_cnt"];
			$retval["total"]["amt"] += $row["sub_sum"];
		} else if ($row["cc"]==2) {
			$retval[2] = $row;
			if ($row["sub_cnt"]) $retval[2]["avg_amt"] = floor($row["sub_sum"]/$row["sub_cnt"]);
			$retval["total"]["cnt"] += $row["sub_cnt"];
			$retval["total"]["amt"] += $row["sub_sum"];
		} else if ($row["cc"]==3) {
			$retval[3] = $row;
			if ($row["sub_cnt"]) $retval[3]["avg_amt"] = floor($row["sub_sum"]/$row["sub_cnt"]);
			$retval["total"]["cnt"] += $row["sub_cnt"];
			$retval["total"]["amt"] += $row["sub_sum"];
		} else if ($row["cc"]==4) {
			$retval[4] = $row;
			if ($row["sub_cnt"]) $retval[4]["avg_amt"] = floor($row["sub_sum"]/$row["sub_cnt"]);
			$retval["total"]["cnt"] += $row["sub_cnt"];
			$retval["total"]["amt"] += $row["sub_sum"];
		} else if ($row["cc"]==5) {
			$retval[5] = $row;
			if ($row["sub_cnt"]) $retval[5]["avg_amt"] = floor($row["sub_sum"]/$row["sub_cnt"]);
			$retval["total"]["cnt"] += $row["sub_cnt"];
			$retval["total"]["amt"] += $row["sub_sum"];
		} else if ($row["cc"]==6) {
			$retval[6] = $row;
			if ($row["sub_cnt"]) $retval[6]["avg_amt"] = floor($row["sub_sum"]/$row["sub_cnt"]);
			$retval["total"]["cnt"] += $row["sub_cnt"];
			$retval["total"]["amt"] += $row["sub_sum"];
		} else if ($row["cc"]==7) {
			$retval[7] = $row;
			if ($row["sub_cnt"]) $retval[7]["avg_amt"] = floor($row["sub_sum"]/$row["sub_cnt"]);
			$retval["total"]["cnt"] += $row["sub_cnt"];
			$retval["total"]["amt"] += $row["sub_sum"];
		} else if ($row["cc"]==8) {
			$retval[8] = $row;
			if ($row["sub_cnt"]) $retval[8]["avg_amt"] = floor($row["sub_sum"]/$row["sub_cnt"]);
			$retval["total"]["cnt"] += $row["sub_cnt"];
			$retval["total"]["amt"] += $row["sub_sum"];
		} else if ($row["cc"]==9) {
			$retval[9] = $row;
			if ($row["sub_cnt"]) $retval[9]["avg_amt"] = floor($row["sub_sum"]/$row["sub_cnt"]);
			$retval["total"]["cnt"] += $row["sub_cnt"];
			$retval["total"]["amt"] += $row["sub_sum"];
		}
	...
	if ($retval["total"]["cnt"]) $retval["total"]["avg_amt"] = $retval["total"]["amt"] / $retval["total"]["cnt"];
