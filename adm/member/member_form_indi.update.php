...
if( in_array($mode, array('new','edit')) ) {
	...
	$ARR['member_investor_type'] = $member_investor_type;
	...
	$ARR['is_owner_operator'] = $is_owner_operator;
	...
	if($is_owner_operator) {
		...
		$ARR['mb_co_name']     = sql_real_escape_string($mb_co_name);			// 업체명
		...
		$ARR['corp_phone']     = masterEncrypt($corp_phone, false);				// 사업장 연락처
		$ARR['corp_phone_ineb']= DGuardEncrypt($corp_phone);							// 사업장 연락처
	}
	else {
		$ARR['mb_co_name']     = '';			// 업체명
		$ARR['mb_co_reg_num']  = '';			// 사업자등록번호
		$ARR['mb_co_owner']    = '';			// 대표자명
		$ARR['corp_rdate']     = '';			// 설립일
		$ARR['corp_phone']     = '';			// 사업장 연락처
		$ARR['corp_phone_ineb']= '';
	}

	$ARR['zip_num']         = $zip_num;						// 우편번호
	...
	$ARR['eng_first_nm']     = sql_real_escape_string($eng_first_nm);
	$ARR['eng_last_nm']      = sql_real_escape_string($eng_last_nm);
	...
	if( $account_num = preg_replace('/(-| )/', '', $account_num) ) {
		...
		$account_num_ineb = DGuardEncrypt($account_num);
	}
	...
	if($mode == 'new') {
		...
		if(sql_query($sqlx)) {
			...
			if($mb_f_no) {
				sql_query("UPDATE g5_member SET mb_f_no='$mb_f_no', edit_datetime=NOW() WHERE mb_no='$mb_no'");
			}
			else {
				sql_query("UPDATE g5_member SET mb_f_no=mb_no, edit_datetime=NOW() WHERE mb_no='$mb_no'");
			}
