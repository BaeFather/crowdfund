...
$sql = "SELECT * FROM {$g5['menu_table']} ORDER BY me_code ASC, me_id ASC";
//$sql = "SELECT * FROM {$g5['menu_table']} ORDER BY me_order ASC, me_id ASC";
...
<?php
	for($i=0; $i<$rows; $i++) {
		...
		$bgcolor = $fcolor = "";
		if($row['me_use']==1 && $row['me_mobile_use']==1) {
			$bgcolor = "";
			$fcolor = "";
...
?>
	<tr class="<?=$bg?> menu_list menu_group_<?=substr($row['me_code'], 0, 2)?>" style="background:<?=$bgcolor?>;color:<?=$fcolor?>">
		<td class="td_category<?=$sub_menu_class?>">
			<input type="hidden" name="code[]" value="<?=$row['me_code']?>">
			<label for="me_name_<?=$i?>" class="sound_only"><?=$sub_menu_info?> 메뉴<strong class="sound_only"> 필수</strong></label>
			<input type="text" name="me_name[]" value="<?=$me_name;?>" id="me_name_<?=$i?>" required class="required frm_input full_input">
		</td>
		<td>
			<label for="me_link_<?=$i?>" class="sound_only">링크<strong class="sound_only"> 필수</strong></label>
			<input type="text" name="me_link[]" value="<?=$row['me_link'] ?>" id="me_link_<?=$i?>" required class="required frm_input full_input">
		</td>
		<td class="td_mng">
		...
		<td class="td_num">
			...
			<input type="text" name="me_order[]" value="<?=$row['me_order'] ?>" id="me_order_<?=$i?>" class="frm_input" size="5">
		</td>
		<td class="td_mng">
