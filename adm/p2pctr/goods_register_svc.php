...
$url  = $p2p_host . "goods";
//if ($product_idx=="3107") $url  = $p2p_host . "data/goods";  // 이관용
$method = "POST";
?>
<?
...
$data["goods_info"]["goods_id"] = $goods_id;   // P2P온투업자 상품ID String ANS(40) (필수)
$data["goods_info"]["goods_type"] = get_goods_type($product_idx);  // 투자 상품유형 String AN(4) (필수)
$data["goods_info"]["goods_name"] = $prd_row["title"];   // 상품명 String max: 255 (필수)
$data["goods_info"]["goods_status"] = "T200";  // 상품모집 상태 String AN(4) (필수)  T200:모집중 , T210:모집완료
$data["goods_info"]["offering_amount_limit"] = (int)$prd_row["recruit_amount"]; // 모집금액 Number N(19) (필수)
$data["goods_info"]["offering_start_date"] = preg_replace('/[^0-9]/','', $prd_row["start_date"]);  // 모집 시작일 String Date (필수)
$data["goods_info"]["offering_end_date"]   = preg_replace('/[^0-9]/','', $prd_row["end_date"]); // 모집 종료일 String Date (필수)
$data["goods_info"]["loan_interest_rate"] = (float)$prd_row["loan_interest_rate"]; // 대출금리 Number N(5,2) (필수)
//$data["goods_info"]["fee_etc"] = (int)$fee_etc; // 수수료 및 제비용 Number N(19)
$data["goods_info"]["profit_rate"] = (float)$prd_row["invest_return"]; // 수익률 Number N(5, 2) (필수)
//$data["goods_info"]["goods_detail_info"]["collection_info"]["collection_type"] = "CO99";  // 채권추심 방식 AN(4)  CO99 채권추심 기타
//$data["goods_info"]["goods_detail_info"]["collection_info"]["collection_fee"] = 0;   // 채권추심 수수료 Number (19)
//$data["goods_info"]["goods_detail_info"]["collection_info"]["collection_note"] = "";   // 채권추심 기타 내용 max:128byte
$data["goods_info"]["goods_detail_info"]["split_yn"] = false;  // 분할모집상품 여부 true / false

$data["loan_register_info"]["loan_register_id"] = $prd_row["loan_register_id"]; // P2P온투업자 대출신청 ID ANS(40) (필수)
...
$data["borrower_info"]["type"] = $brw_info["brw_type"]; // 차입자 유형 string an(4)
if ($data["borrower_info"]["type"]=="B300") $data["borrower_info"]["business_register_no"] = $brw_info["business_register_no"]; // 차입자 유형 string an(4)

if ($mode=="send" AND !$prd_row["goods_id"]) {
...
<table class="table table-bordered table-condensed">
	...
	<tr>
		<th>모집금액</th>
		<td style="text-align:center;vertical-align:middle;">
			<?=number_format($data["goods_info"]["offering_amount_limit"])?>
		</td>
		<td style="background-color:#F5F5F5;"></td>
	</tr>
	<tr>
		<th>모집일</th>
		<td style="text-align:center;vertical-align:middle;">
			<?=$data["goods_info"]["offering_start_date"]?> ~ <?=$data["goods_info"]["offering_end_date"]?>
		</td>
		<td style="background-color:#F5F5F5;"></td>
	</tr>
	<tr>
		<th>대출금리</th>
		<td style="text-align:center;vertical-align:middle;">
			<?=$data["goods_info"]["loan_interest_rate"]?>
		</td>
		<td style="background-color:#F5F5F5;"></td>
	</tr>
	<!--tr>
		<th>수수료 및 제비용</th>
		<td style="text-align:center;vertical-align:middle;">
			<?=number_format($data["goods_info"]["fee_etc"])?>
		</td>
	...
	<tr>
		<th>분할모집상품 여부</th>
		<td style="text-align:center;vertical-align:middle;">
			<?=var_dump($data["goods_info"]["goods_detail_info"]["split_yn"])?>
		</td>
		<td style="background-color:#F5F5F5;"></td>
	</tr>
	<tr>
