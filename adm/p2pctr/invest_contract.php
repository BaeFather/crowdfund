...
for ($i=0 ; $i<$cnti ; $i++) {
	...
	for ($j=0 ; $j<count($repay_info["REPAY"]) ; $j++) {
		...
		$par_amt=0;
		//if ($repay_info["REPAY"][$j]["LIST"][$key]["partial_principal"]==$repay_info["REPAY"][$j]["LIST"][$key]["paied_amount"]) $par_amt=$repay_info["REPAY"][$j]["LIST"][$key]["partial_principal"];
		if (is_array($repay_info["REPAY"][$j]["PARTIAL"])) {
			$par_amt = $repay_info["REPAY"][$j+1]["LIST"][$key]["partial_principal"];
			//echo "$j $par_amt = ". $repay_info["REPAY"][$j]["LIST"][$key]["partial_principal"]." + ".$repay_info["REPAY"][$j]["LIST"][$key]["repay_principal"]."<br/>";;
			//echo "$par_amt <br/>";
		}
	...
	if ($mode=="send" AND !$rowi["contract_id"]) {
		...
		if ($resj["rsp_code"] == "A0000") {
			//$up_sql = "UPDATE cf_product_invest_detail SET contract_id = '$contract_id' WHERE idx = '$rowi[idx]' AND contract_id='' ";
...
<?
//echo "<pre>"; print_r($repay_info["REPAY"][3]["PARTIAL"]); echo "</pre>";
//echo "<pre>"; print_r($repay_info["REPAY"]); echo "</pre>";
?>