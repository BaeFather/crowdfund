...
?>
<?
$prd_sql = "SELECT idx, loan_contract_id, title
			  FROM cf_product WHERE idx='$product_idx'";
$prd_row = sql_fetch($prd_sql);

if (!$prd_row["loan_contract_id"]) die("대출계약 ID 없음");

$url  = $p2p_host . "loans/contract/".$prd_row["loan_contract_id"];
$method = "PUT";

$data = array();
$data["status"] = "S301";

if ($mode=="send" and $prd_row["loan_contract_id"]) {
	...
	if ($resj["rsp_code"] == "A0000") {
		...
		$main_sql = "SELECT * FROM p2pctr_product WHERE product_idx='".$prd_row["idx"]."'";
