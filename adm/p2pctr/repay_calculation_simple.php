...
function repayCalculationSim($product_idx, $invest_member_id='', $only_summary='') {
  ...
  $INI['invest_count']        = $PRDT['invest_count'];
	...
	for($i=0; $i<$invest_count; $i++) {
		...
		$sql2 = "
			SELECT
				mb_no, mb_id, member_type, mb_name, mb_co_name, mb_co_reg_num,
				is_creditor, is_owner_operator, receive_method, remit_fee,
				bank_code, account_num, bank_private_name, bank_private_name_sub,
				va_bank_code, virtual_account, va_private_name, va_bank_code2, virtual_account2, va_private_name2,
		...
		$r['va_bank_code']          = $r2['va_bank_code'];
		$r['virtual_account']       = $r2['virtual_account'];
		$r['va_private_name']       = $r2['va_private_name'];
	...
	for($i=0,$j=1; $i<$turn_count; $i++,$j++) {
		...
		$succ_sql = "
			...
			WHERE 1
				...
				AND turn = '".$RX['turn']."'
				AND turn_sno = '0'";
		//print_rr($ssql, "font-size:12px");
		$SROW = sql_fetch($succ_sql);

		$REPAY[$i]['SUCCESS'] = $SROW;
		//print_rr($REPAY[$i]['SUCCESS'], "font-size:12px");
	...
	for($i=0,$turn=1; $i<$turn_count; $i++,$turn++) {
		...
		for($j=0; $j<$invest_count; $j++) {
			...
			//--------------------------------------------------------------------------------------------
			// 이자정산지급일에 따른 세율 변환
			$interest_tax_ratio = $CONF['interest_tax_ratio'];		// 이자소득세 : 25%
			$local_tax_ratio    = $CONF['local_tax_ratio'];			// 지방세: 이자소득세의 10% => 합계 27.5%

			// 소득세율이 변경될 경우 (법인-개인간 개별설정값 적용)
			...
			//if($REPAY[$i]['repay_schedule_date'] >= $CONF['lastTaxChangeDate']) {
				$interest_tax_ratio = ($INVEST[$j]['member_type']=='2') ? $CONF['corp']['interest_tax_ratio'] : $CONF['indi']['interest_tax_ratio'];		// 이자소득세
				$local_tax_ratio    = ($INVEST[$j]['member_type']=='2') ? $CONF['corp']['local_tax_ratio'] : $CONF['indi']['local_tax_ratio'];		// 지방세: 이자소득세의 10%
			}
			//if($member['mb_id']=='admin_sori9th') echo $interest_tax_ratio . " " . $local_tax_ratio . "<br>\n";
			//--------------------------------------------------------------------------------------------
			...
			if($REPAY[$i]['turn'] < $INI['repay_turn']) {			//-- 일반회차 지급예정원금
				...
				if($product_idx=='1782') {
					...
					// 실지급된 원금 확인 (부분상환액 합계)
					$tsql = "SELECT ( ".$INVEST[$j]['amount']." - IFNULL(SUM(principal),0) ) AS real_remain_principal FROM cf_product_give WHERE invest_idx='".$INVEST[$j]['idx']."' AND turn_sno > 0";
			...
			$REPAY[$i]['LIST'][$j]['TAX']['interest_tax'] = $row['interest_tax'];						// 이자소득세
			$REPAY[$i]['LIST'][$j]['TAX']['local_tax']    = $row['local_tax'];							// 지방소득세
			$REPAY[$i]['LIST'][$j]['TAX']['sum']          = $row['interest_tax'] + $row['local_tax'];		// 소득세합계
			...
			if( $PRDT['ib_trust']=='Y' ) {
				...
				$ib_sql = "
					...
					ORDER BY
						rdate DESC
					LIMIT 1";
				//print_rr($ib_sql, "font-size:12px");
				$IB = sql_fetch($ib_sql);
			...
			$give_sql = "
				...
				WHERE 1
					...
					AND turn_sno = '0'
					AND is_overdue = 'N'";
			//print_rr($give_sql, "font-size:12px");
			...
			//echo "<font color='red'>" . number_format($REPAY[$i]['LIST'][$j]['invest_usefee']) . "</font><br/>\n";
			...
			if($REPAY[$i]['LIST'][$j]['is_creditor']=='Y') {
				...
				$REPAY[$i]['LIST'][$j]['TAX']['interest_tax'] = floor( ( ($REPAY[$i]['LIST'][$j]['invest_interest'] * $interest_tax_ratio) / 10) ) * 10;					// 당월 이자소득세 = 이자수익 * 0.25
				$REPAY[$i]['LIST'][$j]['TAX']['local_tax']    = floor( ( ($REPAY[$i]['LIST'][$j]['TAX']['interest_tax'] * $local_tax_ratio) / 10) ) * 10;				// 당월 지방소득세(원단위 절사)
			}
			$REPAY[$i]['LIST'][$j]['TAX']['sum'] = $REPAY[$i]['LIST'][$j]['TAX']['interest_tax'] + $REPAY[$i]['LIST'][$j]['TAX']['local_tax'];		// 당월 세금 합계
			$REPAY[$i]['LIST'][$j]['withhold']   = $REPAY[$i]['LIST'][$j]['TAX']['sum'] + $REPAY[$i]['LIST'][$j]['invest_usefee'];								// 당월 징수할 금액 (세금 + 플랫폼이용료)
			$REPAY[$i]['LIST'][$j]['interest']   = $REPAY[$i]['LIST'][$j]['invest_interest'] - $REPAY[$i]['LIST'][$j]['withhold'];								// 실 수령액

			//print_rr($REPAY[$i]['LIST'][$j], "font-size:12px;color:red");
		...
		if($i==0) {
			//print_rr($REPAY[$i],'font-size:11px;line-height:12px');
		}
		...
		$ptl_sql = "
			...
			ORDER BY
				turn_sno ASC";
		if($REPAY[$i]['turn']=='3') {
			//print_rr($REPAY[$i]['turn'] . "회차 부분상환내역" . $ptl_sql, "font-size:12px");
		}
		...
		if($ptl_rows) {
			...
			for($k=0; $k<$ptl_rows; $k++) {
				...
				if($PARTIAL['turn_sno']) {
					...
					//print_rr($succ_sql,'font-size:12px');
					...
					for($j=0; $j<$invest_count; $j++) {
						...
						//if($_SERVER['REMOTE_ADDR']=='220.117.134.164') print_rr($ptl_sql2,'font-size:12px');
						...
						//if($_SERVER['REMOTE_ADDR']=='220.117.134.164') echo $PARTIAL['amount']." * (".$PTLDETAIL['invest_importance']." / 100)<br>\n";
		...
		if( $REPAY[$i]['SUCCESS']['overdue_start_date'] > '0000-00-00' ) {
			...
			$overdue_sdate = $REPAY[$i]['SUCCESS']['overdue_start_date'];
			$overdue_edate = ($REPAY[$i]['SUCCESS']['overdue_end_date']=='' || $REPAY[$i]['SUCCESS']['overdue_end_date']=='0000-00-00') ? G5_TIME_YMD : $REPAY[$i]['SUCCESS']['overdue_end_date'];
			$total_overdue_days = repayDayCount($overdue_sdate, $overdue_edate);		// 연체대상일수

			$REPAY[$i]['OVERDUE']['start_date'] = $overdue_sdate;
			$REPAY[$i]['OVERDUE']['end_date']   = $overdue_edate;
			$REPAY[$i]['OVERDUE']['day_count']  = $total_overdue_days;
			...
			for($j=0; $j<$invest_count; $j++) {
				...
				$REPAY[$i]['OVERDUE']['LIST'][$j]['invest_usefee']   = floor($day_invest_usefee * $REPAY[$i]['OVERDUE']['day_count']);
				if($INVEST[$j]['remit_fee']=='1') { $REPAY[$i]['OVERDUE']['LIST'][$j]['invest_usefee'] = 0; }
			...
			//if($REPAY[$i]['repay_schedule_date'] >= $CONF['lastTaxChangeDate']) {
				...
				// 원천징수액 계산 (대부업 회원 일때 원천징수 제로 처리)
				if($REPAY[$i]['LIST'][$j]['is_creditor']=='Y') {
					$REPAY[$i]['OVERDUE']['LIST'][$j]['TAX']['interest_tax'] = 0;
					$REPAY[$i]['OVERDUE']['LIST'][$j]['TAX']['local_tax']    = 0;
				}
				else {
					$REPAY[$i]['OVERDUE']['LIST'][$j]['TAX']['interest_tax'] = floor( ( ($REPAY[$i]['OVERDUE']['LIST'][$j]['invest_interest'] * $interest_tax_ratio) / 10) ) * 10;					// 당월 이자소득세 = 이자수익 * 0.25
					$REPAY[$i]['OVERDUE']['LIST'][$j]['TAX']['local_tax']    = floor( ( ($REPAY[$i]['OVERDUE']['LIST'][$j]['TAX']['interest_tax'] * $local_tax_ratio) / 10) ) * 10;				// 당월 지방소득세(원단위 절사)
				}
				$REPAY[$i]['OVERDUE']['LIST'][$j]['TAX']['sum'] = $REPAY[$i]['OVERDUE']['LIST'][$j]['TAX']['interest_tax'] + $REPAY[$i]['OVERDUE']['LIST'][$j]['TAX']['local_tax'];		// 당월 세금 합계
				...
				//print_rr($OVD_REPAY_SUM, 'font-size:12px;color:#FF2222;');
