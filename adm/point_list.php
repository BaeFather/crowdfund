...
<div class="local_ov01 local_ov">
    <?=$listall ?>
    전체 <? echo number_format($total_count) ?> 건
    <?
...
<select name="sfl" id="sfl">
    <option value="mb_id"<? echo get_selected($_GET['sfl'], "mb_id"); ?>>회원아이디</option>
    <option value="po_content"<? echo get_selected($_GET['sfl'], "po_content"); ?>>내용</option>
</select>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col"><? echo subject_sort_link('mb_id') ?>회원아이디</a></th>
        <th scope="col">이름</th>
        <th scope="col">닉네임</th>
        <th scope="col"><? echo subject_sort_link('po_content') ?>포인트 내용</a></th>
        <th scope="col"><? echo subject_sort_link('po_point') ?>포인트</a></th>
        <th scope="col"><? echo subject_sort_link('po_datetime') ?>일시</a></th>
        <th scope="col">만료일</th>
    ...
    <tr class="<?=$bg; ?>">
        <td class="td_chk">
            ...
            <input type="hidden" name="po_id[<?=$i?>]" value="<?=$row['po_id'] ?>" id="po_id_<?=$i?>">
            <label for="chk_<?=$i; ?>" class="sound_only"><?=$row['po_content'] ?> 내역</label>
            <input type="checkbox" name="chk[]" value="<?=$i ?>" id="chk_<?=$i ?>">
        </td>
        <td class="td_mbid"><a href="?sfl=mb_id&amp;stx=<?=$row['mb_id'] ?>"><?=$row['mb_id'] ?></a></td>
        <td class="td_mbname"><? echo get_text($row2['mb_name']); ?></td>
        <td class="td_name sv_use"><div><?=$mb_nick ?></div></td>
        <td class="td_pt_log"><?=$link1 ?><?=$row['po_content'] ?><?=$link2 ?></td>
        <td class="td_num td_pt"><? echo number_format($row['po_point']) ?></td>
        <td class="td_datetime"><?=$row['po_datetime'] ?></td>
        <td class="td_date<?=$expr; ?>">
            <? if ($row['po_expired'] == 1) { ?>
            만료<? echo substr(str_replace('-', '', $row['po_expire_date']), 2); ?>
            <? } else echo $row['po_expire_date'] == '9999-12-31' ? '&nbsp;' : $row['po_expire_date']; ?>
        </td>
        <td class="td_num td_pt"><? echo number_format($row['po_mb_point']) ?></td>
    </tr>
...
<section id="point_mng">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td><input type="text" name="po_expire_term" value="<?=$po_expire_term; ?>" id="po_expire_term" class="frm_input" size="5"> 일</td>
        </tr>
        <? } ?>
        </tbody>
        </table>
