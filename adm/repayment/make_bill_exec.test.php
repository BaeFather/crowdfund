...
while($row = sql_fetch_array($res0)) {
	...
	// 특별처리상품 플래그 (초기상품중 종료일이 5일 이전일때 이전회차와 최종상환회차를 동일회차로 처리한 상품 구분)  : 110,115,126,127,149,151,157,168,172
	$exceptionProduct = ( in_array($PRDT['idx'] array(110,115,126,127,149,151,157,168,172)) ) ? true : false;
	$shortTermProduct = ( $PRDT['invest_period']==1 && $PRDT['invest_days'] > 0 ) ? 1 : 0;
