<?
###############################################################################
## 3194번 상품을 처리하기 위한 repay_calculate.php
###############################################################################

if(trim($_REQUEST['idx']) != '3194') exit;
...
if($PRDT['state']=='5') {
	...
	if($PRDT['loan_end_date'] == $PRDT['loan_end_date_orig']) {
		$early_repay_button = '<button type="button" onClick="alert(\'대출종료일자가 대출계약상 종료일자와 동일하여 중도상환 처리가 되지 않습니다!\');" class="btn btn-gray" style="margin:0;width:100%;">중도상환</button>';
	}
	else {

		// 최종회차 상환처리 확인
		$succsql = "SELECT idx, invest_give_state, invest_principal_give FROM cf_product_success WHERE 1 AND product_idx='".$PRDT['idx']."' AND turn='".$INI['repay_turn']."' ORDER BY turn_sno DESC LIMIT 1";		// 상환이 있을 수 있으므로 turn_sno 끝번만 가져온다.
...
if( in_array($PRDT['state'], array('1','2','4','5','7','8','9')) ) {
	...
	for($i=0,$turn=1; $i<$repay_count; $i++,$turn++) {
		...
		if( in_array($PRDT['state'], array('1','5','7','8')) ) {
			...
			if($turn==$INI['repay_turn']) {
				if($REPAY[$i]['SUCCESS']['loan_principal_state']=='Y') {
					...
					if($REPAY[$i]['SUCCESS']['invest_give_state']=='S') {		// 기관처리완료시 지급액션버튼 출력
						$repay_request_button = '<button type="button" class="btn btn-danger" onClick="loanInterestGive(\''.$PRDT['idx'].'\', \''.$REPAY[$i]['repay_date'].'\', \''.$turn.'\', \'principal\');">전체지급</button>';
					}
					if($REPAY[$i]['SUCCESS']['invest_give_state']=='Y') {		// 지급처리완료
						//$repay_request_button = '<button type="button"  class="btn btn-gray" style="color:gray">전체지급완료</button>';
					}
...
divideReady = function(idx, date, turn, repay_only) {
	...
	if(repay_only_val=='interest') {
		confirm_msg = '이자';
	}
	else if(repay_only_val=='principal') {
		confirm_msg = '원금';
	}
	else {
		confirm_msg = '원리금';
