...
if(!$PRDT['idx']) {
	...
	if($PRDT['loan_mb_no']) {
		$LOANER = sql_fetch("SELECT mb_no, mb_id, member_type, mb_name, mb_co_name, mb_hp, va_bank_code2, virtual_account2, va_private_name2  FROM g5_member WHERE mb_no='".$PRDT['loan_mb_no']."' AND member_group='L'");
	...
	$INVEST['advance_invest_amount'] = sql_fetch("SELECT IFNULL(SUM(amount),0) AS sum_amount FROM cf_product_invest_detail WHERE product_idx='".$PRDT['idx']."' AND invest_state='Y' AND is_advance_invest='Y'")['sum_amount'];
	$INVEST['auto_invest_amount']    = sql_fetch("SELECT IFNULL(SUM(amount),0) AS sum_amount FROM cf_product_invest_detail WHERE product_idx='".$PRDT['idx']."' AND invest_state='Y' AND is_auto_invest='1'")['sum_amount'];
	$INVEST['normal_invest_amount'] = $INVEST['amount'] - $INVEST['auto_invest_amount'];
	$INVEST['end_date'] = sql_fetch("SELECT insert_datetime FROM cf_product_invest WHERE product_idx='".$PRDT['idx']."' AND invest_state='Y' ORDER BY idx DESC LIMIT 1")['insert_datetime'];

	$PAID['last_paid_turn'] = sql_fetch("SELECT IFNULL(MAX(turn),0) AS max_turn FROM cf_product_give WHERE product_idx='".$prd_idx."' AND turn_sno='0' AND is_overdue='N'")['max_turn'];
...
$sql = "SELECT COUNT(idx) AS cnt FROM {$bill_table} WHERE product_idx='".$PRDT['idx']."'";
...
.tab_container ul.tabX li.on { border:1px solid #ccc; background-color:#fff; border-bottom-color:#fff; color:#000 }
...
<!-- 투자자 정산요약 정보 START //-->
			<table class="table table-bordered prdt_table mb20" style="border:2px solid #000">
				<tr>
					<td class="subtitle"><span id="repaySummary_switch" class="switch">▼ 투자자 정산요약 정보</span></td>
				</tr>
				<tr>
					<td style="padding:0">

						<div id="repaySummary" class="pd8">
							<table class="tblx table-bordered prdt_table">
								<colgroup>
									<col style="width:%">
								...
								<tr align="center" style="background:#EEE">
									<td rowspan="2">상환구분</td>
									<td colspan="2">원금</td>
									<td colspan="2">이자</td>
									<td colspan="2">플랫폼이용료</td>
									<td colspan="2">원천징수</td>
									<td colspan="2">세후이자</td>
								</tr>
								<tr align="center" style="background:#EEE">
									<td>예정</td>
									<td>지급</td>
								...
								<tr align="right" style="font-size:12px">
									...
									<td id="principal_repay">0</td>
									<td id="principal_paid">0</td>
									<td id="invest_interest_repay">0</td>
									<td id="invest_interest_paid">0</td>
									<td id="fee_repay">0</td>
									<td id="fee_paid">0</td>
									<td id="tax_repay">0</td>
									<td id="fee_paid">0</td>
									<td id="interest_repay">0</td>
									<td id="interest_paid">0</td>
								</tr>
								<tr align="right" style="font-size:12px">
									<td align="center">연체</td>
									<td align="center">-</td>
									<td align="center">-</td>
									<td id="ovd_invest_interest_repay">0</td>
									<td id="ovd_invest_interest_paid">0</td>
									<td id="ovd_fee_repay">0</td>
									<td id="ovd_fee_paid">0</td>
									<td id="ovd_tax_repay">0</td>
									<td id="ovd_fee_paid">0</td>
									<td id="ovd_interest_repay">0</td>
									<td id="ovd_interest_paid">0</td>
								</tr>
							</table>
						</div>
...
<? for($i=0,$j=1; $i<$INI['total_repay_turn']; $i++,$j++) { ?>
					<li class="<?=($i==0)?'on':'';?>" data-turn="<?=$j?>" style="min-width:2%;max-width:8%;width:<?=(98/$INI['total_repay_turn']).'%';?>"><?=$j?>회차</li>
<? } ?>
				</ul>
<? for($i=0,$j=1; $i<$INI['total_repay_turn']; $i++,$j++) { ?>
