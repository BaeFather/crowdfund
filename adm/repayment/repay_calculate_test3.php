...
SELECT
	A.idx, A.start_num, A.state, A.title, A.recruit_amount, A.repay_acct_no,
	A.loan_start_date, A.loan_end_date,
	(SELECT MAX(insert_datetime) FROM cf_product_invest WHERE product_idx=A.idx AND invest_state='Y') AS max_invest_dt,
	(IFNULL((SELECT ref_no FROM KSNET_VR_ACCOUNT WHERE VR_ACCT_NO=A.repay_acct_no AND REF_NO=A.idx), '')) AS ref_no
FROM
	cf_product A
WHERE 1
	AND A.repay_acct_no = '56212670593630'
	AND A.state IN('1','2','5','8','9')
ORDER BY
	state ASC,
	max_invest_dt DESC,
	A.loan_end_date DESC;
