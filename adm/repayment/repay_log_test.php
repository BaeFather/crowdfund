...
if ($display_mode=="excel") {
	...
	header( "Content-Disposition: attachment; filename=taxlist.xls" );
...
if($date_field) {
	if($date_field=='A.banking_date') {
		if($sdate) $where.= " AND LEFT(A.banking_date,10) >= '".$sdate."'";
		if($edate) $where.= " AND LEFT(A.banking_date,10) <= '".$edate."'";
	}
	else {
...
if($investor_field && $investor_kwd) {
	if($investor_field=='investor_title') {
		$where.= " AND (B.mb_name LIKE '%".$investor_kwd."%' OR B.mb_co_name LIKE '%".$investor_kwd."%')";
	}
	else {
...
if($turn) {
	if($turn=='finished_turn') {
		$where.= " AND A.principal > 0";
	}
	else if($turn=='ovd_turn') {
		...
		<ul class="col col-md-* list-inline" style="padding:0;margin-bottom:5px">
			<li>
				<select name="turn" class="form-control input-sm">
					...
					<option value='finished_turn' <?=($turn=='finished_turn')?'selected':''?>>원금상환회차</option>
					<option value='ovd_turn' <?=($turn=='ovd_turn')?'selected':''?>>연체금상환회차</option>
<?
...
?>
			...
			<li>
				<select name="bill_result" class="form-control input-sm">
					...
					<option value="null" <?=($give_rcv_method=='2')?'selected':'';?>>미발행</option>
					<option value="1" <?=($give_rcv_method=='1')?'selected':'';?>>발행</option>
				</select>
			</li>
			<li>
...
?>
	...
	<div style="width:100%;height:100%;padding:0">
		<table class="table table-striped table-bordered table-hover" style="font-size:12px">
			<tr align="center">
				<th style="background:#F8F8EF;min-width:70px">지급번호</th>
				<th style="background:#F8F8EF;min-width:60px"">품번</th>
				<th style="background:#F8F8EF">상품명</th>
				<th style="background:#F8F8EF"><div class="overhidden">상환회차</div></th>
				<th style="background:#F8F8EF"><div class="overhidden">회원번호</div></th>
				<th style="background:#F8F8EF"><div class="overhidden">회원구분</div></th>
				<th style="background:#F8F8EF">아이디</th>
				<th style="background:#F8F8EF">성명.법인명</th>
				<th style="background:#F8F8EF">지급원금</th>
				<th style="background:#F8F8EF">이자(세후)</th>
				<th style="background:#F8F8EF">세금</th>
				<th style="background:#F8F8EF">플랫폼이용료</th>
				<th style="background:#F8F8EF">지급방법</th>
				<th style="background:#F8F8EF">지급계좌</th>
				<th style="background:#F8F8EF"><div class="overhidden">세금계산서<br>문서번호</div></th>
				<th style="background:#F8F8EF">지급일시</th>
			</tr>
...
?>
			<tr align="center" style="background:<?=$bgcolor?>;">
				...
				<td align="right"><?=number_format($LIST[$i]['principal'])?><?=$display_mode<>"excel"?"원":""?></td>
