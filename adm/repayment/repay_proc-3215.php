	...
	if($action=='loan_interest_give') {
		...
		for($j=0,$k=1; $j<$repay_count; $j++,$k++) {
			...
			if($TARGET_REPAY['LIST'][$j]['receive_method']=='2') {		// 가상계좌환급
				...
				if($MB['insidebank_after_trans_target']=='Y')  $proc_auth_flag = false;
			...
			if($ROW['idx']) {
				...
				if($repay_only=='interest') {			// ------- 이자만 지급 처리
					if( ($ROW['interest'] <> $TARGET_REPAY['LIST'][$j]['interest']) || ($ROW['interest_tax'] <> $TARGET_REPAY['LIST'][$j]['TAX']['interest_tax']) || ($ROW['local_tax'] <> $TARGET_REPAY['LIST'][$j]['TAX']['local_tax']) || ($ROW['fee'] <> $TARGET_REPAY['LIST'][$j]['invest_usefee']) )
					{
						...
						insert_point($TARGET_REPAY['LIST'][$j]['mb_id'], $final_trans_amount, $point_subject, '@repay', $member['mb_id'], $member['mb_id'].'-'.uniqid(''));		//**** 예치금으로 등록 ****//
				...
				else if($repay_only=='principal') {			// ------- 원금만 지급 처리
					...
					if( ($ROW['principal'] <> $TARGET_REPAY['LIST'][$j]['repay_principal']) || ($ROW['interest'] <> $TARGET_REPAY['LIST'][$j]['interest']) || ($ROW['interest_tax'] <> $TARGET_REPAY['LIST'][$j]['TAX']['interest_tax']) || ($ROW['local_tax'] <> $TARGET_REPAY['LIST'][$j]['TAX']['local_tax']) || ($ROW['fee'] <> $TARGET_REPAY['LIST'][$j]['invest_usefee']) )
					{
				...
				if($ib_trust) {
					...
					if($proc_auth_flag && $bank_code && $account_num) {
						...
						if($TARGET_REPAY['LIST'][$j]['receive_method']=='1') {
							...
							//print_rr($ARR, 'font-size:12px'); echo ""<br/>\n";
