...
if($repay_turn) {
	...
	$TARGET_REPAY = $REPAY[$repay_arr_no]['OVERDUE'];
	$repay_count  = count($TARGET_REPAY['LIST']);

	$FLAG = sql_fetch("
		SELECT
			idx, overdue_receive, overdue_ib_request_ready, overdue_give, overdue_start_date, overdue_end_date
		FROM
			cf_product_success
		WHERE 1
			AND product_idx = '".$prd_idx."' AND turn = '".$repay_turn."' AND turn_sno = '0' AND overdue_start_date > '0000-00-00'");

	$repay_amount = $TARGET_REPAY['SUM']['invest_interest'] - $TARGET_REPAY['SUM']['withhold'];	// 당회차 지급필요총액
...
if($action) {
	...
	if($action=='overdue_rcv_success') {
		...
		if($FLAG['idx']) {
			$sql = "UPDATE cf_product_success SET overdue_receive = 'Y', overdue_end_date = CURRENT_DATE() WHERE idx = '".$FLAG['idx']."' AND overdue_receive = ''";
		}
		else {
			$sql = "INSERT INTO cf_product_success (overdue_receive, product_idx, turn, turn_sno, overdue_end_date, date) VALUES ('Y', '".$prd_idx."', '".$repay_turn."', '0', '".$req_date."', CURRENT_DATE())";
		}
	...
	}		// end if($action=='overdue_rcv_success')
	...
	///////////////////////////////////////////////////////////////////////////////
	if($action=='overdue_devide_ready') {
		...
		if($FLAG['idx']=='' || $FLAG['overdue_receive'] != 'Y') {
		...
		//$REPAY_RECEIPT_NB = 'M' . $TARGET_REPAY['LIST'][$j]['member_idx'] . 'P' . $prd_idx . 'I' . $TARGET_REPAY['LIST'][$j]['invest_idx'];		// 원리금 수취권 번호
			...
			if(!$RECORDED['cnt']) {
				...
				$TR_AMT_P = 0;																										// 상환원금 (연체이자 정산시 원금은 포함하지 않음)
			...
			if( sql_query($dtlsql) ) {
				...
				sql_query("
					UPDATE
						cf_product_success
					SET
						overdue_ib_request_ready = 'Y'
					WHERE 1
						AND idx = '".$FLAG['idx']."'
						AND overdue_ib_request_ready = ''");

				$msg = "대출상품명 :  " . $PRDT['title'] . "\n" .
						 "상환회차 :  " . $repay_turn ."회차(연체이자)\n" .
	...
	}		// end if($action=='overdue_devide_ready')


	///////////////////////////////////////////////////////////////////////////////
	// 연체이자 배분요청 전문 구성 및 등록 (제3자 예치시스템)
	///////////////////////////////////////////////////////////////////////////////
	if($action=='overdue_devide_request') {
	...
	}		// end if($action=='overdue_devide_request')


	/////////////////////////////////////////////////////////////////////////////
	// 연체이자 지급 실행
	/////////////////////////////////////////////////////////////////////////////
	if($action=="overdue_give") {
		...
		for($j=0,$k=1; $j<$repay_count; $j++,$k++) {
			...
			// 면세점 연체상품 연체건 수취방식 고정 : 예치금충전방식
			if( in_array($prd_idx, array('3023','3187','3194','3201','3215','3223','3224')) ) {
				$TARGET_REPAY['LIST'][$j]['receive_method'] = 2;			// 처리 후 주석처리할것
			}
					...
					if($proc_auth_flag && $bank_code && $account_num) {
						...
						if($TARGET_REPAY['LIST'][$j]['receive_method']=='1') {
							...
							if($final_trans_amount > 0) {
								...
								$ARR['GUAR_MEMO']       = '연체상('.$PRDT['idx'].')';																// 예치금모계좌통장메모
									...
									if( $RETURN_ARR['RCODE']=='IS0102') {
										...
										$LAST_REQUEST = sql_fetch("
											...
											WHERE 1
												...
												AND exec_path='/adm/repayment/repay_proc_overdue.php'
						...
						/////////////////////////////////////////////////////////////////////////////
							...
							if($final_trans_amount > 0) {
								...
								insert_point($TARGET_REPAY['LIST'][$j]['mb_id'], $final_trans_amount, $point_subject, '@overdue_repay', $member['mb_id'], $member['mb_id'].'-'.uniqid(''));
							}
	...
	}		// end if($action=="overdue_give")


	///////////////////////////////////////////////////////////////////////////////
	// 연체이자 지급완료 플래그 처리
	///////////////////////////////////////////////////////////////////////////////
	if($action=='overdue_give_success') {
			...
			SELECT
				(
					...
					FROM
						IB_FB_P2P_REPAY_REQ_DETAIL
					WHERE 1
						AND DC_NB = '".$prd_idx."' AND turn = '".$repay_turn."' AND turn_sno = '0' AND is_overdue = 'Y'
				) AS reques_count,
				(
					SELECT
						COUNT(idx)
					FROM
						cf_product_give
					WHERE 1
						AND product_idx = '".$prd_idx."' AND turn = '".$repay_turn."' AND turn_sno = '0' AND is_overdue = 'Y' AND banking_date IN NOT NULL
				)  AS give_count");
		...
		$res = sql_query("UPDATE cf_product_success SET overdue_give = 'Y' WHERE idx = '".$FLAG['idx']."' AND overdue_give != 'Y'");
		...
		if($res) {
			$prdt_update_sql = "
				...
				SET
					...
					overdue_turn = ''
				WHERE
					idx='".$prd_idx."' AND state='8'";
			$res2 = sql_query($prdt_update_sql);					// 상태값: 연체중->이자상환중 전환
	...
	}		// end if($action=='overdue_give_success')
