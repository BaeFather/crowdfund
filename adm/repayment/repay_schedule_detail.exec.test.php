...
$sql = "
	SELECT
		A.bill_date, A.remain_principal,
		( SELECT FLOOR(IFNULL(SUM(day_interest),0)) FROM {$bill_table} WHERE invest_idx = A.invest_idx AND A.turn = '".$turn."' AND A.turn_sno = '0' AND A.is_overdue = 'N' ) AS _interest,
		( SELECT FLOOR(IFNULL(SUM(fee),0)) FROM {$bill_table} WHERE invest_idx = A.invest_idx AND A.turn = '".$turn."' AND A.turn_sno = '0' AND A.is_overdue = 'N' ) AS _fee,
