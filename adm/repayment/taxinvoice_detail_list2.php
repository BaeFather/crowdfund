...
$where.= " AND C.invest_usefee > '0.00'";
if($type) {
...
for($i=0,$j=$num; $i<$list_count; $i++,$j--) {
	...
	$add_border = ($type=='p' && substr($LIST[$i]['mgtKey'],0,1)=='C') ? 'border:2px solid #FF2222;' : '';

	$taxinvoice_vlink = "/LINKHUB/hellofunding/Taxinvoice/GetPopUpURL.php?mgtKey=".$LIST[$i]['mgtKey'];

?>
		<tr align="center" style="background:<?=$bgcolor?>;<?=$add_border?>">
			<td><?=$j?></td>
			<td><?=$print_exec_dt?></td>
			<td><?=$LIST[$i]['idx']?></td>
