...
var LIST_deposit_amt2 = new Array();
...
function check_data(i,tdate) {
	$.ajax({
		...
		success:function(ret_data) {
			...
			LIST_POINT_REPAY_AMT[i] = Number(res.POINT_REPAY_AMT); // 원리금 => 예치금 전환 금액
			LIST_deposit_amt2[i] = Number(res.deposit_amt2);       // 원리금 => 예치금 전환 금액
...
<div class="tbl_head02 tbl_wrap">
	...
	<div id="hi_container" style="min-width: 310px; min-height: 400px; margin: 0 auto; height: 500px;"></div>
...
$(document).ajaxStop(function() {
	...
	chart.addSeries({
		name : "원리금=>예치금",
		data : LIST_POINT_REPAY_AMT
	});

	LIST_deposit_amt2.reverse();
	chart.addSeries({
		name : "원리금 처리",
		data : LIST_deposit_amt2
	});
	//console.log(LIST_POINT_REPAY_AMT);

});
</script>

<script>
function number_format_jsc(val)
{
    if(val){
...
var BAL_DEP_AMT = JSON.parse('<?= addslashes(json_encode($BAL_DEP_AMT)) ?>');
BAL_DEP_AMT.reverse();

var BAL_ALLAMT = JSON.parse('<?= addslashes(json_encode($BAL_ALLAMT)) ?>');
BAL_ALLAMT.reverse();

var BAL_RET_AMT = JSON.parse('<?= addslashes(json_encode($BAL_RET_AMT)) ?>');
BAL_RET_AMT.reverse();

var REPAY_AMT = JSON.parse('<?= addslashes(json_encode($REPAY_AMT)) ?>');
REPAY_AMT.reverse();

var LOAN_AMT = JSON.parse('<?= addslashes(json_encode($LOAN_AMT)) ?>');
LOAN_AMT.reverse();
            ...
            dataLabels: {
                ...
                formatter: function() {
                    ...
                    if (this.x == this.series.data[this.series.data.length-1].category) {
                        return this.series.name + ': '+ number_format_jsc(this.y);
                    } else {
                        return null;
                    }
