...
if($sdate) $give_search.= " AND LEFT(banking_date, 10) >= '$sdate'";
if($edate) $give_search.= " AND LEFT(banking_date, 10) <= '$edate'";

$give_search2 = " 1=1 ";
$give_search2.= " AND B.ib_trust='Y' ";
$give_search2.= " AND A.banking_date > '2017-10-15 00:00:00'";
if($sdate) $give_search2.= " AND LEFT(A.banking_date, 10) >= '$sdate'";
if($edate) $give_search2.= " AND LEFT(A.banking_date, 10) <= '$edate'";
...
for($i=0; $i<$list_count;$i++) {
	...
	$sqlx = "
		SELECT
			...
			IFNULL(SUM(TR_AMT), 0) AS deposit_amt
		FROM
			IB_FB_P2P_IP
		WHERE	1
			AND TR_AMT_GBN = '10'
			AND LEFT(ERP_TRANS_DT, 8)='".preg_replace('/-/', '', $LIST[$i]['TARGET_DATE'])."'";
	$IB_DETAIL_LOG = sql_fetch($sqlx);
	$LIST[$i]['deposit_cnt'] = $IB_DETAIL_LOG['deposit_cnt'];
	$LIST[$i]['deposit_amt'] = $IB_DETAIL_LOG['deposit_amt'];


	$PTOTAL['BAL_DEP_CNT'] += $LIST[$i]['BAL_DEP_CNT'];
	$PTOTAL['BAL_DEP_AMT'] += $LIST[$i]['BAL_DEP_AMT'];
	...
	$GIVE = sql_fetch("
		...
		WHERE 1
			...
			AND LEFT(banking_date, 10)='".$LIST[$i]['TARGET_DATE']."'");

	$LIST[$i]['POINT_REPAY_CNT'] = $GIVE['cnt'];
	$LIST[$i]['POINT_REPAY_AMT'] = $GIVE['amount'];
	...
	$GIVE2 = sql_fetch("
		SELECT
			COUNT(A.idx) AS cnt,
			(IFNULL(SUM(A.interest), 0)+IFNULL(SUM(A.principal), 0)) AS amt
		FROM
			cf_product_give A
		LEFT JOIN
			cf_product B  ON A.product_idx=B.idx
		WHERE 1
			AND B.ib_trust='Y'
			AND LEFT(A.banking_date, 10)='".$LIST[$i]['TARGET_DATE']."'");
	$LIST[$i]['deposit_cnt2'] = $GIVE2['cnt'];
	$LIST[$i]['deposit_amt2'] = $GIVE2['amt'];

	$POINT_REPAY = sql_fetch("
		SELECT
			COUNT(B.mb_no) AS cnt,
			IFNULL(SUM(A.po_point),0) AS amt
		FROM
			g5_point A
		LEFT JOIN
			g5_member B  ON A.mb_no = B.mb_no
		WHERE 1
			AND B.mb_level BETWEEN 1 AND 8
			AND B.member_group='F'
			AND A.po_content LIKE BINARY('%회차 원리금%')
			AND LEFT(A.po_datetime, 10)='".$LIST[$i]['TARGET_DATE']."'");
	$LIST[$i]['point_repay_cnt'] = $POINT_REPAY['cnt'];
	$LIST[$i]['point_repay_amt'] = $POINT_REPAY['amt'];
	...
	<div>
		...
		<div class="form-group">
			<ul class="col-sm-10 list-inline" style="width:100%;">
				...
				<li><button type="submit" class="btn btn-sm btn-primary">검색</li>
...
?>
		...
		<tr align="right" style="background:#EFEFEF;color:brown">
			...
			<td><?=number_format($PTOTAL['POINT_REPAY_CNT'])?>건</td>
			<td style="border-right:1px solid #999;"><?=number_format($PTOTAL['POINT_REPAY_AMT'])?>원</td>

			<td><?=number_format($PTOTAL['BAL_RET_CNT'])?>건</td>
			...
			//-->
			<td><?=number_format($PTOTAL['deposit_cnt2'])?>건</td>
			<td style="border-right:1px solid #999;"><?=number_format($PTOTAL['deposit_amt2'])?>원</td>
...
?>
		<tr align="right" style="background:<?=$tr_bgcolor?>">
			<td style="text-align:center;border-right:1px solid #999;"><?=$LIST[$i]['TARGET_DATE']?></a></td>
			<td><a href="<?=$DETAIL['deposit']?>" style="color:<?=$FCOLOR['deposit_cnt']?>"><?=number_format($LIST[$i]['BAL_DEP_CNT'])?>건</a></td>
			<td style="border-right:1px solid #999;"><a href="<?=$DETAIL['deposit']?>" style="color:<?=$FCOLOR['deposit_amt']?>"><?=number_format($LIST[$i]['BAL_DEP_AMT'])?>원</a></td>

			<td><a href="<?=$DETAIL['principal']?>" style="color:<?=$FCOLOR['point_repay_cnt']?>"><?=number_format($LIST[$i]['POINT_REPAY_CNT'])?>건</a></td>
			<td style="border-right:1px solid #999;"><a href="<?=$DETAIL['principal']?>" style="color:<?=$FCOLOR['point_repay_amt']?>"><?=number_format($LIST[$i]['POINT_REPAY_AMT'])?>원</a></td>
			...
			//-->
			<td><?=number_format($LIST[$i]['deposit_cnt2'])?>건</td>
			<td style="border-right:1px solid #999;"><?=number_format($LIST[$i]['deposit_amt2'])?>원</td>
