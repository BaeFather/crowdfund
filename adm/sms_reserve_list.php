...
function update_sms_res() {
	...
	//$sql = "select * from cf_agent_msgqueue where state<>'3' and reservedTime<now()";
	//$sql = "select * from cf_agent_msgqueue where state<>'3'";
	$sql = "select * from cf_agent_msgqueue where state<1";

	$res = sql_query($sql,G5_DISPLAY_SQL_ERROR,$link3);
	$cnt = sql_num_rows($res);

	$agent_table = "agent_msgqueue";
...
$row = sql_fetch("SELECT count(distinct send_id) AS cnt FROM $db_table WHERE $where", G5_DISPLAY_SQL_ERROR, $link3);
$total_count = $row['cnt'];
$page_rows   = 100;
...
$sql  = "SELECT *,count(id) as grp_cnt FROM $db_table WHERE $where group by send_id ORDER BY $order DESC LIMIT $from_record, $page_rows";
$res  = sql_query($sql, G5_DISPLAY_SQL_ERROR, $link3);
...
if($num > 0) {
	for($i=0,$j=1; $i<$list_count; $i++,$j++) {
		...
		if ($LIST[$i]['moved']=="Y") $print_state = "완료";
		...
		if ($LIST[$i]['deleted']=='Y') { $print_state = "취소"; $state_color = "#333333"; $result_color = "#333333"; }
?>
				<tr align="center">
					<td>
