...
//update_sms_res();
$rsv_sql = "select count(idx) as cou from cf_agent_msgqueue where state='0' and deleted<>'Y'";
			...
			<form name="form1" id="form1" method="get">
				...
				<ul class="col-sm-10 list-inline" style="width:100%;padding-left:0;margin-bottom:5px">
					...
					<li style="float:right;">
						<button type="button" id="kp_admin" class="btn btn-success" style="width:100px;margin-left:0px;" onclick="go_kp_admin();">KP Admin</button>
					</li>
				</ul>
				<ul class="col-sm-10 list-inline" style="width:100%;padding-left:0;margin-bottom:5px">
					...
					<li style="float:left;">
						<select id="reqdateM" name="reqdateM" class="form-control">
					...
					<?
					if ($_REQUEST["jtest"]=="Y" or 2>1) {
						?>
						<button type="button" id="search_reserve" class="btn btn-success" style="width:140px;margin-left:10px;">예약현황<?=$rsv_cnt?" ($rsv_cnt)":""?></button>
						<?
					}
					?>
					</li>
...
if($num > 0) {
	for($i=0,$j=1; $i<$list_count; $i++,$j++) {
		...
		switch($LIST[$i]['result']) {
			...
			case '-1' : $print_result = "미확인";  break;
			default   : $print_result = "실패(".$LIST[$i]['result'].")";    break;
		}
...
$("#search_reserve").on('click', function() {
	self.location.href = "sms_reserve_list.php";
});
...
function go_kp_admin() {
	window.open("http://110.45.180.36:8020/kshotb2b/login.do","KP_ADMIN");
}
</script>
