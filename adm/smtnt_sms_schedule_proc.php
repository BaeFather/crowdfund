...
if($mode=='select_delete') {
	...
	$sql = "DELETE FROM Msg_Tran WHERE id IN($idx_arr)";
	...
	*/
	...
	$sql = "DELETE FROM Msg_Tran WHERE $where";
	$res = sql_query($sql, G5_DISPLAY_SQL_ERROR, $link3);
	$delete_count = sql_affected_rows($link3);
	...
	*/
	...
	$sql = "UPDATE cf_Msg_Tran SET deleted='Y' WHERE send_id IN($idx_arr)";
	$res = sql_query($sql, G5_DISPLAY_SQL_ERROR, $link3);
	$cancel_count = sql_affected_rows($link3);

	echo number_format($chk_count). "건 요청 중 " . number_format($cancel_count) ."명 취소";
