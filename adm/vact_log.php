...
if($key_search && $keyword) {
	if($key_search=='iacct_no') { $sql_search.= " AND A.iacct_no LIKE '%".$keyword."%'"; }
}

$sql_search2 = " 1=1 ";
$sql_search2.= " AND virtual_account=A.iacct_no";
if($key_search && $keyword) {
	if(!$iacct_no) { $sql_search2.= " AND {$key_search} LIKE '%".$keyword."%' "; }
}
...
$rows = 50;						//$rows = $config['cf_page_rows'];
...
$sql = "
	SELECT
		...
		(SELECT mb_id FROM g5_member WHERE $sql_search2) AS mb_id,
		(SELECT member_type FROM g5_member WHERE $sql_search2) AS member_type,
		(SELECT mb_name FROM g5_member WHERE $sql_search2) AS mb_name,
		(SELECT mb_co_name FROM g5_member WHERE $sql_search2) AS mb_co_name
	FROM
		vacs_ahst A
	WHERE
		$sql_search
	...
	<div>
		...
		<div class="form-group">
			<ul class="col-sm-10 list-inline" style="margin-bottom: 4px;">
				<li><input type="text" id="sdate" name="sdate" value="<?=$sdate?>" class="form-control datepicker" placeholder="입금일(시작)"></li>
				<li>~</li>
				<li><input type="text" id="edate" name="edate" value="<?=$edate?>" class="form-control datepicker" placeholder="입금일(종료)"></li>
				<li>까지</li>
				<li>|</li>
				<li>
			...
			<ul class="col-sm-10 list-inline" style="margin-bottom: 0;">
				<li>
					<select name="key_search" class="form-control">
						...
						<option value="iacct_no" <?=($key_search=='iacct_no')?'selected':''; ?>>가상계좌번호</option>
					</select>
				</li>
	...
	if($list_count > 0) {
		for($i=0; $i<$list_count; $i++) {
			...
			if($LIST[$i]['mb_id']) {
				...
				$mb_co_name = '<span style="color:brown">탈퇴회원</span>';
			}
