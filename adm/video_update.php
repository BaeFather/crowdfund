...
switch($mode)
{
	...
	try{
		...
		}else if(!filter_var($target_link, FILTER_VALIDATE_URL)){
		...
		// 정렬순서 중복제외
		$sortyn = sql_fetch("SELECT COUNT(*) FROM media_video_list WHERE sort = {$sort}");
		...
		{
			/*$thumbnailData = sql_fetch("SELECT thumbnail FROM media_video_list WHERE id = {$id}");
			...
			$sql = "
				...
				WHERE id = '{$id}'";
			sql_query($sql);
			alert("수정되었습니다.","./video_list.php?page={$page}&id={$id}");
		}else{ // 등록
			...
			alert("등록되었습니다.","./video_list.php?page={$page}&id={$id}");
		}
	...
	case "delete" :
		...
		if(isset($id) && count($id) > 0)
		{
			...
			/*// 이미지 삭제
			$sql = sql_query("SELECT id, thumbnail FROM media_video_list WHERE id IN ({$ids})");

			while($row = sql_fetch_array($sql)){
				if(isset($row["thumbnail"]) && !empty($row["thumbnail"])){
					unlink(G5_IMG_PATH.'/review/'.$row["thumbnail"]);

					sql_query("UPDATE media_video_list SET thumbnail = '', thumbnail_origin = ''; WHERE id = {$row["id"]}");
				}
			}*/

			$sql = "DELETE FROM media_video_list WHERE id IN ({$ids})";
	...
	case "sort" :
		if(isset($id) && count($id) > 0)
		{
			...
			foreach($id as $index){
				$sql = "UPDATE media_video_list SET sort = '{$nLoop}' WHERE id = {$index}";
				$result = sql_query($sql);
				$nLoop += 1;
			}

			$ids = implode(',', $id);
			$sql = sql_query("SELECT id FROM media_video_list WHERE id NOT IN ({$ids}) AND sort >= {$nLoop} ORDER BY sort ASC regdate DESC");
			$idss = array();
			...
			foreach($idss as $index){
				$usql = "UPDATE media_video_list SET sort = '{$nLoop}' WHERE id = {$index}";
