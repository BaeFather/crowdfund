...
<div id="content">
	...
	<div id="loan">
		...
		<input type="hidden" name="rdo_aptaaaaa" value="1" /> 
		...
		<div class="loan4">
			...
			<ul class="radiobt1">
				<li><label class="apt_radio"><input type="radio" id="rdo_apt_a" name="rdo_apt" value="1" onclick="Rradio_OnOff('Radio_On');" checked><span>선택</span></label></li>
				<li class="pd"><label class="apt_radio"><input type="radio" id="rdo_apt_m" name="rdo_apt" value="2" onclick="Rradio_OnOff('Radio_Off');"><span>직접입력</span></label></li>
			</ul>
			...
			<ul id="Radio_On" style="display:;">
				<li class="step2 clear">
					...
					<input type="text" name="address" value="" id="address" onClick="win_zip_loan('wform', 'zipcode', 'address', 'address_detail', 'address2', 'address' , 'si' , 'gu' ,'dong' , 'apt_name');" class="addr_new" placeholder="주소를 입력해주세요" readonly >
...
function check_w_form_re(fmname, event) {
	...
	} else if (f.rdo_apt[1].checked) {
		if (!f.apt_name2.value) {
			alert("아파트 및 상세주소를 입력해주세요.");
			return;
		}
	} else {
		alert("선택 또는 직접입력중 하나를 선택하고 주소를 입력해 주세요.");
		return;
	}
...
function set_price() {
	...
	for (var i=0 ; i<sise_data["outH0001"]["list"].length; i++) {
		//if ($("#apt_area").val() == i+","+sise_data["outH0001"]["list"][i]["areaSerialNumber"]+","+sise_data["outH0001"]["list"][i]["areaSerialNumber"]) {
		if ($("#apt_area").val() == i+","+sise_data["outH0001"]["list"][i]["areaSerialNumber"]+","+sise_data["outH0001"]["list"][i]["exclusiveSpace"]) {
			idx = i;
		}
	}
	...
	$.ajax({
		...
		success : function(data) {
			...
			for (var i=0 ; i<data["outH0001"]["list"].length; i++) {
				...
				$("#apt_area").append("<option value='"+i+","+data["outH0001"]["list"][i]["areaSerialNumber"]+","+data["outH0001"]["list"][i]["exclusiveSpace"]+"'>"+data["outH0001"]["list"][i]["exclusiveSpace"]+" "+data["outH0001"]["list"][i]["supplySpaceType"]+"</option>");
			}
...
//	var addr = $("address").val();
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data["out"]["outB0002"]["list"].length<1) {
				$("#apt_area").empty();  // 평형선택 초기화
				$("#apt_area").append("<option value=''>전용면적 선택 (㎡)</option>");

				alert("입력된 주소의 시세를 찾을수 없습니다.\n직접입력화면으로 이동합니다.");
				$("#rdo_apt_m").prop("checked", true);
...
*/
	    ...
	    beforeSend: function(){
			$("#apt_area option").remove();
			$("#apt_area").append("<option value='' style='text-align-last: center; text-align: center; -ms-text-align-last: center; -moz-text-align-last: center;'>....... 검 색 중 .......</option>");
	    },
		complete: function(){
		}, 
		error : function(XMLHttpRequest, textStatus, errorThrown){
	...
	switch(zip_case) {
		case 1 :	//iframe을 이용하여 페이지에 끼워 넣기
			...
			if(element_wrap == null) {
				...
				//jQuery('form[name="'+frm_name+'"]').find('input[name="dong_num"]').before(element_wrap);
