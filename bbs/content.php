...
$co_id = trim($_REQUEST['co_id']);

if( $co_id == 'company' ) {
	header("HTTP/1.1 301 Moved Permanently");		// 301 항구적 이동, 302 임시로 이동
	header("Location: /company/introduce.php");
	//if(G5_IS_MOBILE) header("Location: /company/introduce_m.php");
	exit;
}

// 투자가이드 페이지 경로 변경 (2016-09-25 by sori9th)
if( $co_id == 'funding_guide' ) {
	header("HTTP/1.1 301 Moved Permanently");
	header("Location: /investment/guide.php");
	exit;
}

// 이용약관 및 개인정보보호정책 경로 변경 (2016-06-28 by sori9th)
if( in_array($co_id, array('provision','provision2','privacy','provision_loan','provision_ec','marketing_agreement','3rdparty_agreement','usecredit','identify','codeofethics')) ) {
	header("HTTP/1.1 301 Moved Permanently");
	header("Location: /company/".$co_id.".php");
	exit;
}

//if($member['mb_id']=='sori9th') { exit; }
