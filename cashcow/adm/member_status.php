...
$sql_common = " FROM {$g5['member_table']} A LEFT JOIN cf_visit_status B ON A.vi_idx=B.idx ";

$sql_search = " WHERE 1=1 ";
$sql_search.= " AND A.pid='".$_CONF['SYNDI_ID']."'";
...
// 등록회원
$sql   = "SELECT COUNT(*) AS cnt FROM g5_member WHERE pid='$pid'";
$row   = sql_fetch($sql);
$count1 = $row['cnt'];

// 탈퇴회원
$sql   = "SELECT COUNT(*) AS cnt FROM g5_member_drop WHERE pid='$pid'";
$row   = sql_fetch($sql);
$count2 = $row['cnt'];
		...
		<div id="content" style="position:absolute;">
			<div class="content investment" style="width:98%;margin:-50px auto;">
				...
				<ul class="tab_type03" style="margin:0">
					...
					<li data-gubun="tab3" onClick="location.href='join_status.php'">가입 통계</li>
					...
					<div style="margin-bottom:10px;">
						...
						<span style="margin-right:15px;">
							...
							<select name="key_search" class="frm_input" style="margin-left:10px;">
								...
								<option value="A.mb_3" <? if($key_search == 'A.mb_3'){echo 'selected';} ?>>RESPONSE_IDX</option>
							</select>
					...
					<table class="tblX">
						...
						<thead>
							<tr style="background-color:#EFEFEF">
								...
								<th scope="col" style="text-align:center;">가입일시</th>
								<th scope="col" style="text-align:center;">RESPONSE_IDX</th>
...
if($num > 0) {
	for($i=0,$j=1; $i<$rcount; $i++,$j++) {
		...
		$print_hp = masterDecrypt($LIST[$i]['mb_hp'],false);

?>
						<tr bgcolor="<?=$tr_bgcolor?>">
							<td style="text-align:center;"><?=$num?></td>
							<td style="text-align:center;"><?=$LIST[$i]['mb_name']?></td>
							<td style="text-align:center;"><?=substr($LIST[$i]['mb_id'],0,2)."****"?></td>
							<td style="text-align:center;"><?=substr($print_hp,0,7)."****"?></td>
							<td style="text-align:center;"><?=substr($LIST[$i]['mb_datetime'],0,16)?></td>
							<td style="text-align:center;"><?=$LIST[$i]['mb_3']?></td>
