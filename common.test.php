...
$http_origin = @EXPLODE("/",STR_REPLACE("https://","",$_SERVER["HTTP_REFERER"]));
...
if(IN_ARRAY($http_origin[0], $allowed_origin)) {
	header("Access-Control-Allow-Origin: {$strFullDomain}");
}
else {
	header("Access-Control-Allow-Origin: https://www.hellofunding.co.kr");
}
...
/*
	// 2020-03-26 공지
	$notice_start_date = '2020-03-26 15:30:00';
	$notice_end_date   = '2020-03-26 17:00:00';

	if(date('Y-m-d H:i:s') >= $notice_start_date && date('Y-m-d H:i:s') < $notice_end_date) {
		if( @$_SERVER['HTTP_USER_AGENT'] ) {
			$noti_image = '/images/system_notice_20190813.jpg';
			$width= ( @preg_match('/(android|iphone)/i', $_SERVER['HTTP_USER_AGENT']) ) ? '90%' : '';

			echo "
				<div style='width:100%;padding-top:150px;text-align:center'><img src='$noti_image' style='width:$width'></div>
				<script>
				function alertNoticeMsg() {
					alert('금융 전산망 점검으로 사이트 이용이 일시 제한됩니다. 고객 여러분들의 양해 부탁드립니다.\\n\\n'
						+ '[일시제한기간]\\n'
						+ ' " . date('Y년m월d일 H시', strtotime($notice_start_date)) . "부터 " . date('H시', strtotime($notice_end_date)) . "까지');
				}
				setTimeout(function(){ alertNoticeMsg(); }, 500);
				</script>
			";
			exit;
		}
...
/*
//점검공지
if( preg_match("/(220\.117\.134\.164)/", @$_SERVER['REMOTE_ADDR']) ) {

	$notice_start_date = '2020-04-28 15:00:00';
	$notice_end_date   = '2020-04-28 17:00:00';
	...
	if(date('Y-m-d H:i:s') >= $notice_start_date && date('Y-m-d H:i:s') < $notice_end_date) {
		if( @$_SERVER['HTTP_USER_AGENT'] ) {
			...
			echo "
				...
				<script>
				function alertNoticeMsg() {
					alert('금융전산서버 업그래이드 작업으로 인하여 사이트 이용이 일시 제한됩니다. 고객 여러분들의 양해 부탁드립니다.\\n\\n'
						+ '[일시제한기간]\\n'
						+ ' " . date('Y년m월d일 H시', strtotime($notice_start_date)) . "부터 " . date('H시', strtotime($notice_end_date)) . "까지');
				}
				setTimeout(function(){ alertNoticeMsg(); }, 500);
...
/*
if( !preg_match("/220\.117\.134/", @$_SERVER['REMOTE_ADDR']) &&  !preg_match("/(122\.42\.6\.248|211\.248\.149\.248|125\.177\.159\.44)/", @$_SERVER['REMOTE_ADDR']) && ( !@preg_match("/dev2\.wow4989\.co\.kr|wowstar\.co\.kr/i", @$_SERVER['HTTP_REFERER']) && !preg_match("/\/syndicate\/wowstar/i", @$_SERVER['PHP_SELF']) ) ) {
	if( preg_match("/\/bbs\/register_choice\.php/i", $_SERVER['PHP_SELF']) ) {
		echo "
			<script>
				alert('헬로펀딩 시스템 업데이트 진행으로 사이트 이용이 일시제한됨을 양해부탁드립니다.\\n\\n'
					+ '[일시제한사항]\\n'
					+ ' 1. 로그인\\n'
					+ ' 2. 회원가입\\n'
					+ ' 3. 예치금 입/출금\\n\\n'
					+ '[일시제한기간]\\n'
					+ ' 2017년 10월 11일 18시부터 10월 15일 18시까지\\n\\n'
					+ '(해당기간에도 투자자분들의 투자금 관리를 비롯한 모든 운영사항은 정상진행됩니다.)');
				location.replace('/');
			</script>";
	}
}
...
/*if(!preg_match("/220\.117\.134/", @$_SERVER['REMOTE_ADDR'])) {
	if(preg_match("/(iphone|ipad|ipod|android|blackberry|opera|mini|windows\sce|palm|smartphone|iemobile)/i", $_SERVER["HTTP_USER_AGENT"])) {
		include_once($_SERVER['DOCUMENT_ROOT']."/popup/repair_m.html");
	}
	else {
		include_once($_SERVER['DOCUMENT_ROOT']."/popup/repair.html");
	}
	exit;
}*/
...
if(!empty(@$_SERVER['HTTP_X_FORWARDED_FOR'])) $_SERVER['REMOTE_ADDR'] = $_SERVER['HTTP_X_FORWARDED_FOR'];  // KISA DDOS ZONE 이용시 클라이언트 IP가 KISA 방화벽 IP로만 전달되는것을 방지하기 위한 설정
...
if (file_exists($dbconfig_file)) {
	...
	/*
	if($_SERVER['REMOTE_ADDR']=='211.248.149.48')	{
		include_once(G5_LIB_PATH.'/common.lib.test.php');	// 공통 라이브러리(테스트용)
	}
	else {
		include_once(G5_LIB_PATH.'/common.lib.php');	// 공통 라이브러리
	}
	*/
	...
	//$IP_AREA = array('country_code'=>'국가코드','region_name'=>'주명','city'=>'도시명');


/*
	$connect_db2 = sql_connect(G5_MYSQL_HOST2, G5_MYSQL_USER2, G5_MYSQL_PASSWORD2) or die('MySQL Connect Error!!!');
	$select_db2  = sql_select_db(G5_MYSQL_DB2, $connect_db2) or die('MySQL DB Error!!!');

	// mysql connect resource $g5 배열에 저장 - 명랑폐인님 제안
	$g5['connect_db2'] = $connect_db2;

	sql_set_charset('utf8', $connect_db2);
	if(defined('G5_MYSQL_SET_MODE2') && G5_MYSQL_SET_MODE2) sql_query("SET SESSION sql_mode = ''");
	if (defined(G5_TIMEZONE)) sql_query(" set time_zone = '".G5_TIMEZONE."'");
*/
