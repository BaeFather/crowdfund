...
<link href="m_status.css?ver=2" rel="stylesheet">
...
<div id="mobile">
	...
	<div class="title">
		<ul>
			<li><span class="h3">투자현황</span></li><br><br>
			<li class="date">
	...
	<div class="title">
		<ul>
			<li><span class="h3">유형별 투자현황</span></li><br><br>
			<li class="date">
	...
	<div class="box_type_invest_hello">
		<div class="wrap_type_invest_hello">
			<table class="invest-data" id="inState">
				<thead>
					<tr>
						<th align="right" width="20%">누적 투자 금액 <span>(원)</span></th>
						<th align="right" width="20%">투자 잔액 <span>(원)</span></th>
						<th width="25%" style="padding-left: 100px;">연체율 <span>(%)</span></th>
				...
				<tbody>
					...
					<td style="padding-left: 100px;"><div id="self_overdue_rate" class="self_overdue_rate">0</div></td>
	...
	<div class="title">
		<ul>
			...
			<li class="date">
				<select id="year3" name="year3" class="dates" onchange="loadBadDebtLoad();">
					<? for($y=date(Y); $y>=2016; $y--) { echo "<option value='".$y."'>".$y."</option>\n"; } ?>
				</select>
				<select id="month3" name="month3" class="dates" onchange="loadBadDebtLoad();" style="display:none;">
					<option value=''>전체</option>
					<? for($m=1; $m<=12; $m++) { echo "<option value='".sprintf('%02d', $m)."'>".sprintf('%02d', $m)."월</option>\n"; } ?>
				</select>
			</li>
		</ul>
	</div>
	...
	<div class="wrap_info">
		<table class="info">
			<tbody>
				<tr>
					<th>헬로펀딩 안내사항</th>
					<td><a href="hellofunding_info.pdf" target="_blank"><img src="img/view.png"></a></td>
				</tr>
			</tbody>
		</table>
	</div>
...
function reset_chart2() {
	//$("#tot_amt").text("-");
}


// 채권 매각
function loadBadDebtLoad() {
	var year = $('#year3').val();
		...
		success: function(data) {
			if(data.result == 'SUCCESS') {
				for(i=0; i<data.sdata.length; i++) {
					...
					print_data += '<td>제' + data.sdata[i].start_num + '호</td>';
				...
				$('.disposal > tbody').append('<tr><td colspan="6" style="text-align:<?=(G5_IS_MOBILE)?'left':'center';?>">' + data.message + '</td>');
			}
		},
		error: function () { alert("통신 에러입니다. 잠시 후 다시 시도하여 주십시요."); }
	});
}
$(document).ready(function() { loadBadDebtLoad(); });

//재무 현황
function loadFinancialStatus(year) {
<? for($i=0; $i<count($FINAN); $i++) { ?>
//if(year=='<?=$FINAN[$i]['biz_year']?>') $('.finance').html('<tr><th><?=$FINAN[$i]['fin_contents']?></th><td><a href="<?=$FINAN[$i]['fin_url']?>" target="_blank"><img src="img/view.png"></a></td></tr>');
...
<? for($i=0; $i<count($STAFF); $i++) { ?>
	if(year == "<?=$STAFF[$i]['biz_year']?>") {
		if(gb == "mb") {
			$(".member").html("<tr><th>임직원</th><td><?=$STAFF[$i]['emp_member']?><span>명</span></td></tr> <tr><th>여신심사역</th><td><?=$STAFF[$i]['emp_simsa']?><span>명</span></td></tr> <tr><th>전문인력</th><td><?=$STAFF[$i]['emp_professional']?><span>명</span></td></tr>");
		}
		else {
...
<? for($i=0; $i<count($STOCK); $i++) { ?>
	if(year == "<?=$STOCK[$i]['biz_year']?>") {
		$(".stockholder").html('<tr><td><span style="color:navy"><?=$STOCK[$i]['major_shareholder']?></span></td></tr>');
	}
<? } ?>
}
