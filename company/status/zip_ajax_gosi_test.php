...
if( strlen($ym) > 4 ) {
	...
	$over_sql = " 
		SELECT 
			COUNT(state) AS totcnt, recruit_amount
		FROM 
			cf_product 
		WHERE 1
			AND state = '8' AND NOW() >= DATE_ADD(overdue_start_date, INTERVAL 30 DAY)
	";

	$over_row = sql_fetch($over_sql);

	$overdue_cnt_total = $over_row['totcnt'];
	$over_price = $over_row['recruit_amount'];
...
$ret['recruit_amount'] = round(($over_price / $tot_remain_amt) * 100, 2);  // 연체율
$ret['totcnt'] = $overdue_cnt_total;  // 연체건수
