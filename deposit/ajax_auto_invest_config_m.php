<style>
h4.title1 {font-size:10pt; font-family:NGB; color:#000; }
ul.this  { font-size:12px; font-family:NG; padding-left:24px }
ul.this li { list-style-type:decimal;list-style-position:outside; color:#000; line-height:18px }
ul.this li.nn { list-style-type:none;color:#000; }
.text2 { height:25px; padding:2px 5px; border:1px solid #AAA; vertical-align:middle; }
</style>
...
?>
  <table class="tblX" style="border-top:1px solid #284893;">
    <colgroup>
      <col style='width:28%'>
      <col style='width:%'>
    </colgroup>
    <tr>
      <td style="text-align:center;background:#F7F7F7;">자동투자그룹</td>
      <td style="color:green"><?=$LIST[$i]['grp_title']?></td>
		</tr>
		<tr>
			<td style="text-align:center;background:#F7F7F7;">상품군</th>
      <td><?=$print_category?></td>
		</tr>
		<tr>
			<td style="text-align:center;background:#F7F7F7;">투자기간</th>
      <td><?=$period_days?></td>
		</tr>
		<tr>
			<td style="text-align:center;background:#F7F7F7;">수익률(연)</th>
      <td><?=$profit?></td>
		</tr>
		<tr>
			<td style="text-align:center;background:#F7F7F7;">자동투자</th>
      <td>
		...
		<tr>
			<td style="text-align:center;background:#F7F7F7;">1회투자<br>설정금액</th>
      <td><input type="text" id="setup_amount<?=$i?>" value="<?=$print_amount?>" class="text2" maxlength="8" style="width:80px;text-align:right;" onKeyup="onlyDigit(this);" <?=($LIST[$i]['setup_idx'])?'':'disabled'?>> 만원</td>
		...
		<tr>
      <td colspan="2"><button type="button" id="btnToggle<?=$i?>" onClick="letsToggle(<?=$i?>);" class="btn_default" style="width:100%">상품설명 보기 ▼</button></td>
		</tr>
		<tr>
      <td colspan="2" align="left" style="background:#FAFAFA;height:0;padding:0">
...
<?
		if($prdt_count) {
			for($j=0; $j<$prdt_count; $j++) {
				...
				echo '
					<ul style="clear:both;">
						<li style="float:left;margin:4px 6px 2px;">' .
							$LIST[$i]['PRDT'][$j]['title'] . ' / ' .
...
<?=$summary?>
	...
	<p style="padding:8px 0 20px; text-align:center;"><button type="button" onClick="<?=$button_action?>" class="btn_blue" style="width:100%;">저장</button></p>
<?
	}
}
else {
?>
  <table class="tblX">
		<tr><td align="center">등록된 데이터가 없습니다.</td></tr>
	</table>
<?
}
?>
  ...
  <div style="margin-top:40px; padding:10px 10px; border:1px solid #AAA;background:#FAFAFA; border-radius:3px;">
    ...
    <ul class="this">
      ...
      <li>자동투자 'ON' 선택 시 투자자 유형별 최대 투자금액까지 설정 가능합니다.(일만원 단위)<br>
		...
		- 소득적격투자자: 2,000만원<br>
		- 전문투자자, 법인회원: 제한없음<br>
		(투자금은 1만원 단위로 설정하실 수 있습니다.)<br></li>
