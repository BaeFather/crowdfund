...
$sql = "
	...
	WHERE 1
		...
		AND TR_AMT_GBN = '10'
		AND ERP_TRANS_DT BETWEEN DATE_FORMAT(DATE_ADD(NOW(), INTERVAL -4 HOUR),'%Y%m%d%H%i%s') AND DATE_FORMAT(NOW(),'%Y%m%d%H%i%s')
...
if($TRADE) {
	...
	$LOG = sql_fetch("
		...
		WHERE 1
			AND FB_SEQ='".$TRADE['FB_SEQ']."' AND mb_no='".$member['mb_no']."'
			AND rdate BETWEEN DATE_FORMAT(DATE_ADD(NOW(), INTERVAL -4 HOUR),'%Y-%m-%d %H:%i:%s') AND DATE_FORMAT(NOW(),'%Y-%m-%d %H:%i:%s')");
	...
	if(!$LOG['cnt']) {
		...
		if( sql_query($sql3) ) {
			...
			// 한달전 알림 로그 삭제
			$limit_date = date('Y-m-d H:i:s', strtotime('first day of -1 month'));
			sql_query("DELETE FROM IB_deposit_notify_daylog WHERE rdate < '".$limit_date."'");
			...
			/*카카오 모듈로 교체 */
			/*
			$tcode = "hello003";			// TO-BE적용시 hello015 로 변경
