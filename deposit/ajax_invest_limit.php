...
if($member['member_type']=='1') {
	if($member['member_investor_type']=='3') {
		...
		$invest_possible_amount_str = number_format($member['invest_possible_amount_ds'])."원";
		if($member['invest_possible_amount'] > 0) $invest_possible_date = "상시가능";
	}
}
...
if($member['member_type']=='1' && in_array($member['member_investor_type'], array('1','2'))) {
	...
	$where.= " AND A.insert_date > '".date('Y-m-d', strtotime('-1 year'))."' ";
...
if($_SERVER['REMOTE_ADDR']=='220.117.134.164') {
	print_rr($member, 'font-size:12px;line-height:13px;');
}
*/
?>
...
<div class="type03 mb30">
	<table class="tblX">
		...
		<tr>
			<th rowspan="2">투자잔액<br/><span style="font-size:12px;color:#aaa">(상환중인 투자금액)</span></th>
			<th>전체</th>
			<th>P2P 대출 가이드라인 적용 이후</th>
		</tr>
		<tr>
		...
		<tr>
			...
			<td><?=($member['member_type']=='2' || $member['member_investor_type']=='3')? '구분없음' : number_format($member['invest_possible_amount_prpt']).'원'; ?></td>
		</tr>
		<tr>
			<th>투자가능일</th>
...
<div>
	<table class="tblX">
		<colgroup>
			...
			<col style='width:10%'>
			<col style='width:10%'>
			<col style='width:<?=($member['member_investor_type']=='1')?10:20;?>%'>
			<? if($member['member_investor_type']=='1') { ?><col style='width:10%'><? } ?>
		</colgroup>
		<tr align="center" style="background-color:#F7F7F7;border-top:2px solid #284893;">
			<th class="title border_r" rowspan="2">일자</th>
			<th class="title border_r" colspan="3">투자금액</th>
			<th class="title border_r" colspan="3">상환(예정)금액</th>
			<th class="title" <?=($member['member_investor_type']=='1')?'colspan="2"':'rowspan="2"';?>><span style="color:#FF2222">투자가능금액</span></th>
		</tr>
		<tr align="center" style="background-color:#F7F7F7;">
			<th class="title" style="opacity:0.6">부동산</th>
			<th class="title" style="opacity:0.6">동산/기타</th>
			<th class="title border_r">전체</th>
			<th class="title" style="opacity:0.6">부동산</th>
			<th class="title" style="opacity:0.6">동산/기타</th>
			<th class="title border_r">전체</th>
			<? if($member['member_investor_type']=='1') { ?>
			<th class="title"><span style="color:#FF2222;opacity:0.6">부동산</span></th>
			<th class="title"><span style="color:#FF2222">전체</span></th>
			<? } ?>
		</tr>
<?
	for($i=0,$j=1; $i<$day_count; $i++,$j++) {
...
?>
		<tr align="right" style="height:25px;background-color:<?=$tr_bgcolor?>">
			<td align="center" class="border_r"><?=$print_date?></td>
			<td><?=$RS['LIST'][$i]['start_amount_A']?></td>
			<td><?=$RS['LIST'][$i]['start_amount_B']?></td>
			<td class="border_r"><?=$RS['LIST'][$i]['start_amount']?></td>
			<td><?=$RS['LIST'][$i]['exit_amount_A']?></td>
			<td><?=$RS['LIST'][$i]['exit_amount_B']?></td>
			<td class="border_r"><?=$RS['LIST'][$i]['exit_amount']?></td>
			<? if($member['member_investor_type']=='1') { ?><td><?=$RS['LIST'][$i]['possible_amount_A']?></td><? } ?>
			<td><?=$RS['LIST'][$i]['possible_amount']?></td>
		</tr>
<?
		}
	}
