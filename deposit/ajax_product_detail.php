...
/*
if( in_array($_SERVER['REMOTE_ADDR'], array('220.117.134.164','211.248.149.48')) ) {
...
if($repay_count) {
	...
	for($i=0,$j=1; $i<$repay_count; $i++,$j++) {
		...
		$is_last_turn = ($j==$repay_count) ? true : false;
		...
		$RLIST['repay_title']         = (($INVEST['product_idx']=='6830' && $REPAY['turn']=='4') || ($INVEST['product_idx']=='6853' && $REPAY['turn']=='4') || ($INVEST['product_idx']=='6854' && $REPAY['turn']=='4')) ? $REPAY['turn'] . "회차 원금" : $REPAY['turn'] . "회차 원리금";		// 광주시 우산동 매각 상환
		...
		// 2020-08-05 최종회차에 이자만 상환을 해야할 경우를 위한 임시 방편 (면세점 연체건을 위한 설정)
		if( in_array($INVEST['product_idx'], $CONF['OVDPRDT']) && $is_last_turn ) {
			if($REPAY['SUCCESS']['invest_give_state']=='Y' && $REPAY['SUCCESS']['invest_principal_give']=='Y') {
				//
			}
			else {
				if( in_array($REPAY['SUCCESS']['invest_give_state'], array('S','Y')) ) {
		...
		if( $PSTATE['REPAY'][$i]['OVERDUE']['start_date'] ) {
			...
			$OLIST['repay_title']         = ($OVERDUE['day_count'] > 30) ? $REPAY['turn'] . '회차 연체이자 (' . $OVERDUE['day_count'] . '일)' : $REPAY['turn'] . '회차 지연이자 (' . $OVERDUE['day_count'] . '일)';
		...
		if( count($PSTATE['REPAY'][$i]['PARTIAL']) ) {
			for($k=0; $k<count($PSTATE['REPAY'][$i]['PARTIAL']); $k++) {
				...
				$PTLIST['paid_principal']      = $PARTIAL['LIST'][0]['paied_amount'];
				$PTLIST['last_repay_amount']   = $PARTIAL['LIST'][0]['interest'] + $PARTIAL['LIST'][0]['repay_principal'];


				// 2020-10-06 품번.1782 원금 과지급건에 대한 출력보정처리
				//if($INVEST['product_idx']=='1782') {
					if($PTLIST['paid']=='Y') {
						$PTLIST['repay_principal']   = $PARTIAL['LIST'][0]['paied_amount'];
						$PTLIST['last_repay_amount'] = $PARTIAL['LIST'][0]['interest'] + $PARTIAL['LIST'][0]['paied_amount'];
					}
				//}
		...
		<li style="list-style-type:decimal;margin-top:6px;">
			...
			<table style="width:96%;margin:6px 00">
				...
				<tr>
					...
					<td valign="top" style="padding:2px 6px">투자수익에 소득세(14%)와 주민세(1.4%가)가 추가되어 15.4%가 세금으로 산정됩니다.</td>
					<? } else { ?>
