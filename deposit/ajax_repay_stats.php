...
$sql = "
	...
	FROM
		`cf_product_invest` A
	LEFT JOIN
		`cf_product` B  ON B.idx=A.product_idx
	WHERE 1=1
		AND A.member_idx = '".$member['mb_no']."'
		AND A.invest_state = 'Y'
		AND B.state IN('1','2','4','5','8','9')";
if($type=='2') {
	$sql.= " AND (B.loan_start_date <= '".$sdate."' AND B.loan_end_date >= '".$edate."')";
}
else {
	$sql.= " AND (LEFT(B.loan_start_date,4) <= '".$sy."' AND LEFT(B.loan_end_date,4) >= '".$sy."')";
}
$sql.= "
	ORDER BY
		A.insert_date";
...
for($i=0,$j=1; $i<$rows; $i++,$j++) {
	...
	for($x=0; $x<$repay_count[$i]; $x++) {
		...
		$SIMUL[$date]['target_principal'] += ($PSTATE['REPAY'][$x]['paied']=='Y') ? 0 : $PSTATE['REPAY'][$x]['principal'];			// 지급예정 원금
		$SIMUL[$date]['target_interest']  += ($PSTATE['REPAY'][$x]['paied']=='Y') ? 0 : $PSTATE['REPAY'][$x]['interest'];				// 지급예정 이자

		/*
		if($row['invest_idx']=='143446') {
			if($repay_count[$i]==count($PSTATE['REPAY'])) print_rr($PSTATE['REPAY'][$x]);

			echo "<div style='font-size:12px'>" .
					 " 지급예정일 : " . $date .
					 " | 원금 : " . $PSTATE['REPAY'][$x]['principal'] .
					 " | 이자 : " . $PSTATE['REPAY'][$x]['interest'] .
					 " | 지급완료일 : " . $PSTATE['REPAY'][$x]['paied_date'] .
					 "</div>\n";
		}
		*/
...
if($sql_invest_arr && $sql_product_arr) {
	...
	for($i=0; $i<$rows; $i++) {
		$r = sql_fetch_array($res);
		if($r['banking_date']) {
			$REPAYLOG[$r['banking_date']]['payed_principal']= $r['principal'];
			$REPAYLOG[$r['banking_date']]['payed_interest'] = $r['interest'];
		}
		unset($r);
	}
}
...
for($i=0,$j=1; $i<$loop_count; $i++,$j++) {
	...
	$DATA[$date]['target_principal'] = $SIMUL[$date]['target_principal'];
	$DATA[$date]['target_interest']  = $SIMUL[$date]['target_interest'];

	$DATA[$date]['payed_principal']  = $REPAYLOG[$date]['payed_principal'];
...
<div class="mb30">
	...
	<div style="width:100%;padding:0 0 4px; display:inline-block;">
		<span class="left">
...
<? } ?>
	...
	<table class="tblX">
		...
		<tr style="height:25px;color:red;background-color:#999;">
			<td style="text-align:center;color:#fff;">합계</td>
			<td style="text-align:right;color:#fff;"><?=number_format($TOTAL['invest_count'])?> 건</td>
			<td style="text-align:right;color:#fff;"><?=number_format($TOTAL['invest_amount'])?> 원</td>
			<td style="text-align:right;color:#fff;"><?=number_format($TOTAL['target_principal'])?> 원</td>
			<td style="text-align:right;color:#fff;"><?=number_format($TOTAL['payed_principal'])?> 원</td>
			<td style="text-align:right;color:#fff;"><?=number_format($TOTAL['target_interest'])?> 원</td>
			<td style="text-align:right;color:#fff;"><?=number_format($TOTAL['payed_interest'])?> 원</td>
...
?>
		<tr style="height:25px;" onMouseOver="this.bgColor='#F7F7F7'" onMouseOut="this.bgColor=''">
			<td style="text-align:center"><a href="#" onClick="<?=$link?>"><?=$DATA_ARR[$i]?></td>
			<td style="text-align:right;color:<?=($DATA[$date]['invest_count'])?'#000':'#ccc'?>"><?=number_format($DATA[$date]['invest_count'])?> 건</td>
			<td style="text-align:right;color:<?=($DATA[$date]['invest_amount'])?'#000':'#ccc'?>"><?=number_format($DATA[$date]['invest_amount'])?> 원</td>
			<td style="text-align:right;color:<?=$fcolor1?>"><?=number_format($target_principal)?> 원</td>
			<td style="text-align:right;color:<?=($DATA[$date]['payed_principal'])?'#000':'#ccc'?>"><?=number_format($DATA[$date]['payed_principal'])?> 원</td>
			<td style="text-align:right;color:<?=$fcolor2?>"><?=number_format($target_interest)?> 원</td>
			<td style="text-align:right;color:<?=($DATA[$date]['payed_interest'])?'#000':'#ccc'?>"><?=number_format($DATA[$date]['payed_interest'])?> 원</td>
			<td style="text-align:center"><a href="#" onClick="<?=$link?>"><span class="btn_gray_s2"><?=($type==1)?'일별보기':'월별보기'?></span></a></td>
		</tr>
<?
	}
}
