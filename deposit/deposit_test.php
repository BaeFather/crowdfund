	...
	<div class="content">
		...
		<div class="deposit">
						...
						<div class="type03">
							<table>
								...
								<tr>
									...
									<td><?php // ECHO $RowsE['invest_usefee']?></td>
								</tr>
								<?
										$event_invest_count--;
									}
			...
			load_repay_stats = function() {
				$.ajax ({
					url : '/deposit/ajax_repay_stats<?=($mode=='test')?'.test':''?>.php',
					type: 'GET',
					data: {type:1},
					dataType: 'html',
					success: function(data) {
...
//-->
			...
			getRecommender = function() {
				$.ajax({
					url : "<?=($_SERVER['REMOTE_ADDR']=='220.117.134.164')?'ajax_recommender.php':'ajax_recommender.php'?>",
					type: 'GET',
					dataType: 'html',
					success: function(data) {
						if(data=='ERROR-DATA') { alert('시스템 오류 입니다. 관리자에 문의해주세요.'); return; }
...
$(document).ready(function() {
	setTimeout(function(){ realtime_point_check(); }, 3*1000);
	setInterval(function(){ realtime_point_check(); }, 10*1000);
});
...
function realtime_withdrawal_amount_check() {
	$.ajax({
		url: 'https://ext.hellofunding.co.kr/deposit/ajax_withdrawal_amount_check.php',
		type: 'post',
...
$('#money_status').on('click', function() {
	setTimeout(function(){ realtime_withdrawal_amount_check(); }, 0.3*1000);
});
