...
if($is_member)
{
	...
	$gstrWithdrawlPosibleAmount	=	$member['withdrawal_posible_amount'];
...
//환급계좌 등록 리다이렉션 URL
$bank_edit_url = "/bbs/member_confirm.php?url=".urlencode('/mypage/mypage.php#bank_edit');
					...
					<div class="type03 mb30">
						<table>
							...
							<tr>
								...
								<td style="color:#3366FF"><?=number_format($gstrWithdrawlPosibleAmount)?>원</td>
							</tr>
							</tbody>
						</table>
...
//-->
			...
			<?
			if ($gstrMemberId=="test9898") $auto_inv_dir = "auto_invest_n";
			else $auto_inv_dir = "auto_invest";
			?>
			<?
			if ($gstrMemberId=="test9898") include_once(G5_PATH.'/auto_invest_n/auto_inv_popup.php');
			else include_once(G5_PATH.'/auto_invest/auto_inv_popup.php');
			?>
			<script>
			auto_invest_config = function(){
				$.ajax({
					<? if ($gstrMemberId=="test9898") { ?>
					url : '/auto_invest_n/ajax_auto_invest_config2.php',
					<? } else { ?>
					url : '/auto_invest/ajax_auto_invest_config2.php',
					<? } ?>
					type: 'POST',
					success: function(data) {
						$('#ajax_return_txt').val(data);
...
<div id="withdraw" style="height:auto;" class="popbluetheme">
	...
	<div class="con">
		<div class="notes">출금 가능금액 <span id="realtime_point3" class="blue"><?=number_format($gstrWithdrawlPosibleAmount)?></span> 원</div>
		<div class="type01">
			<table>
				<tbody>
				<tr>
		...
		$.ajax({
			...
			success: function(data) {
				if(data=="ERROR-DATA") { alert("시스템 오류 입니다. 관리자에 문의해주세요."); return; }
					...
					$.blockUI({
						message: $('#detail'),
						css: { border:'0', cursor:'default', top:'30px',left:'25%',width:'930px', position:'fixed' }
					});
				}
			},
			error: function(e) { alert("네트워크 오류 입니다. 잠시 후 다시 요청하십시요."); return; }
...
$(document).ready(function() {
	setInterval(function() {
		$.ajax({
			url : "./ajax_withdrawal_amount_check.php",
			success: function(data) {
