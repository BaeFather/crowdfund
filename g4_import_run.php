...
<div id="wrapper">
    ...
    <div id="container">
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            // 중복체크
            $sql2 = " select count(mb_no) as cnt from {$g5['member_table']} where mb_id = '{$row['mb_id']}' ";
            $row2 = sql_fetch($sql2);
            if($row2['cnt'])
                continue;
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            // 중복체크
            $sql2 = " select count(lo_ip) as cnt from {$g5['login_table']} where lo_ip = '{$row['lo_ip']}' ";
            $row2 = sql_fetch($sql2);
            if($row2['cnt'])
                continue;
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            // 중복체크
            /*
            $sql2 = " select count(vi_id) as cnt from {$g5['visit_table']} where vi_ip = '{$row['vi_ip']}' and vi_date = '{$row['vi_date']}' ";
            $row2 = sql_fetch($sql2);
            if($row2['cnt'])
                continue;
            */
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            // 중복체크
            /*
            $sql2 = " select count(vs_date) as cnt from {$g5['visit_sum_table']} where vs_date = '{$row['vs_date']}' ";
            $row2 = sql_fetch($sql2);
            if($row2['cnt'])
                continue;
            */
