...
if($PRDT_STATE['code']=='A02') {
	...
	if($member['mb_id']) {
		...
		$invest_button = '<a href="/bbs/login.php?url='.urlencode('/investment/investment.php?prd_idx='.$PRDT['idx']).'" class="'.$button_class.'">투자하기</a>';
	}
}
else {
...
if($member['member_type']=='1') {
	if($member['member_investor_type']=='1') {
		$invest_possible_amount_ca = ($PRDT['category']=='2') ? $member['invest_possible_amount_prpt'] : $member['invest_possible_amount_ds'];        // 부동산 본인의 현재 투자 가능금액
