...
$sql = "
	SELECT
		A.idx, A.gr_idx, A.ai_grp_idx, A.invest_period, A.invest_days,
		A.recruit_amount, A.recruit_period_start, A.recruit_period_end, A.open_datetime, A.start_datetime, A.end_datetime, A.start_date, A.loan_start_date, start_hour,
		advance_invest, advance_invest_ratio
	FROM
		cf_product A
	WHERE
		A.idx = '".$prd_idx."'";
...
$PRDT['total_invest_count']  = $TMP['total_invest_count'];
...
IF($PRDT['ai_grp_idx'])
{
	IF(!$member['mb_no'])
	{
		$button_area1_data.= '<li class="reser_invest"><a href="#none" OnClick="fn_login_check();" class="btn_big_orange">자동투자설정</a></li>';
	} ELSE {
		$button_area1_data.= '<li class="reser_invest"><a href="/deposit/deposit.php?tab=5" class="btn_big_orange">자동투자설정</a></li>';
	}
}
...
if( $member['member_type']=='1' && in_array($member['member_investor_type'], array('1','2')) ) {
	...
	$sql2 = "SELECT idx FROM cf_product WHERE gr_idx='".$PRDT['gr_idx']."' AND state IN ('', '1') AND idx > '{$CONF['old_type_end_prdt_idx']}' ORDER BY idx";
	$res  = sql_query($sql2);
