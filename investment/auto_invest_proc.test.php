...
## * * * * * /usr/local/php/bin/php -q /home/crowdfund/public_html/investment/auto_invest_proc.test.php debug
###############################################################################
...
$TEST = array('product_idx' => "4572" , "member_idx" => "16218", "recruit_amount"=>"117250000", "mb_point"=>"200000000", "invested_amount"=>"10000");
...
WHILE($PRDT = sql_fetch_array($res)) {
	...
	ELSE {
		...
		$member_where.= " AND A.member_idx!='11838'";
			...
			IF(!$MB_INVESTED['cnt']) {
				...
				IF($invest_posible_flag) {
					...
					if($mode=='yes') {
						...
						// 원리금 수취권 번호 업데이트
						$prin_rcv_no = 'M' . $PRDT['AUTO_INV_MB'][$i]['member_idx'] . 'P' . $PRDT['idx'] . 'I' . $invest_idx;								//원리금 수취권 번호: M회원번호P상품번호I투자번호
