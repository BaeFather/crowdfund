...
?>
			...
			<b>투자금액</b> <input type="text" class="text" id="invest_value" name="invest_value" DISABLED placeholder="0" maxlength="9" onKeyUp="NumberFormat(this);" style="width:150px;border:4px solid #284893;text-align:right;"> 만원<br>
...
<div id="complete2" class="detail">
	...
	<!--<a href="/bbs/member_confirm.php?url=/mypage/mypage.php" id="main" class="btn_big_link">환급계좌 신청및변경</a> &nbsp;-->
		...
		success: function(data) {
			...
			if(data=="SUCCESS" || data=="SUCCESS-ADVANCE-INVEST") {
				...
				*/
				...
				else if(data=="ERROR-MAX-INVEST-AMOUNT-OVER"){ alert("투자 최대가능 한도액을 초과 하였습니다.\n전문투자자 최대투자가능 한도액 : <?=$pro_max_invest_amount?>원"); $("input[name='invest_value']").focus(); }
				else if(data=="ERROR-GROUP-INVEST-AMOUNT-LIMITED") { alert("동일차주상품 투자한도금액을 초과 하였습니다."); $("input[name='invest_value']").focus(); }
				else if(data=="ERROR-INVEST-AMOUNT-LIMITED") { alert("전체 상품에 대한 투자가능금액이 초과되어 투자를 진행 하실 수 없습니다."); }
				else if(data=="ERROR-ADVANCE-INVEST-AMOUNT") { alert("사전 투자 가능한 금액을 초과 입력 하셨습니다."); }
				else if(data=="ERROR-MONEY_UNIT")            { alert("투자금액중 만원 미만 단위 금액이 확인 되어 투자를 진행 하실 수 없습니다."); }
			...
			//else if(data=="ERROR-INVESTEDFROM-FINNQ")    { alert("타사 앱(핀크)을 통한 투자내역이 존재하여 추가 투자가 불가합니다."); }
			//else if(data=="ERROR-INVESTEDFROM-OLIGO")    { alert("타사 앱(올리고)을 통한 투자내역이 존재하여 추가 투자가 불가합니다."); }
			//else if(data=="ERROR-INVESTEDFROM-KAKAOPAY") { alert("타사 앱(카카오페이)을 통한 투자내역이 존재하여 추가 투자가 불가합니다."); }
				...
				else if(data=="ERROR-P2PCTR-FAIL-CANCEL")    { alert("중앙기록관리 오류\n관리자에게 문의하십시요."); }
				<? if(G5_TIME_YMD >= $CONF['online_invest_policy_sdate']) { ?>else if(data=="ERROR-INVEST-AMOUNT-LIMITED-PRPT") { alert("부동산 상품에 대한 투자가능금액이 초과되어 투자를 진행 하실 수 없습니다."); }<? } ?>
