...
##	2022-05-10 리스트 추출 방식 수정 : 단일 쿼리로 변경
###############################################################################
...
switch($CA) {
	case "A" : $category = '2'; $subtitle.= '부동산';		break;
	case "A2": $category = '2'; $subtitle.= '주택담보';		break;
	case "B" : $category = '1'; $subtitle.= '동산';		break;
	case "C" : $category = '3'; $subtitle.= 'SCF';		break;
	default  : $category = '';  $subtitle.= '전체';	 $CA = '';		break;
}
...
$common_field.= "(SELECT IFNULL(MAX(turn),0) FROM cf_product_success WHERE product_idx=A.idx AND invest_give_state='Y' AND overdue_start_date IS NULL) AS repay_count";
...
if($mode=="success") {
	...
	if($goods_officer || $tmp_special_user || $tmp_user ) {
		if($goods_officer) {
