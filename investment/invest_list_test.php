...
if($_SERVER['REMOTE_ADDR']=='220.117.134.164') {
	print_rr($psql0, 'text-align:left;font-size:12px');
	print_rr($psql1, 'text-align:left;font-size:12px');
	print_rr($psql, 'text-align:left;font-size:12px');
}
*/
...
if(G5_IS_MOBILE) {
	include_once("invest_list_m_test.php");
	return;
}
...
?>
			...
			<div>
				<div id="<?=$CATEGORY['id']?>" title="<?=$CATEGORY['title']?>">
					<div>
					<span id="fold_button" class="<?=($_COOKIE['tImgShow'])?'unfold':'fold';?>" style="z-index: 900;"></span>
				</div>
...
<?
	...
	for($i=0; $i<$plist_count; ++$i) {
		...
		switch($PLIST[$i]['category']) {
			...
			case '3' : $cFlag = '<li><span class="p_ca-C">헬로페이</span></li>'; break;
			default  : $cFlag = ''; break;
		}
		...
		$onlyVipFlag = ($PLIST[$i]['only_vip']=='1') ? '<li><span class="p_vip">지정투자</span></li>' : '';
		...
		if($PLIST[$i]['display']=='Y') {
			...
			if($PLIST[$i]['buttonAndCover']['code']=='B01') {
				$coverCaption = '<img src="/theme/2018/img/main/pro_ready.gif" style="width:100%;height:60px;">';
