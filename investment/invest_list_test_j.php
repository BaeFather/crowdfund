...
switch($CA) {
	...
	case "C" : $category = '3'; $subtitle.= '헬로페이';				 break;		//헬로페이(확정매출채권)
...
if($CA == '' || $mode != "success") {
	...
	$eWhere = "";
	$eWhere.= " AND A.display='Y' AND A.isTest=''";
	$eWhere.= " AND A.end_datetime > NOW()";
	if($search_title) {
		$search_title = $search_title;
		$eWhere.= " AND A.title LIKE '%".$search_title."%' ";
	}
...
if( in_array($search_div,array('','9')) ) {
	// 모집중인 상품
	$where0.= "";
	$where0.= " AND state='' AND display='Y' AND isTest=''";
	$where0.= " AND start_datetime<=NOW() AND invest_end_date=''";
	if($category) {
...
if($search_div) {
	if($search_div=="9")      $where.= " AND A.state='' AND A.start_datetime <= now() AND A.end_datetime > now() ";
...
$common_field = "A.idx, A.gr_idx, A.ai_grp_idx, A.state, A.category, A.title,
		...
		A.purchase_guarantees, A.advanced_payment, A.mortgage_guarantees, A.success_example, A.popular_goods, A.display, A.isConsor,
...
if($page==1 && $where0) {
	...
	$psql0 = "
		...
		ORDER BY
			invest_percent ASC,
			start_datetime DESC";
	//if($_SERVER['REMOTE_ADDR']=='220.117.134.164') { print_rr($psql0, 'text-align:left;font-size:12px'); }
	$pres0  = sql_query($psql0);
	$prows0 = sql_num_rows($pres0);
	if($prows0) {
		$ing_product_array = "";
		for($i=0,$j=1; $i<$prows0; ++$i,++$j) {
			$PLIST[$i] = sql_fetch_array($pres0);

			$ing_product_array.= $PLIST[$i]['idx'];
...
$psql = "
	...
	ORDER BY
		...
		invest_percent ASC,
		start_date DESC,
		RIGHT(A.start_datetime,8) ASC,
		A.start_num DESC,
		A.idx DESC
	LIMIT
		$sql_limit";
//if($_SERVER['REMOTE_ADDR']=='220.117.134.164') { print_rr($psql, 'text-align:left;font-size:12px'); }

$ing_product_array2 = explode(",", $ing_product_array);
print_r($ing_product_array2);
$pres  = sql_query($psql);
$prows = sql_num_rows($pres);
for($i=0; $i<$prows; ++$i) {
	$ROW = sql_fetch_array($pres);
	echo "$ROW[idx] ";
	if($ing_product_array) {
		echo " 11 ";
...
<? } ?>
			...
			<!-- 탭메뉴 //-->
			<ul class="tab_type03">
				<li onClick="location.href='<?=$_SERVER['SCRIPT_NAME']?>'" <?=($category=='')?'class="on"':'';?>>전체</li> <li class="line">|</li>
				...
				<li onClick="location.href='<?=$_SERVER['SCRIPT_NAME']?>?CA=B'" <?=($CA=='B')?'class="on"':'';?>>동산</li> <li class="line">|</li>
				<li onClick="location.href='<?=$_SERVER['SCRIPT_NAME']?>?CA=C'" <?=($CA=='C')?'class="on"':'';?>>헬로페이</li>
			</ul>
			<!-- 탭메뉴 //-->
...
switch($CA) {
	...
	case 'C'  : $CATEGORY = array('id'=>'CAC',   'title'=>'헬로페이', 'guide_image_a'=>'/images/investment/ca_ban_tit05.jpg', 'guide_image_b'=>'/images/investment/ca_ban05.jpg'); break;
...
?>
			...
			<div>
				<div id="<?=$CATEGORY['id']?>" title="<?=$CATEGORY['title']?>">
					<span id="fold_button" class="<?=($_COOKIE['tImgShow'])?'unfold':'fold';?>"></span>
...
<?
	...
	for($i=0; $i<$plist_count; ++$i) {
		...
		switch($PLIST[$i]['category']) {
			...
			case '2' : $cFlag = ($PLIST[$i]['mortgage_guarantees']=='1') ? '<li class="p_ca-A2">주택담보</li>' : '<li class="p_ca-A">부동산</li>'; break;
		...
		$conFlag = ($PLIST[$i]['isConsor']=='1') ? '<li class="p_con">컨소시엄</li>' : '';
