...
//의정부 역세권
if( in_array($prd_idx, array('1533','3982','4157','5039','5219','5402','5663','5829','6093','6364','6585','6653','6735','7055')) ) {
	$PRDT['stream_url1'] = 'ready';
}
...
if(G5_IS_MOBILE) {
	if($prd_idx <= 244) {
		...
		include_once('./investment_m_test.php');
	}
	return;
}
else {
...
add_stylesheet('<link rel="stylesheet" type="text/css" href="/investment/css/investment_info.css?ver=2019072511">', 0);
...
?>
	...
	<!--div style="text-align: center; border: 1px solid #ccc; padding: 15px 0;">
		<p>예상 당월 이자 <?=number_format($est_amount);?></p>
		<p>플랫폼이용료 <?=number_format($platform_tax);?></p>
		<p>소득세 <?=number_format($interest_tax);?></p>
		<p>지방세 <?=number_format($local_tax);?></p>
		<p><? $retention_tax = $interest_tax + $local_tax; echo $retention_tax; ?> 원천징수</p>
		<p>실수령액 <?=number_format($real_amount);?></p>
		<p style="color: red;">예상 순수익률 <?=$profit.'%';?></p> <!-- 소수점 첫째자리까지 표시, 나머지 절삭>
	</div-->
