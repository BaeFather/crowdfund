...
$sql = "
	...
	WHERE
		A.idx='".$prd_idx."'";
$sql.= ($syndi_id=='finnq' || ($is_admin=='super' || $developer || $goods_officer || $tmp_special_user || $tmp_user)) ? "" : " AND A.display='Y'";
$PRDT = sql_fetch($sql);
...
//남양주시 별내동 다가구 주택 건축자금 라이브 오류로 인한 임시처리
if( in_array($prd_idx, array('174','212','301')) ) {
	$PRDT['stream_url1'] = 'ready';
}

//원주 기업도시 근린생활시설 준공자금 라이브 종료 처리
if( in_array($prd_idx, array('402')) ) {
	$PRDT['stream_url1'] = 'ready';
}

//경기도 양주시 다세대주택 준공자금
if( in_array($prd_idx, array('321','299')) ) {
	$PRDT['stream_url1'] = 'ready';
}

//면목동 쉐어하우스 준공자금
if( in_array($prd_idx, array('462','509','553')) ) {
	$PRDT['stream_url1'] = 'ready';
}

//경기도 남양주시 화도읍 다세대주택 신축사업
if( in_array($prd_idx, array('735','674','577','523','478')) ) {
	$PRDT['stream_url1'] = 'ready';
}

//여수 봉산동 다세대주택 준공자금
if( in_array($prd_idx, array('233','262','328','364','367','389','507','524')) ) {
	$PRDT['stream_url1'] = 'ready';
}

//경기 가평군 현리 주상복합오피스텔
if( in_array($prd_idx, array('421','445','449')) ) {
	$PRDT['stream_url1'] = 'ready';
}

//경기도 가평군 묵안리 단독주택(풀빌라) 준공자금
if( in_array($prd_idx, array('458','483','502','620')) ) {
	$PRDT['stream_url1'] = 'ready';
}
//제주 신화역사공원 다세대주택 준공자금
if( in_array($prd_idx, array('279','281','336','342','363','372','375','387','409','416','434','442','448','457','460','464','493','536','599','699','848','1162','1225','1226')) ) {
	$PRDT['stream_url1'] = 'ready';
}
//제주 영어교육도시 다세대
if( in_array($prd_idx, array('548','578','627','628','700','886','1133')) ) {
	$PRDT['stream_url1'] = 'ready';
}
//천안시 사직동 주상복합
if( in_array($prd_idx, array('426','476','489','492','987')) ) {
	$PRDT['stream_url1'] = 'ready';
}
//천안시 사직동 주상복합
if( in_array($prd_idx, array('519','592','840','1150','1354')) ) {
	$PRDT['stream_url1'] = 'ready';
}

//영종도 하늘도시
if( in_array($prd_idx, array('1563','1625','1816','1932','2109','2319','2516')) ) {
	$PRDT['stream_url1'] = 'ready';
}


//다산신도시 역세권 근생
if( in_array($prd_idx, array('1550','1729','1854','1883','2069','2243','2621')) ) {
	$PRDT['stream_url1'] = 'ready';
}


//서초구다세대
if( in_array($prd_idx, array('2013','2258','2410','2827','3100','3351')) ) {
	$PRDT['stream_url1'] = 'ready';
}

//화곡동까치산역 오피스텔
if( in_array($prd_idx, array('1211','1484','1672','1843','2029','2123','2286','2495','3108','3286','3553','3554','3409','3555')) ) {
	$PRDT['stream_url1'] = 'ready';
}


//신림 역세권 숙박시설
if( in_array($prd_idx, array('3107','832')) ) {
	$PRDT['stream_url1'] = 'ready';
}
...
//상봉 도시형생활주택
if( in_array($prd_idx, array('3512','3824','4306','4515','4630','4745')) ) {
	$PRDT['stream_url1'] = 'ready';
}
...
//용인 주상복합
if( in_array($prd_idx, array('4222','4159','1730')) ) {
	$PRDT['stream_url1'] = 'ready';
}


//신림역 주상복합
if( in_array($prd_idx, array('1014','1093','3225','3247','3248')) ) {
	$PRDT['stream_url1'] = 'ready';
}

//여수 충무동
if( in_array($prd_idx, array('3271','3323','3577','3765','4074','4295','4472','4482','4734','5156','5340','5577')) ) {
	$PRDT['stream_url1'] = 'ready';
}


//광명시 광명동 주택
if( in_array($prd_idx, array('4629','4703','4723','4780','4933','2145','2228','2411','2742','2997','3208','3366','3510','3731','4091','4273','4519','5230','5361','5545','5793','5908','6025')) ) {
	$PRDT['stream_url1'] = 'ready';
}
...
//수원인계동
if( in_array($prd_idx, array('1232','1300','1731','1925','2102','2284','2448','2677','2899','3186','3331','3462','3472','3493','3551','3550','3788','4063','4253','4445','4694','4791','4912','4934','5116','5309','5513','5759')) ) {
	$PRDT['stream_url1'] = 'ready';
}
...
//의정부 역세권
if( in_array($prd_idx, array('1533','3982','4157','5039','5219','5402','5663','5829','6093','6364','6585','6653','6735','7055','7272','7554','7845')) ) {
	$PRDT['stream_url1'] = 'ready';
}
...
//성동구 오피스텔
if( in_array($prd_idx, array('3720','3787','3810','3823')) ) {
	$PRDT['stream_url1'] = 'ready';
}
...
//화곡동 단지 다세대
if( in_array($prd_idx, array('4396','5250')) ) {
	$PRDT['stream_url1'] = 'ready';
}
...
//용산구 다세대
if( in_array($prd_idx, array('6677','6234','5932','5828','5599','5442','5362','5240','5169','4744','4536','4045')) ) {
	$PRDT['stream_url1'] = 'ready';
}
...
//광진구 화양동 오피스텔
if( in_array($prd_idx, array('6282','4867','4835','4824','2363','2244')) ) {
	$PRDT['stream_url1'] = 'ready';
}


//로데오거리 아파트
if( in_array($prd_idx, array('6186','5805','5695','5490','5308','5097','4889','4666','4407','4384','4383','4381','4243','4044','3764','3511','3401','3155','2898','1782')) ) {
	$PRDT['stream_url1'] = 'ready';
}


//광주 우산동
if( in_array($prd_idx, array('6854','6853','6830','6734','6427','5600','5391','5350','5179','5000','4792','4340','4065','3983','3911','3641','3390','3307','3226','2801','2379','2362','2190','1884','1627')) ) {
	$PRDT['stream_url1'] = 'ready';
}
...
//용인 수지구 신봉동
if( in_array($prd_idx, array('5588')) ) {
	$PRDT['stream_url1'] = 'ready';
}
...
if(G5_IS_MOBILE) {
	if($prd_idx <= 244) {
		include_once('./investment_m_old.php');
	}
	else {
	...
	if($prd_idx <= 244) {
		include_once('./investment_old.php');
		return;
	}
}
...
add_stylesheet('<link rel="stylesheet" type="text/css" href="/investment/css/investment_info.css?ver=20220328_1">', 0);
...
<!-- 신한은행 배너 -->
	...
	<!--div id="detail_btn" class="detail_btn">
		<p>
			<a href="javascript:;" onclick="detailShow(this); return false;">
				상품상세보기
				<span><img src="/theme/2018/img/sub/up_btn01.png" alt="Up" width="40" class="upDownBtn"></span>
			</a>
		</p>
	</div-->
...
<?
	if(trim($PRDT['extend_9'])) {
		...
		// 이미지를 프래임 내부에서 호출되도록 하기 위한 작업
		$str = preg_replace("/<li>/i", "", $extend_9);
		$_ARR = explode("</li>", $str);
		for($i=0; $i<count($_ARR); $i++) {
...
?>
		<div class="file_upload container">
			<div class="file_upload_tit"><span style="font-size: 24px;">증빙자료</span></div>
				<ul class="file-list">
					<li>
						<img class="fr-fic fr-dii" src="/theme/2018/img/sub/file_icon.png">
						<a class="fr-file" href="/hello/image/aHR0cHM6Ly93d3cuaGVsbG9mdW5kaW5nLmNvLmtyL2RhdGEvcHJvZHVjdC84MjYwL3hibk5ST00zdXohello.html" target="_blank">헬로펀딩 고객확인서</a>
					</li>
					<li>
						<img class="fr-fic fr-dii" src="/theme/2018/img/sub/file_icon.png">
						<a class="fr-file" href="/hello/image/aHR0cHM6Ly93d3cuaGVsbG9mdW5kaW5nLmNvLmtyL2RhdGEvcHJvZHVjdC84MjYwL3hibk5ST00zdXohello.html" target="_blank">헬로펀딩 고객확인서</a>
