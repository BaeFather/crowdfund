...
?>
<?
function fn_hello_status_smssend_scf_1($report_idx) {
	echo $report_idx."<br/>";
	...
	if ($Row=sql_fetch_array($Result)) {
		...
		WHILE($Rowm=sql_fetch_array($Rm)) {
			...
			$sms_msg = $title."\n\n";
			$sms_msg .= $product."\n\n";
			$sms_msg .= "https://www.hellofunding.co.kr/hello_report/scf_report.php";
...
function fn_cf_product_admin_report_scf_1($today) {
	...
	$Query = "INSERT INTO
						   ...
						   VALUES
						   ('','".addslashes($strTitle)."','".addslashes($PRDT['title'])."','".addslashes($strContent)."',now())";
	sql_query($Query);
