...
function onlyAlphabet(e)  {
  str = e.value.replace(/[^A-Za-z ]/ig, '');
	e.value = str;
}

function onlyAlphabetUpper(e)  {
  str = e.value.replace(/[^A-Za-z ]/ig, '').toUpperCase();
	e.value = str;
}

function onlyAlphabetLower(e)  {
  str = e.value.replace(/[^A-Za-z ]/ig, '').toLowerCase();
	e.value = str;
}

function onlyAlphabet2(e)  {
  str = e.value.replace(/[^0-9A-Za-z\.\(\)\- ]/ig, '');
	e.value = str;
}
...
function chk_hobun(ttl) {
	if (!ttl) return;
	$.ajax({
		type: 'post',
		dataType: 'json',
		url: 'ajax_check_ho.php',
		data: {'ttl':ttl},
		success:function(data) {
			if (data["check_result"]=="Y") {
				alert(data["start_num"]+"호 등록가능");
			}
			else {
				$('#ho_check').val('');
				alert(data["start_num"]+"호 이미 등록됨 ("+data["check_count"]+"건)");
			}
			//console.log(data["start_num"]);
		},
