...
function get_p2pord_no() {
	...
	$sql = "SELECT * FROM p2pctr_order_no2 WHERE ymd='".date("Ymd")."' AND gubun='$gubun' LIMIT 1";
	$res = sql_query($sql);
	$cnt = sql_num_rows($res);
...
function get_inv_info($member_idx) {
	...
	$sql = "
		...
		FROM
			g5_member
		WHERE	1
			AND mb_no = '".$member_idx."' AND (mb_level>=1 AND mb_level<=5)";
	$row = sql_fetch($sql);
...
function get_p2pctr_limit($mb_id, $product_idx="") {
	...
	$sql = "SELECT mb_no, member_type, member_investor_type FROM g5_member WHERE mb_id='".$mb_id."' AND (mb_level>=1 AND mb_level<=5)";
	$row = sql_fetch($sql);
	if (!$row['mb_no']) return;
	$mno = $row['mb_no'];
...
function p2pctr_invest_register($member_idx, $product_idx) {
	...
	$psql = "SELECT goods_id, bank_inquiry_id FROM cf_product WHERE idx='$product_idx'";		// 쿼리수정 : 2022-03-18 배부장
	$pres = sql_query($psql);
	$prow = sql_fetch_array($pres);

	$goods_id = $prow["goods_id"];
	$bank_inquiry_id = $prow['bank_inquiry_id'];			// 변수설정 : 2022-03-18 배부장
	...
	//$data["investment_register_info"]["bank_inquiry_id"] = $bank_inquiry_id ;								// 대입변수 수정 : 2022-03-18 배부장
	...
	if ($mode=="send" AND !$rowi["investment_register_id"]) {
		...
		if ($resj["rsp_code"] == "A0000") {
			...
			$up_sql = "UPDATE cf_product_invest 
						  SET investment_register_id = '$investment_register_id', 
							  bank_inquiry_id='".$data["investment_register_info"]["bank_inquiry_id"]."' 
						WHERE idx = '".$rowi['idx']."' AND investment_register_id='' ";
			sql_query($up_sql);
			return true;
...
function p2pctr_invest_register_canc($member_idx, $product_idx, $invest_idx) {
	...
	if (!$member_idx OR !$product_idx OR !$invest_idx) return false;
