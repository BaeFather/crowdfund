...
function repayCalculation($product_idx, $invest_member_id='', $invest_period='', $loan_start_date='', $loan_end_date='', $invest_usefee='', $invest_usefee_type='', $light='') {

	global $CONF, $BANK, $VBANK;
	...
	$connX = sql_connect(G5_MYSQL_HOST, G5_MYSQL_USER, G5_MYSQL_PASSWORD, G5_MYSQL_DB) or die('REPAY DB Connect Error!!!');

	if($invest_member_id) {
		$r = sql_fetch("SELECT mb_no FROM g5_member WHERE mb_id='".$invest_member_id."'", G5_DISPLAY_SQL_ERROR, $connX);
	...
	--	B.comment
		FROM
			cf_product_invest A
		LEFT JOIN
			cf_product_copy B  ON A.product_idx=B.idx
		WHERE
			$where
		ORDER BY
			A.idx DESC";
	...
	if($ib_trust && $PRDT['loan_mb_no']) {
		$sql = "
			SELECT
				mb_id, member_type, is_creditor, is_owner_operator, mb_name, mb_co_name, mb_co_reg_num, mb_co_owner, mb_email, mb_hp,
				bank_code, account_num, bank_private_name, bank_private_name_sub, va_bank_code2, virtual_account2,
				mb_datetime
			FROM
				g5_member
			WHERE
				mb_no='".$PRDT['loan_mb_no']."'";
		$LOANER = sql_fetch($sql);
	...
	$TMP = sql_fetch("SELECT IFNULL(MAX(turn), 0) AS max_turn FROM cf_product_success WHERE product_idx='".$PRDT['idx']."'");
	$INI['max_paied_turn'] = $TMP['max_turn'];
	$TMP = NULL;
	...
	//$INI['calc_count'] = ($PRDT['invest_period']==1 && $PRDT['invest_days']>0) ? 1 : $PRDT['invest_period']+1;		// 1개월 미만 상품
	...
	$sql = "
		SELECT
			A.idx, A.amount, A.member_idx, A.product_idx, A.insert_date,
			B.withhold_tax_rate, B.invest_return, B.invest_period, B.invest_usefee, B.invest_usefee_type,
			C.mb_id, C.member_type, C.mb_name, C.mb_co_name, C.mb_co_reg_num,
		...
		FROM
			cf_product_invest A
			LEFT JOIN cf_product_copy B  ON A.product_idx=B.idx
			LEFT JOIN g5_member C  ON A.member_idx=C.mb_no
		WHERE
			$where2
		ORDER BY
			A.idx DESC";
	$result = sql_query($sql, G5_DISPLAY_SQL_ERROR, $connX);  $sql = NULL;
	$invest_rows = sql_num_rows($result);
	...
	for($j=0; $j<$invest_rows; $j++) {
		if( $r = sql_fetch_array($result) ) {
			...
			if($light=='') {
				$r2 = sql_fetch("SELECT member_idx, COUNT(idx) AS cnt_idx, IFNULL(SUM(amount),0) AS sum_amount FROM cf_product_invest WHERE member_idx='".$r['member_idx']."' AND invest_state IN('Y','R')", G5_DISPLAY_SQL_ERROR, $connX);
...
/*
				if($repay_day <= $static_repay_day) {			// 최종상환일이 약정정산지급일 보다 앞에 있거나 같을 경우 (1~5일)
					if($PRDT['loan_end_date'] < $PRDT['loan_end_date_orig']) {
						$REPAY[$x-1]['repay_date']   = $repay_day;
...
*/
	...
	$EDATE_OBJ = $TOTAL_DATE_OBJ = $DIFF_OBJ = NULL;
...
//$INI['invest_count'] = $PRDT['invest_count'];
	...
	// 투자건(투자자) 루프 시작
	///////////////////////////////
	for($i=0,$n=1; $i<$INI['repay_count']; $i++,$n++) {

		/////////////////////////
		// 투자 성패기록 추출
		/////////////////////////
		$REPAY[$i]['SUCCESS'] = sql_fetch("SELECT * FROM cf_product_success WHERE product_idx='".$PRDT['idx']."' AND turn='".$n."' ORDER BY idx ASC LIMIT 1", G5_DISPLAY_SQL_ERROR, $connX);

		for($j=0; $j<$INI['invest_count']; $j++) {

			$day_invest_interest = ($INVEST[$j]['amount'] * ($PRDT['invest_return']/100)) / 365;												// 일별 투자자 수익금

			$REPAY[$i]['LIST'][$j]['invest_idx']  = $INVEST[$j]['idx'];
			$REPAY[$i]['LIST'][$j]['mb_no']       = $INVEST[$j]['member_idx'];
			...
			if($ib_trust) {
				...
				$ib_sql = "
					...
					ORDER BY
						rdate DESC
					LIMIT 1";
				$IB = sql_fetch($ib_sql, G5_DISPLAY_SQL_ERROR, $connX);
			...
			$give_sql = "
				...
				WHERE 1
					...
					AND turn='".$n."'
					AND is_overdue='N'";
			//echo "<pre style='font-size:11px'>".$give_sql."</pre>\n";
			$GIVE = sql_fetch($give_sql, G5_DISPLAY_SQL_ERROR, $connX);
		...
		//$REPAY[$i]['LIST'][$j]['paied_amount'] = $GIVE['invest_amount'];		// 이자 실입금액 : invest_amount 사용 자제요망
			...
			$REPAY[$i]['LIST'][$j]['invest_interest'] = floor($day_invest_interest * $REPAY[$i]['day_count']);


			////////////////////////////////////////////
			// 일별 플랫폼이용료 설정 (365일 기준, 예외설정사항을 최우선으로 적용)
			////////////////////////////////////////////
			$EXTFEE_ROW = sql_fetch("SELECT idx, fee FROM cf_platform_fee WHERE member_idx='".$INVEST[$j]['member_idx']."' AND product_idx='".$product_idx."'", G5_DISPLAY_SQL_ERROR, $connX);
			if($EXTFEE_ROW['idx']) {
				$day_invest_usefee   = ($INVEST[$j]['amount'] * ($EXTFEE_ROW['fee']/100)) / 365;
			}
			else {
				// 플랫폼 수수료 면제 대상자처리 -> 일별 플랫폼 수수료를 0으로 설정
				$day_invest_usefee = ($REPAY[$i]['LIST'][$j]['remit_fee']=='1') ? 0 : ($INVEST[$j]['amount'] * ($PRDT['invest_usefee']/100)) / 365;
			}

			if($INVEST[$j]['invest_usefee_type']=='A') {
				// 투자자플랫폼이용료(분할징수) :::: 일별플랫폼이용료 * 일자수
				$REPAY[$i]['LIST'][$j]['invest_usefee'] = floor($day_invest_usefee * $REPAY[$i]['day_count']);	// 소수점이하 절사
			}
			else {
				// 투자자플랫폼이용료(만기일시징수)
				if($n==$INI['repay_count']) {
					$REPAY[$i]['LIST'][$j]['invest_usefee'] = floor($day_invest_usefee * $INI['total_day_count']);	// 소수점이하 절사
				}
				else {
					$REPAY[$i]['LIST'][$j]['invest_usefee'] = 0;
				}
			}

			// 원천징수액 계산 (대부업 회원 일때 원천징수 제로 처리)
			if($REPAY[$i]['LIST'][$j]['is_creditor']=='Y') {
				$REPAY[$i]['LIST'][$j]['TAX']['interest_tax'] = 0;
				$REPAY[$i]['LIST'][$j]['TAX']['local_tax']    = 0;
			}
			else {
				$REPAY[$i]['LIST'][$j]['TAX']['interest_tax'] = floor( ( ($REPAY[$i]['LIST'][$j]['invest_interest'] * 0.25) / 10) ) * 10;						// 당월 이자소득세 = 이자수익 * 0.25
				$REPAY[$i]['LIST'][$j]['TAX']['local_tax']    = floor( ( ($REPAY[$i]['LIST'][$j]['TAX']['interest_tax'] * 0.1) / 10) ) * 10;				// 당월 지방소득세(원단위 절사)
			}
			$REPAY[$i]['LIST'][$j]['TAX']['sum'] = $REPAY[$i]['LIST'][$j]['TAX']['interest_tax'] + $REPAY[$i]['LIST'][$j]['TAX']['local_tax'];		// 당월 세금 합계
			...
			$REPAY[$i]['LIST'][$j]['repay_principal'] = ($n < $INI['repay_count']) ? 0 : $INVEST[$j]['amount'];			// 상환원금
		...
		if( $n < count($REPAY) ) {
			...
			if($PRDT['category']=='3' && $PRDT['invest_days']>0) {
				$REPAY[$i]['repay_schedule_date'] = $INI['loan_end_date'];		// 1개월 미만 매출채권은 최종정산일과 동일하게 적용
			}
			else {
				$REPAY[$i]['repay_schedule_date'] = (in_array($product_idx, $EXCEPTION_PRODUCT)) ? $INI['loan_end_date'] : date("Y-m-d", strtotime("+5 day", strtotime($INI['loan_end_date'])));		// 최종정산일 +5일 적용시 (예외적용)
			}
		}
		...
		if($REPAY[$i]['SUCCESS']['overdue_start_date']>'0000-00-00') {
			...
			for($j=0; $j<$INI['invest_count']; $j++) {
				...
				$day_invest_interest = ($INVEST[$j]['amount'] * ($PRDT['overdue_rate']/100)) / 365;												// 일별 투자자 수익금 (연체이자율 대입)
				$day_invest_usefee   = ($INVEST[$j]['amount'] * ($PRDT['invest_usefee']/100)) / 365;												// 일별 플랫폼이용료 (365일 기준)

				$REPAY[$i]['OVERDUE_LIST'][$j]['invest_idx']  = $INVEST[$j]['idx'];
				$REPAY[$i]['OVERDUE_LIST'][$j]['mb_no']       = $INVEST[$j]['member_idx'];
				...
				$ovd_give_sql = "
					...
					WHERE 1
						...
						AND turn='".$n."'
						AND is_overdue='Y'";
				$OVD_GIVE = sql_fetch($ovd_give_sql, G5_DISPLAY_SQL_ERROR, $connX);
				...
				$REPAY[$i]['OVERDUE_LIST'][$j]['invest_interest'] = floor($day_invest_interest * $REPAY[$i]['OVERDUE']['day_count']);

				if($REPAY[$i]['OVERDUE_LIST'][$j]['remit_fee']=='1') $day_invest_usefee = 0;			// 플랫폼 수수료 면제 대상자처리 -> 일별 플랫폼 수수료를 0으로 설정

				// 투자자플랫폼이용료(분할징수) :::: 일별플랫폼이용료 * 일자수
				$REPAY[$i]['OVERDUE_LIST'][$j]['invest_usefee'] = floor($day_invest_usefee * $REPAY[$i]['OVERDUE']['day_count']);	// 소수점이하 절사

				if($REPAY[$i]['LIST'][$j]['is_creditor']=='Y') {
					// 대부업 회원 일때 원천징수 제로 처리
					$REPAY[$i]['OVERDUE_LIST'][$j]['TAX']['interest_tax'] = 0;
					$REPAY[$i]['OVERDUE_LIST'][$j]['TAX']['local_tax']    = 0;
				}
				else {
					$REPAY[$i]['OVERDUE_LIST'][$j]['TAX']['interest_tax'] = floor( ( ($REPAY[$i]['OVERDUE_LIST'][$j]['invest_interest'] * 0.25) / 10) ) * 10;						// 당월 이자소득세 = 이자수익 * 0.25
					$REPAY[$i]['OVERDUE_LIST'][$j]['TAX']['local_tax']    = floor( ( ($REPAY[$i]['OVERDUE_LIST'][$j]['TAX']['interest_tax'] * 0.1) / 10) ) * 10;				// 당월 지방소득세(원단위 절사)
				}
	...
	$PRDT = $LOANER = $INI = $INVEST = $MTOTAL_INVEST_SUM = $REPAY = $REPAY_SUM = $PAIED_SUM = $OVD_REPAY_SUM = $OVD_PAIED_SUM = NULL;
