...
$.extend({
    kcaptcha_load: function() {
        $('#kcaptcha_image').bind('click', function() {
            $.ajax({
                type: 'POST',
                url: '/inc/kcaptcha/kcaptcha_session.php',
                cache: false,
                async: false,
                success: function(text) {
                    $('#kcaptcha_image').attr('src', '/inc/kcaptcha/kcaptcha_image.php?t=' + (new Date).getTime());
                }
            });
        })
        .css('cursor', 'pointer')
...
function check_kcaptcha(input_key)
{
    if (typeof(input_key) != 'undefined') {
        ...
        $.ajax({
            type: 'POST',
            url: '/inc/kcaptcha/kcaptcha_result.php',
            data: {
        ...
        if (!captcha_result) {
			...
			//alert('Correct or enter the number of characters exceeded.\n\nPlease try again by clicking on the image.');
            input_key.select();
            return false;
        }
    }
