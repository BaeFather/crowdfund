...
<div id="popup">
	...
	<div><img src="/popup/images/notice_180607.jpg" style="width:100%;" alt="헬로펀딩 운영철학"></div>
	<div style="background:#eee;text-align:right; padding:6px 16px 6px;font-family:'spoqahansans'; ">
		<input type="checkbox" id="popupClose" value="<?=(24*7)?>">
		<label for="popupClose">일주일간 열지 않음</label>
...
$(document).ready(function() {
	if(get_cookie('popupOpen')==false) {
		$.blockUI({
			...
			<? } else { ?>
			css: { top:'5%', width:'700px', height:'748px', border:0, cursor:'default', left:'31%', transform:'translateX(-50%,-50%)' }
			<? } ?>
		});
	}
});
