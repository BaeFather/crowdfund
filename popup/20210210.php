...
<div id="popup">
	...
	<div style="width:100%;"><img src="/popup/images/20210210.jpg" class="repimg"></div>
	<div style="background:#eee;text-align:right; padding:6px 16px 6px; <?=($intime) ? "display:none" : ""; ?>">
		<input type="checkbox" id="popupClose" value="72">
		<label for="popupClose">3일 동안 열지 않음</label>
...
function noticePopOpen() {
	if(get_cookie('popupOpen')==false) {
		$.blockUI({
			...
			<? } else { ?>
			css: { top:'15%',left:'35%',width:'450px', border:0, cursor:'default' }
			<? } ?>
		});
	}
}
...
$(document).ready(function() {
	setTimeout(function() { noticePopOpen() }, 2500);
});
