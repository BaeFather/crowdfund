...
//-- 주민번호 가져오기 -----------------------
include_once(HF_PATH.'/mypage/crypt.php');

function getJumin($member_idx) {
	global $link2;
	$key = 'jumin';
	$row = sql_fetch("SELECT * FROM member_private WHERE mb_no = '".$member_idx."' ORDER BY idx DESC LIMIT 1", "", $link2);
	return decrypt($row['regist_number'], $key);
}
//-- 주민번호 가져오기 -----------------------
*/

$jumin = getJumin_new($member['mb_no']);
...
<!-- 본문내용 START -->
<script type="text/javascript" src="/js/mypage.js?v=1"></script>
<style>
...
<div class="con">
	<div id="step1" style="display:block">
		<form name="frm" id="frm" method="post">
			...
			<input type="hidden" name="receive_method"         value="1"> <!-- 원리금 수취방식 -->
		...
		<div style="margin:8px auto 8px;color:#FF3333;text-align:left;font-size:12px;padding:4px 10px 4px 10px; border:1px solid #aaa; border-radius:3px;background-color:#F8F8EF">
			- 환급계좌 내용을 <?=($is_certify)?'확인':'등록'?>하시고 아래 가상계좌발급 버튼을 클릭하십시요.
		</div>
		<div class="type01">
			<!-- 개인회원 정보 -->
			<table id="m_type1">
				<tbody>
					<tr>
						...
						<td <?=(G5_IS_MOBILE)?'style="padding-left:1%"':''?>>
							<input type="text"class="text" name="USERNM1" id="USERNM1" value="<?=($member['bank_private_name'])?$member['bank_private_name']:$member['mb_name']?>" placeholder="예금주" style="width:98%;">
							...
							<select name="strBankCode" id="strBankCode" style="width:98%;margin-top:2px;">
							...
							<input type="text" class="text small" name="strAccountNo" id="strAccountNo" value="<?=$member["account_num"]?>" onKeyup="onlyDigit(this);" placeholder="계좌번호" style="width:98%;margin-top:2px;">
						</td>
					</tr>
<? if($is_certify) { ?>
...
<? } else { ?>
					<tr>
						<th>* 주민번호</th>
						<td <?=(G5_IS_MOBILE)?'style="padding-left:1%"':''?>>
							<input type="password" class="text small" name="JUMINNO1" id="JUMINNO1" value="" maxlength="13" onKeyup="onlyDigit(this)" placeholder="숫자만 입력하십시요" style="min-width:50%;width:55%;">
							<input type="button" id="btn_acc_auth" class="btn_green" value="계좌인증" onClick="check_bank_new()" placeholder="숫자만 입력하십시요">
						</td>
					</tr>
<? } ?>
					<tr>
...
$('#btn_bank_account_regist').click(function() {
	...
	if(!input_check(f.USERNM1.value))           { alert('예금주명을 입력해 주세요.'); f.USERNM1.focus(); }
		...
		$.ajax({
			type: 'post',
			url: '/root_bank_account/account_proc_p.php',
			cache: false,
			data: fdata,
