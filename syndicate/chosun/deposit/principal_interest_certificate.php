...
require_once "/home/crowdfund/public_html/plugin/tcpdf/tcpdf.php";

// 핀크에서 넘어올 경우 회원정보 가져오기
...
if($member['mb_id']=='fintech01') {
	$special_print_name = "NH투자증권 <span style='font-size:13px'>(피델리스 Fin Tech 전문투자형 사모투자신탁 제1호 신탁업자 지위)</span>";
}
if($member['mb_id']=='fintech02') {
	$special_print_name = "NH투자증권 <span style='font-size:13px'>(피델리스 Fin Tech 전문투자형 사모투자신탁 제2호 신탁업자 지위)</span>";
}
if($member['mb_id']=='fintech03') {
	$special_print_name = "NH투자증권 <span style='font-size:13px'>(피델리스 대신 P2P 전문투자형 사모투자신탁 제1호 신탁업자 지위)</span>";
}
if($member['mb_id']=='fintech04') {
	$special_print_name = "피델리스 P2P 전문투자형 사모투자신탁 제1호";
}
if($member['mb_id']=='fintech05') {
	$special_print_name = "피델리스 핀테크인컴 전문투자형 사모투자신탁 제1호";
}
if($member['mb_id']=='directlending') {
	$special_print_name = "메리츠Direct Lending전문투자형사모투자신탁[혼합자산]";
}
if($member['mb_id']=='hanilfirst') {
	$special_print_name = "NH투자증권 (한일퍼스트 전문투자형 사모혼합자산투자신탁 제4호 신탁업자 지위)";		// 우선수익자명
}
...
$img_file = '/images/deposit/certificate_bg.jpg';
...
$pdf->SetXY(33, 198);
$pdf->Write(0, '수익증서 발행자');

$pdf->SetXY(33, 208);
$pdf->Write(0, '수익증서 판매자');
...
// 구분 : 내용
$pdf->SetXY(118, 115);
$pdf->Write(0, ($PRDT['category']=='2')?'부동산':'부동산외');
...
//수익증서발행자 : 내용
$pdf->SetXY(70, 198);
$pdf->Write(0, "\"".date($PRDT['title']."\"의 대출자"));
