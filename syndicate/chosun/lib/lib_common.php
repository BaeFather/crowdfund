...
function sql_connect($host, $user, $pass, $db=HF_MYSQL_DB)
{
	...
	// 로컬서버는 3306으로 지정
	if($host == "192.168.56.101") {
	  $db_port = 3306;
	}
	else {
		switch($db) {
			case 'crowdfundXX'       : $db_port = 3307;  break;
			case 'crowdfundXXmirror' : $db_port = 3307;  break;
			case 'crowdfundXXv1'     : $db_port = 3307;  break;
			case 'crowdfundXXv2'     : $db_port = 3307;  break;

			case 'crowdfundXXdev'    : $db_port = 3308;  break;
			case 'crowdfundYY'       : $db_port = 3308;  break;
			case 'crowdfundYYdev'    : $db_port = 3308;  break;
			case 'crowdfundYYmirror' : $db_port = 3308;  break;

			case 'hellosms_data'     : $db_port = 3308;  break;
			default                  : $db_port = 3308;  break;
		}
	}
...
function get_member($mb_id, $fields='*')
{
    global $hf, $INDI_INVESTOR, $CONF;
		...
		$row  = sql_fetch("SELECT $fields FROM {$hf['member_table']} WHERE mb_id=TRIM('$mb_id')");
		...
		$row['allow_date']        = $row2['allow_date'];
		$row['rights_start_date'] = $row2['rights_start_date'];
		$row['rights_end_date']   = $row2['rights_end_date'];

		// 투자 총액 (정상투자 및 강제반환금포함)
		$sql1 = "
			SELECT
			...
			FROM
				cf_product_invest
			WHERE
				member_idx='".$row['mb_no']."'
				AND invest_state='Y'"; 				// AND invest_state IN('Y','R')";
		$INVEST = sql_fetch($sql1);

		// 상환 완료된 투자상품 idx 추출 (1:이자상환중 2:상품마감 3:투자금모집실패 4:부실 5:중도일시상환 6:대출실행취소(기표전) 7:대출실행취소(기표후))
		$sql2 = "
			SELECT
			...
			WHERE (1)
				...
				AND B.state IN ('2','3','5','6','7')
				AND A.invest_state='Y'";		// AND A.invest_state IN('Y','R')";
		$RETURN = sql_fetch($sql2);
		...
		$new_sql1 = "
			...
			FROM
				cf_product_invest
			WHERE (1)
				AND member_idx='".$row['mb_no']."'
				AND invest_state='Y'";
		$new_sql1.= ( in_array($row['member_type'], array('2','3')) ) ? "" : " AND product_idx > '".$CONF['old_type_end_prdt_idx']."'";			// 개인-일반,소득적격투자자는 투자한도 적용시작일 이후의 투자잔액을 가져옴
		$INVEST_NEW = sql_fetch($new_sql1);
		...
		$new_sql1_A = "
			SELECT
				IFNULL(SUM(A.amount), 0) AS sum_amount
			FROM
				cf_product_invest A LEFT JOIN cf_product B ON A.product_idx=B.idx
			WHERE (1)
				AND A.member_idx='".$row['mb_no']."'
				AND A.invest_state='Y'
				AND B.category='2'";
		$new_sql1_A.= ( in_array($row['member_type'], array('2','3')) ) ? "" : " AND product_idx > '".$CONF['old_type_end_prdt_idx']."'";			// 개인-일반,소득적격투자자는 투자한도 적용시작일 이후의 투자총액을 가져옴
		$INVEST_NEW_A = sql_fetch($new_sql1_A);

		// 상환총액 (1:이자상환중 2:상품마감(정상상환) 3:투자금모집실패 4:부실 5:중도일시상환 6:대출실행취소(기표전) 7:대출실행취소(기표후))
		$new_sql2 = "
			SELECT
				IFNULL(SUM(A.amount), 0) AS sum_amount
			FROM
			...
			WHERE (1)
				AND A.member_idx='".$row['mb_no']."'
				AND A.invest_state='Y'
				AND B.state IN ('2','3','5','6','7')";		// AND A.invest_state IN('Y','R')";
		$new_sql2.= ( in_array($row['member_type'], array('2','3')) ) ? "" : " AND B.idx > '".$CONF['old_type_end_prdt_idx']."'";
		$RETURN_NEW = sql_fetch($new_sql2);
		...
		$new_sql2_A = "
			...
			WHERE (1)
				...
				AND B.state IN ('2','3','5','6','7')
				AND B.category='2'";
		$new_sql2_A.= ( in_array($row['member_type'], array('2','3')) ) ? "" : " AND B.idx > '".$CONF['old_type_end_prdt_idx']."'";
		$RETURN_NEW_A = sql_fetch($new_sql2_A);


		$row['ing_invest_amount']          = $INVEST['sum_amount'] - $RETURN['sum_amount'];								// 투자잔액(구)
		$row['ing_invest_amount_new']      = $INVEST_NEW['sum_amount'] - $RETURN_NEW['sum_amount'];				// 투자잔액
		$row['ing_invest_amount_new_prpt'] = $INVEST_NEW_A['sum_amount'] - $RETURN_NEW_A['sum_amount'];		// 투자잔액(부동산)


		$total_invest_amount       = $INVEST['sum_amount'];           //누적투자금액
		$total_invest_amount_new   = $INVEST_NEW['sum_amount'];       //누적투자금액 : 투자한도 설정 이후
		$total_invest_amount_new_A = $INVEST_NEW_A['sum_amount'];     //누적투자금액(부동산)

		if($row['member_type']=='1') {
			if( in_array($row['member_investor_type'], array('1','2')) ) {
				$invest_possible_amount = $INDI_INVESTOR[$row['member_investor_type']]['site_limit'] - $row['ing_invest_amount_new'];  // 개인 및 소득적격 투자자 각 투자한도 설정
			}
			else {
				$invest_possible_amount = $INDI_INVESTOR['3']['site_limit'];  // 전문 투자자 투자한도 무제한
			}

			if($row['member_investor_type']=='1') {
				$invest_possible_amount_A = $INDI_INVESTOR[$row['member_investor_type']]['prpt_limit'] - $row['ing_invest_amount_new_prpt'];  // 개인 및 소득적격 투자자 각 투자한도 설정
			}
		}
		else {
			$invest_possible_amount = $INDI_INVESTOR['3']['site_limit'];		// 법인 투자자 투자한도 무제한
		}

		$row['total_invest_amount']          = $total_invest_amount;
		$row['total_invest_amount_new']      = $total_invest_amount_new;
		$row['total_invest_amount_new_prpt'] = $total_invest_amount_new_A;	//부동산 투자금액
		$row['invest_possible_amount']       = $invest_possible_amount;
		$row['invest_possible_amount_prpt']  = $invest_possible_amount_A;		//부동산 투자 가능금액
	//$row['invest_possible_amount']       = ($invest_possible_amount < 0) ? 0 : $invest_possible_amount;
