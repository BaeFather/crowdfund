...
if(!$_COOKIE['pwdauth']) {
	...
	if($mb_password=='') {
		msg_go("", "/member/member_confirm.php?url=/mypage/mypage.php");
	}
	...
	if($pwd_valid) {
		setcookie("pwdauth", "Y", time()+300, "/");  //5분짜리 쿠키 발행
	}
	else {
		msg_go("비밀번호가 틀립니다.", "/member/member_confirm.php?url=/mypage/mypage.php");
	}
...
if (!($_POST['mb_password'] && check_password($_POST['mb_password'], $member['mb_password']))){
  alert('비밀번호가 틀립니다.');
	echo "<script>location.href='/bbs/member_confirm.php?url=/mypage/mypage.php'</script>";
}
*/
...
$link2 = sql_connect(HF_MYSQL_HOST2, HF_MYSQL_USER2, HF_MYSQL_PASSWORD2, HF_MYSQL_DB2);
$key   = 'jumin';

$member_private_query  = "SELECT * FROM member_private WHERE mb_no='".$member["mb_no"]."' LIMIT 0,1";
$member_private_result = mysqli_query($link2, $member_private_query);
$member_private_cnt    = mysqli_num_rows($member_private_result);
if($member_private_cnt> 0) {
	$member_private_row  = mysqli_fetch_array($member_private_result);

	$befor_private_str = $member["bank_private_name"]."||".decrypt($member_private_row["regist_number"],$key)."||".$member["bank_code"]."||".$member["account_num"];
	$private_mode = "modify";
}
else{
	$befor_private_str = "";
...
$returnurl = G5_URL."/root_mypage/join_sign_success.php";	// 성공시 이동될 URL
$errorurl  = G5_URL."/root_mypage/join_sign_fail.php";			// 실패시 이동될 URL
...
<!-- 본문내용 START -->
<script type="text/javascript" src="<?=($_SERVER['REQUEST_SCHEME']=='https')?"https://spi.maps.daum.net/imap/" : "http://dmaps.daum.net/";?>map_js_init/postcode.v2.js"></script>
...
<? if($member["member_type"]=="1") { ?>
							...
							<tr>
								<th>본인인증</th>
								<td>
									<input type="button" class="btn_green" value="본인인증하기" onClick="fnPopup();"> <span id="member_sign" style="color:tomato;">(휴대폰번호 교체시 재인증 하십시요.)</span>
								</td>
							</tr>
<? } else { ?>
							<tr>
								<th>상호명</th>
								<td><input type="text" class="text" name="mb_co_name" id="mb_co_name" value="<?=$member["mb_co_name"]?>"></td>
							</tr>
							<tr>
								<th>사업자번호</th>
								<td><input type="text" class="text" name="mb_co_reg_num" id="mb_co_reg_num" value="<?=$member["mb_co_reg_num"]?>"></td>
							</tr>
							<tr>
								<th>담당자명</th>
							...
							<tr>
								...
								<td style="line-height:30px;">
									...
									<p style="font-weight:bold;color:#153FA1">
										유효기간 : <?=preg_replace("/-/", ".", $member['rights_start_date'])?> ~ <?=preg_replace("/-/", ".", $member['rights_end_date'])?>
										<? if( time() >= strtotime('-1 month', strtotime($member['rights_end_date'])) ) { ?><a id="extend_btn" class="btn_blue" style="margin-left:30px;">기간연장</a><? } ?>
									</p>
									<? } ?>
								</td>
							</tr>
									...
									<div id="file_zone" style="display:none;">
										<div style="margin:16px 0 8px;">
											...
											<ul>
												...
												<li id="describe02">▶ 전문 투자자 : 금융투자협회 전문투자자 확인증 첨부</li>
											</ul>
											<ul>
...
?>
						...
						<table>
							<tbody>
								<tr>
									<th>알림설정</th>
									<td>
										<label><input type="checkbox" name="mb_mailling" id="mb_mailling" <?=($member["mb_mailling"]==1)?"checked='checked'":""?> value ="1"> 메일수신동의</label><br><br>
										<label><input type="checkbox" name="mb_sms" id="mb_sms" <?=($member["mb_sms"]==1)?"checked='checked'":""?> value ="1"> 문자수신동의</label>
									</td>
								</tr>
							</tbody>
						</table>
				...
				<div class="btnArea">
					<a href="javascript:;" class="btn_big_blue" id="check" onClick="member_modify()">확인</a>
					<a href="./break.php"  class="btn_big_gray" id="check" onClick="member_withdrawal()">회원탈퇴</a>
				</div>
					...
					<table id="m_type1" style="<?=($member["member_type"]==1)?"":"display:none"?>">
						<tbody>
							<tr>
								...
								<td>
<?
if($member["bank_private_name"]) {
	$bank_private_name = $member["bank_private_name"];
}
else {
	$bank_private_name = $member['mb_name'];
}
?>
									<input type="text"class="text"  name="USERNM1" id="USERNM1" value="<?=$bank_private_name?>" readonly> &nbsp;
...
<? if(!$receive_type1) { ?>
							<tr>
								<th>주민등록번호</th>
								<td><input type="text" class="text" name="JUMINNO1" id="JUMINNO1" value="<?=decrypt($member_private_row["regist_number"],$key)?>" onKeyup="onlyDigit(this)"> (숫자만 입력하십시요)</td>
							</tr>
<? } else { ?>
							<input type="hidden" name="JUMINNO1" id="JUMINNO1" value="<?=decrypt($member_private_row["regist_number"],$key)?>">
<? } ?>
						</tbody>
					</table>
					<!------------------->
...
?>
								...
								<td>
									<ul>
										<li id="doc1" style="padding:0 0 10px 10px;float:left">
											<label><input type="radio" name="receive_method" value="1" <?=($member["receive_method"]=='1' || ($member["receive_method"]!='2' || !$receive_type2)) ? 'checked':'';?> onClick="checkHighlight();"> <strong>환급계좌 상환</strong>
										...
										<li id="doc2" style="padding:0 10px 10px 10px;float:left">
											<label><input type="radio" name="receive_method" value="2" <?=($member["receive_method"]=='2') ? 'checked':'';?> onClick="checkHighlight();"> <strong>예치금 상환</strong>
							...
							<tr>
								<td style="line-height:20px;padding:15px 0;">
									...
									<div style="padding:6px 0 0 16px">원금은 대출자가 대출금을 상환할 시 5영업일 이내에 회원님이 선택하신 원리금 수취방식으로 해당 월 이자와 원금이 함께 지급 됩니다.<br>
									수익금은 매월 5일에 지급되며, 매월 1일에서 5일은 원리금 정산 자료 정리 기간으로 해당 기간에 환급계좌를 변경할 경우 다음 회차에 반영될 수 있으니 유의 바랍니다.</div>
								</td>
							</tr>
						...
						<tbody>
							<tr>
								...
								<td>
									...
									<input type="button" id='zip_btn' class="btn_green" onClick="search_address('zip_num','address_road','address_dong')" value="주소찾기"><br>
									...
									<!-- <input type="text" class="text small">&nbsp; <input type="button" class="btn_green" value="주소찾기"> -->
								</td>
							</tr>
							<tr>
								<th>상세주소</th>
							...
							<tr>
								<td><input type="text" class="text small" name="strAccountNo" id="strAccountNo" value="<?=$member["account_num"]?>" onKeyup="onlyDigit(this);">
									<input type="button" class="btn_green" value="계좌인증" onClick="check_bank_new()"> (숫자만 입력하십시요)
								</td>
							</tr>

							<tr>
								<th>가상계좌</th>
								<td><span style='font-weight:bold;color:#2222FF'><?=$BANK[$KSNET_VACT['BANK_CODE']]?> <?=$KSNET_VACT['VR_ACCT_NO']?> <?=$KSNET_VACT['CORP_NAME']?> <?=$vact_reg_button?></span></td>
							</tr>
							...
							<tr>
								...
								<td>
									<ul>
										<li id="doc1" style="padding:0 0 10px 10px;float:left">
											<label><input type="radio" name="receive_method" value="1" <?=($member["receive_method"]!='2' || !$receive_type2) ? 'checked':'';?> onClick="checkHighlight();"> <strong>환급계좌 상환</strong>
											<div style="padding:6px 10px 0 16px;font-size:0.96em">원리금을 회원님 본인명의의 실 계좌로 지급하여 드리는 방식입니다.<br>
											별도의 출금신청이 필요 없어 편리합니다.</div></label>
										</li>
										<li id="doc2" style="padding:0 10px 10px 10px;float:left">
...
<? } ?>
			...
			<div class="btnArea">
				<a href="javascript:;" class="btn_big_blue" id="check" onClick="<?=($member['member_type']=='2')?'private_process_for_business();' : 'private_process();'?>">확인</a>
			</div>
