...
$REQUEST['data']['aggregateDate'] = '20200619';		// 집계일(YYYYmmdd)
$REQUEST['data']['pageNumber'] = 1;								// 페이지번호 (최소:1)
$REQUEST['data']['pageSize'] = 200;								// 페이지번호 (최대:1000)
...
$sql = "
	SELECT
		...
		(SELECT syndi_userid FROM g5_member WHERE mb_no=A.member_idx) AS syndi_userid,
...
for($i=0,$j=1; $i<$totalCount; $i++,$j++) {
	...
	$ARR['data']['investList'][$i]['memberNumber']            = $ROW['syndi_userid'];
