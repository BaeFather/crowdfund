...
/*
회원번호 : $REQUEST['data']['memberNumber']
출금금액 : $REQUEST['data']['withdrawAmount']

$ARR['error'] = array('code'=>'WITHDRAW_EXCEED_AMOUNT',   'message'=>'출금금액초과');
$ARR['error'] = array('code'=>'WITHDRAW_INVALID_AMOUNT',  'message'=>'출금금액오류');
$ARR['error'] = array('code'=>'WITHDRAW_INVALID_ACCOUNT', 'message'=>'출금계좌오류');
$ARR['error'] = array('code'=>'WITHDRAW_INVALID',         'message'=>'오류출금');
*/

if($REQUEST['data']['withdrawAmount'] <= 0) {
	$ARR['error'] = array('code'=>'WITHDRAW_INVALID_AMOUNT', 'message'=>'출금금액오류');
	echo printJson($ARR); exit;
}

$MB['mb_point'] = get_point_sum($MB['mb_id']);		// 또또 다시 호출
if(!$MB['mb_point']) $MB['mb_point'] = 0;

if($MB['mb_point'] <= 0) {
	$ARR['error'] = array('code'=>'WITHDRAW_EXCEED_AMOUNT', 'message'=>'출금금액오류');
	echo printJson($ARR); exit;
}

if($MB['withdrawal_posible_amount'] < $REQUEST['data']['withdrawAmount']) {
	$ARR['error'] = array('code'=>'WITHDRAW_EXCEED_AMOUNT', 'message'=>'출금가능금액초과');
	echo printJson($ARR); exit;
}
...
if($WAITING_DATA['cnt']) {
	$ARR['error'] = array('code' => 'WITHDRAW_EXCEED_AMOUNT', 'message' => '금융기관측의 출금결과를 대기중인 요청이 있음.');
	echo printJson($ARR); exit;
}
...
if(!$RESULT || $RESULT['RCODE']!='00000000' || $RESULT['ERRMSG']=='ERROR:EMPTY_DATA') {
	$ARR['error'] = array('code'=>'WITHDRAW_INVALID', 'message'=>'기관통신오류');
	echo printJson($ARR); exit;
}
*/
...
if($RETURN_ARR['RCODE']=='00000000') {
	...
	if($RETURN_ARR['BALANCE_AMT']=='0') {
		$ARR['error'] = array('code' => 'WITHDRAW_EXCEED_AMOUNT', 'message' => '출금금액초과(예치잔액 없음)');
		echo printJson($ARR); exit;
	}
	if($RETURN_ARR['WITH_AMT']=='0') {
		$ARR['error'] = array('code' => 'WITHDRAW_EXCEED_AMOUNT', 'message' => '출금금액초과(출금제한 해제 금액 없음)');
		echo printJson($ARR); exit;
	}
	...
	if($REQUEST['data']['withdrawAmount'] > $avail_amount || $REQUEST['data']['withdrawAmount'] > $RETURN_ARR['WITH_AMT'])	{
		$ARR['error'] = array('code' => 'WITHDRAW_EXCEED_AMOUNT', 'message' => '출금금액초과(' . number_format($avail_amount) . '원 )');
		echo printJson($ARR); exit;
	}
	...
	if($RETURN_ARR['RCODE']=='') {
		$ARR['error'] = array('code' => 'WITHDRAW_INVALID', 'message' => '금융 전산망 통신 지연');
		echo printJson($ARR); exit;
	}
	else {
		$ARR['error'] = array('code' => 'WITHDRAW_INVALID', 'message' => $RETURN_ARR['ERRMSG']);
		echo printJson($ARR); exit;
	}
...
$REQUEST_ARR['TRAN_AMT']        = $REQUEST['data']['withdrawAmount'];								// 이체금액
$REQUEST_ARR['TRAN_MEMO']       = '핀크-헬로펀딩';																	// 이체계좌통장메모
...
if($RETURN_ARR['RCODE']=='00000000') {
	...
	$ARR['error'] = array('code' => 'WITHDRAW_EXCEED_AMOUNT', 'message' => '출금금액초과(최종 입금내역 발생기준 24시간 경과 후 부터 출금가능)');
	echo printJson($ARR); exit;
	...
	if( $LAST_ORDER['idx'] ) {
		...
		if($last_fbseq) {
			...
			if($RETURN_ARR2['ORI_FB_REQCODE']=='00000000') {
				...
				$RESULT_ARR = array('code' => 'WITHDRAW_INVALID', "message" => "금융기관으로의 전송이 정상적으로 실행되지 않았습니다.\n출금 요청을 다시 실행 하여주십시요.");
	...
	$ARR['error'] = array('code' => 'WITHDRAW_INVALID', 'message' => addSlashes($error_msg));
	echo printJson($ARR); exit;
...
if($banking_ok) {
	...
	$sql = "
		...
		SET
			...
			mb_no     = '".$MB['mb_no']."',
			mb_id     = '".$MB['mb_id']."',
			req_price = '".$REQUEST['data']['withdrawAmount']."',
			regdate   = NOW(),
			GUAR_SEQ  = '".$GUAR_SEQ."'";
	if(sql_query($sql)) {
		...
		$point   = (int)$REQUEST['data']['withdrawAmount'] * -1;
		$content = '예치금 출금';
	...
	$ARR['data']['withdrawRequestNo']     = $withdrawRequestNo;							// 출금요청번호
	$ARR['data']['withdrawCompleteYn']    = "Y";														// 출금완료여부 (즉시이체:Y, 지연이체:N)
	$ARR['data']['withdrawDetailContent'] = "";															// 출금방식에 관한 별도 공지 ex) 익일 23:00 시에 일괄 이체 처리합니다.
	$ARR['data']['deposit']               = get_point_sum($MB['mb_id']);		// 남은 예치금
