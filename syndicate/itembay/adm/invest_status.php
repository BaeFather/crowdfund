...
$sql_search.= " AND A.syndi_id='$syndi_id' AND category='2'";
...
$sql = "
	SELECT
		...
		C.category, C.mortgage_guarantees
	FROM
		cf_product_invest A
	LEFT JOIN
		g5_member B
...
for($i=0; $i<$rcount; $i++) {
	...
	$LIST[$i]['date'] = $ROW['insert_date'];

	IF($ROW['category'] == "2" && $ROW['mortgage_guarantees'] == "")	//부동산
	{
		$intPercent =  0.008;
		$LIST[$i]['invest_cnt1']++;
		$LIST[$i]['invest_amount1'] += $ROW['amount'];
		$LIST[$i]['invest_cms1'] = $ROW['amount'] * $intPercent;
	} ELSE {
		$intPercent =  0.002;
		$LIST[$i]['invest_cnt2']++;
		$LIST[$i]['invest_amount2'] += $ROW['amount'];
		$LIST[$i]['invest_cms2'] = $ROW['amount'] * $intPercent;
	}

	$LIST[$i]['invest_cnt'] += 1;
	$LIST[$i]['invest_amount'] += $ROW['amount'];
	$LIST[$i]['invest_cms']  = $LIST[$i]['invest_cms1']  + $LIST[$i]['invest_cms2'];
...
for($i=0; $i<count($TMP); $i++) {
	...
	IF($LIST[$i]['category'] == "2" && $LIST[$i]['mortgage_guarantees'] == "")	//부동산
	{
		 $intPercent =  0.008;
		 $LIST[$i]['invest_cnt1'] -= $AUTO['invest_cnt'];
		 $LIST[$i]['invest_amount1'] -= $AUTO['sum_invest_amount'];
		 $LIST[$i]['invest_cms1'] = $LIST[$i]['invest_amount'] * $intPercent;
	} ELSE {	// 주택담보
		 $intPercent =  0.002;
		 $LIST[$i]['invest_cnt2'] -= $AUTO['invest_cnt'];
		 $LIST[$i]['invest_amount2'] -= $AUTO['sum_invest_amount'];
		 $LIST[$i]['invest_cms2'] = $LIST[$i]['invest_amount'] * $intPercent;
	}

	$LIST[$i]['invest_cms'] = $LIST[$i]['invest_amount'] * $intPercent;
	...
	for($i=0,$j=1; $i<$list_count; $i++,$j++) {
		?>
							<tr bgcolor="<?=$tr_bgcolor?>">
								...
								<td style="text-align:right;"><?=number_format($LIST[$i]['invest_cnt'])?> 건

								<br />
								(부동산 : <?=number_format($LIST[$i]['invest_cnt1'])?> 건 / 주담대 <?=number_format($LIST[$i]['invest_cnt2'])?> 건)
								</td>
								<td style="text-align:right;"><?=number_format($LIST[$i]['invest_amount'])?> 원

								<br />
								(부동산 : <?=number_format($LIST[$i]['invest_amount1'])?> 원 / 주담대 <?=number_format($LIST[$i]['invest_amount2'])?> 원)
								</td>
								<td style="text-align:right;"><?=number_format($LIST[$i]['invest_cms'])?> 원

								<br />
								(부동산 : <?=number_format($LIST[$i]['invest_cms1'])?> 원 / 주담대 <?=number_format($LIST[$i]['invest_cms2'])?> 원)
