...
<div class="con">
	<div id="step1" style="display:block">
		...
		<div style="margin:8px auto 8px;color:#FF3333;text-align:left;font-size:12px;padding:4px 10px 4px 10px; border:1px solid #aaa; border-radius:3px;background-color:#F8F8EF">
			- 환급계좌 내용을 확인하시고 아래 가상계좌발급 버튼을 클릭하십시요.
		</div>
		<div class="type01">
			<!-- 법인회원 정보 -->
			<table id="m_type1">
				<tbody>
					<tr>
						...
						<td <?=(G5_IS_MOBILE)?'style="padding-left:1%"':''?>>
							<input type="text"class="text" name="bank_private_name" id="bank_private_name" value="<?=($member['bank_private_name'])?$member['bank_private_name']:$member['mb_co_name']?>" placeholder="예금주" style="width:98%;" readonly>
							<input type="text"class="text" name="bank_private_name_sub" id="bank_private_name_sub" value="<?=$member["bank_private_name_sub"]?>" placeholder="(부기명)" style="width:98%;height:20px;margin-top:2px;">
					...
					<tr>
						<th>* 사업자번호</th>
						<td <?=(G5_IS_MOBILE)?'style="padding-left:1%"':''?>>
							<input type="text" class="text" name="mb_co_reg_num" id="mb_co_reg_num" value="<?=$mb_co_reg_num?>" maxlength="13" onKeyup="onlyDigit(this)" style="width:98%;" readonly>
						</td>
					</tr>
					<tr>
						<th>* 주소</th>
...
$('#btn_bank_account_regist').click(function() {
	...
	if(!input_check(f.zip_num.value)) { alert('우편번호를 입력해 주세요.'); $('#zip_btn').focus(); }
		...
		$.ajax({
			type: 'post',
			url: '/root_bank_account/account_proc_c.php',
			cache: false,
			data: fdata,
