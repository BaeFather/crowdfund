...
for($i=0,$j=1; $i<$rows; $i++,$j++) {
	...
	if($PSTATE['PRDT']['state']==2 || $PSTATE['PRDT']['state']==5) {

		$date = ($type==2) ? $row['return_date'] : substr($row['return_date'], 0, 7);
		$REPAY_PRINCIPAL[$date] += $row['amount'];			// 신)원금 지급 기록 기준의 원금상환 정리 2017-06-26

		//$date = ($type==2) ? $PSTATE['INI']['loan_end_date'] : substr($PSTATE['INI']['loan_end_date'], 0, 7);
		//$REPAY_PRINCIPAL[$date] += $PSTATE['INI']['principal'];			// 구)투자종료일 기준의 원금상환 정리 (이전까지 원금 상환에 대한 기록을 하지 않았었음)
	}

	$repay_count[$i] = count($PSTATE['REPAY']);
	for($x=0; $x<$repay_count[$i]; $x++) {
		//색인값을 정산일이 아닌 정산표기일로 변경 : 2017-03-09
		$date = ($type==1) ? substr($PSTATE['REPAY'][$x]['repay_schedule_date'], 0, 7) : $PSTATE['REPAY'][$x]['repay_schedule_date'];
	//$date = ($type==1) ? substr($PSTATE['REPAY'][$x]['repay_day'], 0, 7) : $PSTATE['REPAY'][$x]['repay_day'];

		$SIMUL[$date]['target_principal'] += $PSTATE['REPAY'][$x]['principal'];
		$SIMUL[$date]['target_interest']  += $PSTATE['REPAY'][$x]['interest'];    // 예상 세후이자
	}
	unset($PSTATE);
}
//print_rr($REPAY_PRINCIPAL);
...
//투자내역
$sql = "SELECT LEFT(insert_date, $date_length) AS date, COUNT(idx) AS invest_count, SUM(amount) AS invest_amount
				FROM `cf_product_invest`
				WHERE member_idx='".$member['mb_no']."' AND invest_state='Y'
				GROUP BY LEFT(insert_date, $date_length)
				ORDER BY insert_date, idx";
//echo "<font style='font-size:11px'>".$sql."</font>";
$res = sql_query($sql);
$rows = sql_num_rows($res);
for($i=0; $i<$rows; $i++) {
...
if($sql_invest_arr && $sql_product_arr) {
	$sql = "SELECT LEFT(banking_date, $date_length) AS banking_date, SUM(invest_amount) AS invest_amount
					FROM `cf_product_give`
					WHERE invest_idx IN($sql_invest_arr) AND product_idx IN($sql_product_arr)
					GROUP BY LEFT(banking_date, $date_length)
					ORDER BY banking_date, idx";
	//echo $sql;
	$res  = sql_query($sql);
	$rows = sql_num_rows($res);
	for($i=0; $i<$rows; $i++) {
...
for($i=0,$j=1; $i<$loop_count; $i++,$j++) {
	...
	$DATA[$date]['target_principal'] = $SIMUL[$date]['target_principal'] - $REPAY_PRINCIPAL[$date];		// 2017-04-02 수정
//$DATA[$date]['target_principal'] = $SIMUL[$date]['target_principal'];
	$DATA[$date]['payed_principal']  = $REPAY_PRINCIPAL[$date];
...
<? } ?>
	...
	<table class="tblX">
		...
		<tr style="height:25px;background-color:#F7F7F7;">
			<th><b>건수</b></th>
			<th><b>금액</b></th>
			<th><b>예정</b></th>
			<th><b>지급</b></th>
...
?>
		<tr align="right" style="height:25px;" onMouseOver="this.bgColor='#F7F7F7'" onMouseOut="this.bgColor=''">
			<td align="center"><a href="#" onClick="<?=$link?>"><span style="font-size:0.8em;"><?=$DATA_ARR[$i]?></span></td>
