...
$sql_search = " WHERE A.r114_userid='$syndi_id' AND A.mb_level='1'";
...
$sql = "
	SELECT
		...
		(SELECT COUNT(idx) FROM cf_product_invest WHERE member_idx=A.mb_no AND invest_state='Y' AND syndi_id='".$syndi_id."') AS invest_count,
		(SELECT SUM(amount) FROM cf_product_invest WHERE member_idx=A.mb_no AND invest_state='Y' AND syndi_id='".$syndi_id."') AS invest_amount
	{$sql_common}
	{$sql_search}
	{$sql_order}
	LIMIT
...
// 등록회원
$sql   = "SELECT COUNT(*) AS cnt FROM g5_member WHERE syndi_id='$syndi_id' AND mb_level='1'";
$row   = sql_fetch($sql);
$count1 = $row['cnt'];
...
?>
						<tr bgcolor="<?=$tr_bgcolor?>">
							...
							<td style="text-align:center;"><?=masterDecrypt($LIST[$i]['mb_hp'], false);?></td>
