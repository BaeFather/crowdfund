...
$sql_search = " AND B.display='Y'";
$sql_search.= " AND (A.invest_state = 'Y' OR A.invest_state = 'R')";		// 정상투자금 및 대출취소건에 대한 투자금
$sql_search.= " AND (B.category<>3)";																		// 확정 매출 채권은 신디사측에 노출되지 않는다.
$sql_search.= " AND (A.syndi_id='".$syndi_id."')";											// 확정 매출 채권은 신디사측에 노출되지 않는다.

$sql_group = " GROUP BY A.product_idx ";
$sql_order = " ORDER BY B.open_datetime DESC, A.product_idx DESC";
				...
				<div class="tabArea" style="display:block;padding:30px;border-left:1px solid #ccc; border-right:1px solid #ccc; border-bottom:1px solid #ccc;">
					...
					<table class="tblX">
						...
						<tr style="background-color:#EFEFEF">
							...
							<th scope="col" colspan="2" style="text-align:center;"><b><?=$_CONF['SYNDI_NAME']?></b></th>
						</tr>
						<tr style="background-color:#EFEFEF">
