...
?>
    <tr style="height:50px;">
      ...
      <td style="text-align:left;">
        <label style="color:#FF2222"><input type="radio" name="auto_invest_flag<?=$i?>" id="auto_invest_flag<?=$i?>" value="1" <?=($LIST[$i]['setup_idx'])?'checked':''?> onClick="fActivation('<?=$i?>');" style="-webkit-appearance:radio;"> ON</label>
		<br/>
        <label ><input type="radio" name="auto_invest_flag<?=$i?>" id="auto_invest_flag<?=$i?>" value="0" <?=($LIST[$i]['setup_idx']=='')?'checked':''?> onClick="fActivation('<?=$i?>');"  style="-webkit-appearance:radio;"> OFF</label>
      </td>
      <td><input type="text" id="setup_amount<?=$i?>" value="<?=$print_amount?>" class="text2" maxlength="8" style="width:73px;text-align:right;" onKeyup="onlyDigit(this);" <?=($LIST[$i]['setup_idx'])?'':'disabled'?>> 만원</td>
...
<?=str_replace("<img","<img width=100%",$summary)?>
				</div>
			</td>
    </tr>
<?
