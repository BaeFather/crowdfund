...
if( @preg_match("hellofunding\.kr/i", $_SERVER['HTTP_HOST']) || ($_SERVER['REQUEST_SCHEME'] == 'http')) {
	header("Location: https://r114.hellofunding.co.kr");
}
...
include_once($DOCUMENT_RT.'/syndication_config.php');
...
if( in_array(@$_SERVER['REMOTE_ADDR'], array('220.117.134.164','220.117.134.166','220.117.134.205')) ) {
...
/*
	...
	{
		if(preg_match("/r114\.hello/i", $_SERVER['HTTP_HOST'])) {
			if(!preg_match("/r114/i", $_COOKIE['PHPSESSID'])) {
				$_CONF['SESS_ID'] = uniqid('r114'.date('ymd'));
				session_destroy();
