...
$YmdHis = preg_replace("/(-|:| )/", "", HF_TIME_YMDHIS);
...
<div id="content">
	<div class="location" style="margin-bottom:15px; "><span><a href="<?=G5_URL?>/investment/invest_list.php">투자하기</a></span><b class="blue"><?=($is_advance_invest=='Y')?'사전':'';?>투자 설정: <?=$PRDT["title"]?></b></div>
	...
	<div class="content invest_detail">
		...
		<form method="post" name="frm" id="frm">
			...
			<input type="<?=($_COOKIE['debug_mode'])?'text':'hidden';?>" name="invest_possible_amount" id="invest_possible_amount"  value="<?=$invest_possible_amount?>">
		...
		<h2 class="big" style="margin-top:15px;line-height:1em;font-size:30px !important;"><?=($is_advance_invest=='Y')?'<span class="red">[사전투자]</span> ':'';?><?=$PRDT["title"]?></h2>
		...
		<div class="my_invest">
			<table align="center" style="width:100%;font-size:14px;">
				<? if($shinhan_vacct) { ?>
				<!--tr style="border-bottom:2px solid #284893">
					<td style="width:200px; padding-left:5px">▣ 나의 가상계좌</td>
				...
				<tr style="border-bottom:2px solid #284893">
					<td style="width:200px; padding-left:5px" colspan=2>나의 가상계좌
						<span style="font-weight:bold;color:#4A6FE2"><?=$BANK[$member['va_bank_code2']]." ".$member['virtual_account2']?></span>
						<span style="color:#FF2222;font-size:14px">가상계좌에 예치금을 입금한 후 투자해주세요.</span>
					</td>
				</tr>
				<? } ?>
...
<? if( $member['member_type']=='1' && in_array($member['member_investor_type'], array('1','2')) ) { ?>
			<ul style="clear:both;display:inline-block;padding:0; font-size:14px;">
				...
				<li style="float:left; margin-left:30px; color:brown">* <strong>P2P대출 가이드라인</strong>에 의해 개인(<?=$INDI_INVESTOR[$member['member_investor_type']]['title']?>)의 경우 동일 대출자에게는 <strong><?=price_cutting($INDI_INVESTOR[$member['member_investor_type']]['group_product_limit'])?>원</strong>까지만 투자가 가능합니다.</li>
			</ul>
...
?>
		...
		<div class="textarea">
			<div class="agree">
				<label style="padding:0 10px 5px 0;">
				  <a href="/etc/content.php?co_id=provision2" target="_blank" class="blue2">투자이용약관</a>에 동의합니다. &nbsp;
		...
		<p align="left" style="padding:10px 0 10px 40px; ">
			<span id="" style="font-size:14px;">▣ 본 사이트에서 이루어지는 "P2P 투자상품"의 매매,중개의 주체는 "헬로펀딩" 이며,<br/>&nbsp;&nbsp;&nbsp;&nbsp;"부동산114주식회사"는 어떠한 관계도 없습니다.</span>
		</p>
...
<div id="complete2" class="detail">
	...
	<a href="<?=BSC_URL?>/deposit/deposit.php"><span class="btn_big_blue">투자내역확인</span></a>
</div>
...
$('#yes').on('click', function() {
	...
	$.ajax({
		url : "/root_investment/investment_proc.php",
		type: "POST",
		data : ajax_data,
		...
		success: function(data) {
			...
			?>
				...
				else if(data=="ERROR-MIN-PRICE")             { alert("투자 최소 금액 미만 입니다. 투자금액을 확인해 주세요."); $("input[name='invest_value']").focus(); }
				else if(data=="ERROR-MAX-PRICE")             { alert("투자 최대 금액을 초과 하였습니다. 투자금액을 확인해 주세요. "); $("input[name='invest_value']").focus(); }
				else if(data=="ERROR-INVEST-AMOUNT_LIMITED_PRPT") { alert("부동산 상품에 대한 투자 가능 금액이 초과되어 투자를 진행 하실 수 없습니다."); }
				else if(data=="ERROR-INVEST-AMOUNT_LIMITED") { alert("전체 상품에 대한 투자 가능 금액이 초과되어 투자를 진행 하실 수 없습니다."); }
				else if(data=="ERROR-ADVANCE-INVEST-AMOUNT") { alert("사전 투자 가능한 금액을 초과 입력 하셨습니다."); }
				else if(data=="ERROR-ADVANCE-INVEST-DATE")   { alert("사전 투자 가능 기간이 종료 되었습니다."); window.location.replace('/investment/investment.php?prd_idx=<?=$prd_idx?>'); }
				else if(data=="ERROR-ADVANCE-INVEST-END")    { alert("사전투자가 마감 되었습니다."); window.location.replace('/investment/investment.php?prd_idx=<?=$prd_idx?>'); }
				else { alert(data); }
...
$(document).ready(function(){
	...
	$("#btn_invest").click(function(evt){
		...
		if(invest_value=='' || invest_value=='0') { alert("투자 금액을 입력해주세요"); $("input[name='invest_value']").focus(); return; }
		...
		if(invest_possible_amount==0) { alert("투자 가능 금액 (모집 금액)이 없습니다."); return; }
	//if(invest_possible_amount==0) { alert("회원님의 투자한도가 초과되어 더 이상 투자를 진행하실 수 없습니다.\n투자스케쥴러를 참조하십시요."); return; }
		if(invest_possible_amount < invest_value) { alert('투자 가능 금액을 초과 입력 하셨습니다.\n\n - 현재 투자 가능 금액:    <?=price_cutting($invest_possible_amount)?>원'); return; }
		if($("input[name='ajax_invest_value']").val() < min_invest_limit) { alert("투자 최소 금액은 " + min_invest_limit + "원 입니다."); $("input[name='invest_value']").focus(); return; }
