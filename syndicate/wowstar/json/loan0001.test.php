...
## DESC:	펀딩 상품 리스트
## MESSAGE_ID:	LOAN0001
## REQUEST_URL: http://www.hellofunding.co.kr/syndicate/wowstar/json/loan0001_test.php
################################################################################
/*
...
$where = "";
$where.= " AND A.display='Y' AND A.scrap_out='' AND A.isTest='' AND A.only_vip='' AND A.platform LIKE '%".$_CONF['SYNDI_ID']."%' AND (A.category IN(1,2) OR (A.category='3' AND category2='2'))";
	...
	for($i=0; $i<$rows; $i++) {
		...
		*/
		...
		$LIST[$i] = array(
			...
			'FUND_PROG_RATE'     => $P['invest_percent'],							//펀딩 진행윯
