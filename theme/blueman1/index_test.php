...
$STORY = array('url'=>'http://blog.naver.com/hellofunding/221133848659', 'image'=>'', 'subject'=>'10만원으로 건물투자를!?', 'target'=>'_blank', 'media_url'=>'
https://serviceapi.nmv.naver.com/flash/convertIframeTag.nhn?vid=4D2EE78B0E0E27F48398280C11654CEED952&outKey=V127afa44a0d440ddb263528e2fcba8005b17474b4c312f5f39c1528e2fcba8005b17&width=544&height=306');
...
$PRESS = array('url'=>'http://news.naver.com/main/read.nhn?mode=LSD&mid=sec&oid=009&aid=0004060761&sid1=001', 'image'=>'/images/main/report20171205.jpg', 'subject'=>'투자자 보호장치 마련한 P2P업체는? ', 'target'=>'_blank');
...
?>
						<li><!-- onClick="location.href='<?=$ALIST[$i]['DETAIL_URL']?>';" style="cursor:pointer"-->
							<div class="invest_tit">
								NEW 투자상품 안내 – [ <?=$print_sdate?> 투자시작 ]

								<div class="flag_red" style="display:<?=($ALIST[$i]['STEREAM_URL1'])?'block':'none'?>"><img src="/images/investment/live_icon01.gif"></div>
								<div class="flag_green" style="display:<?=($ALIST[$i]['ADVANCE_INVEST_RATIO'])?'block':'none'?>">사전투자 <?=(int)$ALIST[$i]['ADVANCE_INVEST_RATIO']?>%</div>
							</div>
							<div class="invest_contents">
								<div class="img"><?=$main_image_tag?></div>
								<div class="info">
									<ul class="info_tit">
										<li style="height:24px;overflow:hidden;"><?=$ALIST[$i]['TITLE']?></li>
...
?>
		...
		<ul id="list_info1">
			...
			<li><p><?=$PRNT_SUBJECT['total_invest']?></p><p><?=price_cutting($DP['total_invest'])?>원</p></li>
			<li><p><?=$PRNT_SUBJECT['total_repay']?></p><p><?=price_cutting($DP['total_repay'])?>원</p></li>
			<li><p><?=$PRNT_SUBJECT['invest_ing_amount']?></p><p><?=price_cutting($DP['invest_ing_amount'])?>원</p></li>
			<li><p>연체율
			<span id="btn1" style="display:inline-block;border-radius:100px;background-color:#fff;width:18px;height:18px;color:#000;font-size:15px;font-family:'NGB';font-weight:400;text-align:center;cursor:pointer;">!</span>
			<!--연체율이란-->
      ...
      연체율 = 연체잔여원금 / 대출잔여원금 <br/>
			...
			<li><p>부실률
			<span id="btn2" style="display:inline-block;border-radius:100px;background-color:#fff;width:18px;height:18px;color:#000;font-size:15px;font-family:'NGB';font-weight:400;text-align:center;cursor:pointer;">!</span>
			<!--부실률이란-->
      ...
      부실률 = 부실잔여원금 / 총 누적대출액<br/>
			...
			<li>
				<!--<p>펀딩성공</p>-->
				<p onClick="location.href='<?=G5_URL?>/investment/invest_list.php?mode=success';" style="cursor:pointer; margin:0 0 0 0;padding:5px 0;width:100px;font-size:16px;font-weight:500;color:gold;border-radius:6px;border:2px solid gold;">성공사례</p>
		...
		<div class="news">
			<ul>
				<li>
					...
					<div class="story">
                        <p><? if($STORY['media_url']) { ?><iframe src="<?=$STORY['media_url']?>" style="width:323px;height:205px;margin-bottom:-12px;"></iframe><?} else { ?><a href="<?=$STORY['url']?>" target="<?=$STORY['target']?>"><img src="<?=$STORY['image']?>" width="323" height="205"></a><? } ?></p>
